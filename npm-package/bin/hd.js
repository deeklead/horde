...
function getBinaryPath() {
  ...
  if (!fs.existsSync(binaryPath)) {
    console.error(`Error: hd binary not found at ${binaryPath}`);
...
function main() {
  ...
  // Spawn the native hd binary with all command-line arguments
  ...
  child.on('error', (err) => {
    console.error(`Error executing hd binary: ${err.message}`);
    process.exit(1);
  });
