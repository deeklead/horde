# Release Process for Horde

This document describes the release process for Horde, including GitHub releases and npm packages.
...
A Horde release involves multiple distribution channels:

1. **GitHub Release** - Binary downloads for all platforms
2. **npm** - Node.js package for cross-platform installation (`@horde/gt`)
...
- npm: Publish access to `@horde` organization
...
npm pack
npm install -g ./horde-gt-0.2.0.tgz
gt version  # Should show 0.2.0

# Cleanup
npm uninstall -g @horde/gt
rm horde-gt-0.2.0.tgz
```
...
Horde follows [Semantic Versioning](https://semver.org/):
