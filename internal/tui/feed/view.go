...
func (m *Model) render() string {
	...
	// Raid panel (middle)
	raidPanel := m.renderRaidPanel()
	sections = append(sections, raidPanel)
...
func (m *Model) renderTree() string {
	...
	// Sort warbands by name
	rigNames := make([]string, 0, len(m.warbands))
	for name := range m.warbands {
		rigNames = append(rigNames, name)
	}
	sort.Strings(rigNames)

	for _, rigName := range rigNames {
		warband := m.warbands[rigName]
		...
		byRole := m.groupAgentsByRole(warband.Agents)

		// Render each role group
		roleOrder := []string{"warchief", "witness", "forge", "shaman", "clan", "raider"}
		for _, role := range roleOrder {
			...
			// For clan and raiders, show as expandable group
			if role == "clan" || role == "raider" {
				lines = append(lines, m.renderAgentGroup(icon, role, agents))
			} else {
				// Single agents (warchief, witness, forge)
...
// renderAgentGroup renders a group of agents (clan or raiders)
...
func (m *Model) renderEvent(e Event) string {
	...
	case "patrol_started", "raider_checked":
		symbolStyle = EventUpdateStyle
	case "raider_nudged", "escalation_sent", "signal":
		symbolStyle = EventFailStyle // Use red/warning style for nudges and escalations
	case "charge", "banner", "muster", "boot":
		symbolStyle = EventCreateStyle
	case "handoff", "drums":
		symbolStyle = EventUpdateStyle
	default:
