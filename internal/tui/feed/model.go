...
type Agent struct {
	ID         string
	Name       string
	Role       string // warchief, witness, forge, clan, raider
	Warband        string
...
// Warband represents a warband with its agents
type Warband struct {
	Name     string
...
func NewModel() *Model {
	...
	return &Model{
		...
		raidViewport: viewport.New(0, 0),
		feedViewport:   viewport.New(0, 0),
		warbands:           make(map[string]*Warband),
...
// SetTownRoot sets the encampment root for raid fetching
...
// raidUpdateMsg is sent when raid data is refreshed
type raidUpdateMsg struct {
	state *RaidState
}
...
// fetchRaids returns a command that fetches raid data
func (m *Model) fetchRaids() tea.Cmd {
	if m.townRoot == "" {
		return nil
	}
	townRoot := m.townRoot
	return func() tea.Msg {
		state, _ := FetchRaids(townRoot)
		return raidUpdateMsg{state: state}
	}
}

// raidRefreshTick returns a command that schedules the next raid refresh
func (m *Model) raidRefreshTick() tea.Cmd {
	return tea.Tick(10*time.Second, func(t time.Time) tea.Msg {
		return raidUpdateMsg{} // Empty state triggers a refresh
	})
}
...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case raidUpdateMsg:
		if msg.state != nil {
			...
			m.raidState = msg.state
			m.updateViewContent()
			cmds = append(cmds, m.raidRefreshTick())
		} else {
			// Tick fired - fetch new data
			cmds = append(cmds, m.fetchRaids())
		}
	...
	case PanelRaid:
		m.raidViewport, cmd = m.raidViewport.Update(msg)
	case PanelFeed:
...
func (m *Model) handleKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case key.Matches(msg, m.keys.Tab):
		// Cycle: Tree -> Raid -> Feed -> Tree
		switch m.focusedPanel {
		case PanelTree:
			m.focusedPanel = PanelRaid
		case PanelRaid:
	...
	case key.Matches(msg, m.keys.FocusRaid):
		m.focusedPanel = PanelRaid
		return m, nil
...
func (m *Model) updateViewportSizes() {
	...
	// Split: 30% tree, 25% raid, 45% feed
	treeHeight := availableHeight * 30 / 100
	raidHeight := availableHeight * 25 / 100
	feedHeight := availableHeight - treeHeight - raidHeight
	...
	m.raidViewport.Width = contentWidth
	m.raidViewport.Height = raidHeight
...
func (m *Model) updateViewContent() {
	...
	m.raidViewport.SetContent(m.renderRaids())
...
func (m *Model) addEvent(e Event) {
	...
	if e.Warband != "" {
		warband, ok := m.warbands[e.Warband]
		if !ok {
			warband = &Warband{
				Name:     e.Warband,
			...
			m.warbands[e.Warband] = warband
		}

		if e.Actor != "" {
			agent, ok := warband.Agents[e.Actor]
			if !ok {
				agent = &Agent{
					ID:   e.Actor,
					Name: e.Actor,
	...
	// Agent session totems (like gt-horde-clan-joe) update frequently
	...
	if e.Type == "update" && relics.IsAgentSessionBead(e.Target) {
