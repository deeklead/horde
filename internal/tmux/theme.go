// Package tmux provides theme support for Horde tmux sessions.
package tmux
...
// WarchiefTheme returns the special theme for the Warchief session.
// Gold/dark to distinguish it from warband themes.
func WarchiefTheme() Theme {
	return Theme{Name: "warchief", BG: "#3d3200", FG: "#ffd700"}
}

// ShamanTheme returns the special theme for the Shaman session.
// Purple/silver - ecclesiastical, distinct from Warchief's gold.
func ShamanTheme() Theme {
	return Theme{Name: "shaman", BG: "#2d1f3d", FG: "#c0b0d0"}
}
...
// AssignTheme picks a theme for a warband based on its name.
// Uses consistent hashing so the same warband always gets the same color.
