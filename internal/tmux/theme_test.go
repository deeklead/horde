...
func TestAssignTheme_Deterministic(t *testing.T) {
	// Same warband name should always get same theme
	theme1 := AssignTheme("horde")
	theme2 := AssignTheme("horde")
...
func TestAssignTheme_Distribution(t *testing.T) {
	// Different warband names should (mostly) get different themes
	// With 10 themes and good hashing, collisions should be rare
	warbands := []string{"horde", "relics", "myproject", "frontend", "backend", "api", "web", "mobile"}
	themes := make(map[string]int)

	for _, warband := range warbands {
		theme := AssignTheme(warband)
		themes[theme.Name]++
	}

	// We should have at least 4 different themes for 8 warbands
	if len(themes) < 4 {
		t.Errorf("Poor distribution: only %d different themes for %d warbands", len(themes), len(warbands))
	}
}
...
func TestWarchiefTheme(t *testing.T) {
	theme := WarchiefTheme()

	if theme.Name != "warchief" {
		t.Errorf("WarchiefTheme().Name = %q, want %q", theme.Name, "warchief")
	}

	// Warchief should have distinct gold/dark colors
	if theme.BG == "" || theme.FG == "" {
		t.Error("WarchiefTheme() has empty colors")
	}
}
