...
// SignalSession sends a message to a Claude Code session reliably.
...
func (t *Tmux) SignalSession(session, message string) error {
	...
	// See: https://github.com/anthropics/horde/issues/307
...
// Same pattern as SignalSession but targets a pane ID (e.g., "%9") instead of session name.
...
// Note: This replaces the current process with tmux summon.
func (t *Tmux) AttachSession(session string) error {
	_, err := t.run("summon-session", "-t", session)
	return err
}
...
//	function to get the Shaman online. Regex is acceptable here.
//
// Steady-State (use AI observation instead):
//
//	Once any AI agent is running, observation should be AI-to-AI:
//	- Shaman starting raiders → use 'hd shaman pending' + AI analysis
//	- Shaman restarting → Warchief watches via 'hd peek'
//	- Warchief restarting → Shaman watches via 'hd peek'
//
// See: hd shaman pending (ZFC-compliant AI observation)
// See: hd shaman trigger-pending (bootstrap mode, regex-based)
...
func (t *Tmux) SetStatusFormat(session, warband, worker, role string) error {
	// Get icon for role (empty string if not found)
	icon := roleIcons[role]

	// Compact format - icon already identifies role
	// Warchief: 🎩 Warchief
	// Clan:  👷 horde/clan/max (full path)
	// Raider: 😺 horde/Toast
	var left string
	if warband == "" {
		// Encampment-level agent (Warchief, Shaman)
		left = fmt.Sprintf("%s %s ", icon, worker)
	} else if role == "clan" {
		// Clan member - show full path: warband/clan/name
		left = fmt.Sprintf("%s %s/clan/%s ", icon, warband, worker)
	} else {
		// Warband-level agent - show warband/worker
		left = fmt.Sprintf("%s %s/%s ", icon, warband, worker)
	}
...
func (t *Tmux) SetDynamicStatus(session string) error {
	...
	// hd status-line reads env vars and drums to build the status
	right := fmt.Sprintf(`#(hd status-line --session=%s 2>/dev/null) %%H:%%M`, session)
...
// ConfigureHordeSession applies full Horde theming to a session.
// This is a convenience method that applies theme, status format, and dynamic status.
func (t *Tmux) ConfigureHordeSession(session string, theme Theme, warband, worker, role string) error {
	if err := t.ApplyTheme(session, theme); err != nil {
		return fmt.Errorf("applying theme: %w", err)
	}
	if err := t.SetStatusFormat(session, warband, worker, role); err != nil {
	...
	if err := t.SetMailClickBinding(session); err != nil {
		return fmt.Errorf("setting drums click binding: %w", err)
	}
...
// SetMailClickBinding configures left-click on status-right to show drums preview.
// This creates a popup showing the first unread message when clicking the drums icon area.
func (t *Tmux) SetMailClickBinding(session string) error {
	// Bind left-click on status-right to show drums popup
	// The popup runs hd drums peek and closes on any key
	_, err := t.run("bind-key", "-T", "root", "MouseDown1StatusRight",
		"display-popup", "-E", "-w", "60", "-h", "15", "hd drums peek || echo 'No unread drums'")
	return err
}
...
// The hd cycle command automatically detects the session type and cycles
// within the appropriate group:
// - Encampment sessions: Warchief ↔ Shaman
// - Clan sessions: All clan members in the same warband
//
...
func (t *Tmux) SetCycleBindings(session string) error {
	// C-b n → hd cycle next for HD sessions, next-window otherwise
	// The if-shell checks if session name starts with "hd-" or "hq-"
	if _, err := t.run("bind-key", "-T", "prefix", "n",
		"if-shell", "echo '#{session_name}' | grep -Eq '^(hd|hq)-'",
		"run-shell 'hd cycle next --session #{session_name}'",
		"next-window"); err != nil {
		return err
	}
	// C-b p → hd cycle prev for HD sessions, previous-window otherwise
	if _, err := t.run("bind-key", "-T", "prefix", "p",
		"if-shell", "echo '#{session_name}' | grep -Eq '^(hd|hq)-'",
		"run-shell 'hd cycle prev --session #{session_name}'",
...
// Uses `hd feed --window` which handles both creation and switching.
//
...
func (t *Tmux) SetFeedBinding(session string) error {
	...
	_, err := t.run("bind-key", "-T", "prefix", "a",
		"if-shell", "echo '#{session_name}' | grep -Eq '^(hd|hq)-'",
		"run-shell 'hd feed --window'",
		"display-message 'C-b a is for Horde sessions only'")
	return err
}
...
// The agentID is used to identify the agent in crash logs (e.g., "horde/Toast").
func (t *Tmux) SetPaneDiedHook(session, agentID string) error {
	...
	// We run hd log crash which records to the encampment log
	hookCmd := fmt.Sprintf(`run-shell "hd log crash --agent '%s' --session '%s' --exit-code #{pane_dead_status}"`,
		agentID, session)
