...
// It detects outdated rituals (binary updated), missing rituals (user deleted),
// and modified rituals (user customized). Can auto-fix outdated and missing.
type FormulaCheck struct {
	FixableCheck
}

// NewFormulaCheck creates a new ritual check.
func NewFormulaCheck() *FormulaCheck {
	return &FormulaCheck{
		FixableCheck: FixableCheck{
			BaseCheck: BaseCheck{
				CheckName:        "rituals",
				CheckDescription: "Check embedded rituals are up-to-date",
...
// Run checks if rituals need updating.
func (c *FormulaCheck) Run(ctx *CheckContext) *CheckResult {
	report, err := ritual.CheckFormulaHealth(ctx.TownRoot)
	if err != nil {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusWarning,
			Message: fmt.Sprintf("Could not check rituals: %v", err),
		}
	}
	...
	if report.Outdated == 0 && report.Missing == 0 && report.Modified == 0 && report.New == 0 && report.Untracked == 0 {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusOK,
			Message: fmt.Sprintf("%d rituals up-to-date", report.OK),
		}
	}
	...
	for _, f := range report.Rituals {
		...
		case "new":
			details = append(details, fmt.Sprintf("  %s: new ritual available", f.Name))
			needsFix = true
		case "untracked":
	...
	message := fmt.Sprintf("Rituals: %s", strings.Join(parts, ", "))
	...
	if needsFix {
		result.FixHint = "Run 'hd doctor --fix' to update rituals"
	}

	return result
}

// Fix updates outdated and missing rituals.
func (c *FormulaCheck) Fix(ctx *CheckContext) error {
	updated, skipped, reinstalled, err := ritual.UpdateFormulas(ctx.TownRoot)
	if err != nil {
		return err
	}
