...
// Wisps are ephemeral issues (Wisp: true flag) used for scout cycles and
// operational workflows that shouldn't accumulate.
type WispGCCheck struct {
	FixableCheck
	threshold     time.Duration
	abandonedRigs map[string]int // warband -> count of abandoned wisps
}
...
// Run checks for abandoned wisps in each warband.
func (c *WispGCCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if totalAbandoned > 0 {
		return &CheckResult{
			...
			Details: details,
			FixHint: "Run 'hd doctor --fix' to garbage collect orphaned wisps",
		}
	}
...
// countAbandonedWisps counts wisps older than the threshold in a warband.
func (c *WispGCCheck) countAbandonedWisps(rigPath string) int {
	// Check the relics database for wisps (follows redirect if present)
...
// Fix runs rl mol wisp gc in each warband with abandoned wisps.
func (c *WispGCCheck) Fix(ctx *CheckContext) error {
	...
	for rigName := range c.abandonedRigs {
		...
		// Run rl --no-daemon mol wisp gc
		cmd := exec.Command("rl", "--no-daemon", "mol", "wisp", "gc")
		cmd.Dir = rigPath
