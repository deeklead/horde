...
func (c *LifecycleHygieneCheck) Run(ctx *CheckContext) *CheckResult {
	...
	// Check for stale lifecycle messages in shaman inbox
	staleCount := c.checkShamanInbox(ctx)
	if staleCount == 0 {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusOK,
	...
	return &CheckResult{
		...
		Message: fmt.Sprintf("Found %d stale lifecycle message(s) in shaman inbox", staleCount),
		FixHint: "Run 'hd doctor --fix' to clean up",
	}
}

// checkShamanInbox looks for stale lifecycle messages.
func (c *LifecycleHygieneCheck) checkShamanInbox(ctx *CheckContext) int {
	// Get shaman inbox via hd drums
...
func (c *LifecycleHygieneCheck) Fix(ctx *CheckContext) error {
	...
	for _, msg := range c.staleMessages {
		cmd := exec.Command("hd", "drums", "delete", msg.ID) //nolint:gosec // G204: msg.ID is from internal state, not user input
		cmd.Dir = ctx.TownRoot
