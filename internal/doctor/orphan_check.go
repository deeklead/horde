...
// the expected Horde session naming patterns.
...
// Run checks for orphaned Horde tmux sessions.
func (c *OrphanSessionCheck) Run(ctx *CheckContext) *CheckResult {
	...
	// Get list of valid warbands
	validRigs := c.getValidRigs(ctx.TownRoot)

	// Get session names for warchief/shaman
	...
	for _, sess := range sessions {
		...
		// Only check gt-* sessions (Horde sessions)
		if !strings.HasPrefix(sess, "hd-") {
			continue
		}

		if c.isValidSession(sess, validRigs, warchiefSession, shamanSession) {
			validCount++
		} else {
	...
	if len(orphans) == 0 {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusOK,
			Message: fmt.Sprintf("All %d Horde sessions are valid", validCount),
		}
	}
	...
	return &CheckResult{
		...
		Details: details,
		FixHint: "Run 'hd doctor --fix' to kill orphaned sessions",
	}
}

// Fix kills all orphaned sessions, except clan sessions which are protected.
func (c *OrphanSessionCheck) Fix(ctx *CheckContext) error {
	...
	for _, sess := range c.orphanSessions {
		// SAFEGUARD: Never auto-kill clan sessions.
		// Clan workers are human-managed and require explicit action.
		...
		_ = events.LogFeed(events.TypeSessionDeath, sess,
			events.SessionDeathPayload(sess, "unknown", "orphan cleanup", "hd doctor"))
...
// isCrewSession returns true if the session name matches the clan pattern.
// Clan sessions are gt-<warband>-clan-<name> and are protected from auto-cleanup.
func isCrewSession(session string) bool {
	// Pattern: gt-<warband>-clan-<name>
	// Example: gt-horde-clan-joe
	parts := strings.Split(session, "-")
	if len(parts) >= 4 && parts[0] == "hd" && parts[2] == "clan" {
		return true
	}
	return false
}

// getValidRigs returns a list of valid warband names from the workspace.
func (c *OrphanSessionCheck) getValidRigs(townRoot string) []string {
	var warbands []string

	// Read warbands.json if it exists
	rigsPath := filepath.Join(townRoot, "warchief", "warbands.json")
	if _, err := os.Stat(rigsPath); err == nil {
		// For simplicity, just scan directories at encampment root that look like warbands
		entries, err := os.ReadDir(townRoot)
		if err == nil {
			for _, entry := range entries {
				if entry.IsDir() && entry.Name() != "warchief" && entry.Name() != ".relics" && !strings.HasPrefix(entry.Name(), ".") {
					// Check if it looks like a warband (has raiders/ or clan/ directory)
					raidersDir := filepath.Join(townRoot, entry.Name(), "raiders")
					crewDir := filepath.Join(townRoot, entry.Name(), "clan")
					if _, err := os.Stat(raidersDir); err == nil {
						warbands = append(warbands, entry.Name())
...
// isValidSession checks if a session name matches expected Horde patterns.
// Valid patterns:
//   - gt-{encampment}-warchief (dynamic based on encampment name)
//   - gt-{encampment}-shaman (dynamic based on encampment name)
//   - gt-<warband>-witness
//   - gt-<warband>-forge
//   - gt-<warband>-<raider> (where raider is any name)
//
// Note: We can't verify raider names without reading state, so we're permissive.
func (c *OrphanSessionCheck) isValidSession(sess string, validRigs []string, warchiefSession, shamanSession string) bool {
	// Warchief session is always valid (dynamic name based on encampment)
	if warchiefSession != "" && sess == warchiefSession {
		return true
	}

	// Shaman session is always valid (dynamic name based on encampment)
	if shamanSession != "" && sess == shamanSession {
		return true
	}

	// For warband-specific sessions, extract warband name
	// Pattern: gt-<warband>-<role>
	parts := strings.SplitN(sess, "-", 3)
	if len(parts) < 3 {
		// Invalid format - must be gt-<warband>-<something>
		return false
	}

	rigName := parts[1]

	// Check if this warband exists
	rigFound := false
	...
	if !rigFound {
		// Unknown warband - this is an orphan
		return false
	}

	role := parts[2]

	// witness and forge are valid roles
	if role == "witness" || role == "forge" {
		return true
	}

	// Any other name is assumed to be a raider or clan member
...
// or legitimately orphaned processes from crashed Horde sessions.
// This check is informational only - it does not auto-fix since we cannot
// distinguish user sessions from orphaned Horde processes.
...
func (c *OrphanProcessCheck) Run(ctx *CheckContext) *CheckResult {
	...
	details = append(details, "These may be your personal sessions or orphaned Horde processes.")
