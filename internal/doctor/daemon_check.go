...
func NewDaemonCheck() *DaemonCheck {
	return &DaemonCheck{
		FixableCheck: FixableCheck{
			BaseCheck: BaseCheck{
				CheckName:        "daemon",
				CheckDescription: "Check if Horde daemon is running",
...
func (c *DaemonCheck) Run(ctx *CheckContext) *CheckResult {
	...
	return &CheckResult{
		...
		FixHint: "Run 'hd daemon start' or 'hd doctor --fix'",
	}
}
...
func (c *DaemonCheck) Fix(ctx *CheckContext) error {
	...
	// Start daemon in background (dismiss from parent I/O - daemon uses its own logging)
