...
func TestCheckContext_RigPath(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "empty warband name",
			ctx:      CheckContext{TownRoot: "/encampment"},
			wantPath: "",
		},
		{
			name:     "with warband name",
			ctx:      CheckContext{TownRoot: "/encampment", RigName: "myrig"},
			wantPath: "/encampment/myrig",
		},
	}
