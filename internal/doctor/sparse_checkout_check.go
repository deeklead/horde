...
// and instructions don't override Horde agent configuration.
...
// Run checks if sparse checkout is configured for all git repos in the warband.
func (c *SparseCheckoutCheck) Run(ctx *CheckContext) *CheckResult {
	...
	repoPaths := []string{
		filepath.Join(c.rigPath, "warchief", "warband"),
		filepath.Join(c.rigPath, "forge", "warband"),
	}

	// Add clan clones
	crewDir := filepath.Join(c.rigPath, "clan")
	...
	// Add raider worktrees
	raiderDir := filepath.Join(c.rigPath, "raiders")
	if entries, err := os.ReadDir(raiderDir); err == nil {
		for _, entry := range entries {
			if entry.IsDir() {
				repoPaths = append(repoPaths, filepath.Join(raiderDir, entry.Name()))
			}
		}
	}
	...
	return &CheckResult{
		...
		Details: details,
		FixHint: "Run 'hd doctor --fix' to configure sparse checkout",
	}
}
