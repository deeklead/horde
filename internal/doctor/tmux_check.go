...
// Run checks for linked panes across Horde tmux sessions.
func (c *LinkedPaneCheck) Run(ctx *CheckContext) *CheckResult {
	...
	// Cache for Fix (exclude warchief session since we don't want to kill it)
	...
	if len(conflicts) == 0 {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusOK,
			Message: fmt.Sprintf("All %d Horde sessions have independent panes", len(gtSessions)),
		}
	}
	...
	return &CheckResult{
		...
		Details: conflicts,
		FixHint: "Run 'hd doctor --fix' to kill linked sessions (daemon will recreate)",
	}
}

// Fix kills sessions with linked panes (except warchief session).
