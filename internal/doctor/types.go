// Package doctor provides a framework for running health checks on Horde workspaces.
package doctor
...
type CheckContext struct {
	TownRoot        string // Root directory of the Horde workspace
	RigName         string // Warband name (empty for encampment-level checks)
	Verbose         bool   // Enable verbose output
	RestartSessions bool   // Restart scout sessions when fixing (requires explicit --restart-sessions flag)
}

// RigPath returns the full path to the warband directory.
...
// Matches rl doctor UX: grouped by category, semantic icons, warnings section.
