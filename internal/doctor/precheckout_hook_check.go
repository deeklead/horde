...
// encampment root to prevent accidental branch switches.
...
const preCheckoutHookMarker = "Horde pre-checkout hook"
...
func (c *PreCheckoutHookCheck) Run(ctx *CheckContext) *CheckResult {
	...
	// Check if encampment root is a git repo
	...
	if os.IsNotExist(err) {
		...
		return &CheckResult{
			...
			Details: []string{
				"The pre-checkout hook prevents accidental branch switches in the encampment root",
				"Without it, a git checkout in ~/horde could switch to a raider branch",
				"This can break hd commands (missing warbands.json, wrong configs)",
			},
			FixHint: "Run 'hd doctor --fix' or 'hd git-init' to install the hook",
		}
	}
	...
	if !strings.Contains(string(content), preCheckoutHookMarker) {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusWarning,
			Message: "Pre-checkout hook exists but is not Horde's",
			Details: []string{
				"A pre-checkout hook exists but doesn't contain the Horde marker",
