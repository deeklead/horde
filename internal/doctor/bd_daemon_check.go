...
// BdDaemonCheck verifies that the rl (relics) daemon is running and healthy.
...
// NewBdDaemonCheck creates a new rl daemon check.
func NewBdDaemonCheck() *BdDaemonCheck {
	return &BdDaemonCheck{
		FixableCheck: FixableCheck{
			BaseCheck: BaseCheck{
				CheckName:        "bd-daemon",
				CheckDescription: "Check if rl (relics) daemon is running",
...
// Run checks if the rl daemon is running and healthy.
func (c *BdDaemonCheck) Run(ctx *CheckContext) *CheckResult {
	// Check daemon status
	cmd := exec.Command("rl", "daemon", "--status")
	...
	if err == nil && strings.Contains(output, "Daemon is running") {
		...
		healthCmd := exec.Command("rl", "daemon", "--health")
...
// tryStartDaemon attempts to start the rl daemon and returns any error output.
...
func (c *BdDaemonCheck) parseStartError(err *startError) *CheckResult {
	...
	if strings.Contains(output, "LEGACY DATABASE DETECTED") {
		return &CheckResult{
			...
			Details: []string{
				"Database was created before rl version 0.17.5",
	...
	if strings.Contains(output, "DATABASE MISMATCH DETECTED") {
		return &CheckResult{
			...
			Details: []string{
				"The .relics database was created for a different git repository",
				"This can happen if .relics was copied or if the git remote URL changed",
			},
			FixHint: "Run 'bd migrate --update-repo-id' if URL changed, or 'rm -rf .relics && rl init' for fresh start",
		}
	}
	...
	if strings.Contains(output, "lock") || strings.Contains(output, "permission") {
		return &CheckResult{
			...
			FixHint: "Check if another rl daemon is running, or remove .relics/daemon.lock",
		}
	}
	...
	if strings.Contains(output, "corrupt") || strings.Contains(output, "malformed") {
		return &CheckResult{
			...
			Details: []string{output},
			FixHint: "Run 'bd repair' or 'rm .relics/issues.db && rl sync --from-main'",
		}
	}
	...
	return &CheckResult{
		...
		Details: details,
		FixHint: "Check 'bd daemon --status' and logs in .relics/daemon.log",
	}
}

// Fix attempts to start the rl daemon.
func (c *BdDaemonCheck) Fix(ctx *CheckContext) error {
	...
	if strings.Contains(startErr.output, "LEGACY DATABASE") ||
		...
		migrateCmd := exec.Command("rl", "migrate", "--update-repo-id", "--yes")
		...
		// Try starting again
		startCmd := exec.Command("rl", "daemon", "--start")
