...
type RoleData struct {
	Role           string   // warchief, witness, forge, raider, clan, shaman
	RigName        string   // e.g., "greenplace"
	TownRoot       string   // e.g., "/Users/steve/ai"
	TownName       string   // e.g., "ai" - the encampment identifier for session names
	WorkDir        string   // current working directory
	DefaultBranch  string   // default branch for merges (e.g., "main", "develop")
	Raider        string   // raider name (for raider role)
...
// NudgeData contains information for signal messages.
...
func (t *Templates) RoleNames() []string {
	return []string{"warchief", "witness", "forge", "raider", "clan", "shaman"}
}

// MessageNames returns the list of available message templates.
func (t *Templates) MessageNames() []string {
	return []string{"muster", "signal", "escalation", "handoff"}
}

// CreateWarchiefCLAUDEmd creates the Warchief's CLAUDE.md file at the specified directory.
// This is used by both hd install and hd doctor --fix.
func CreateWarchiefCLAUDEmd(warchiefDir, townRoot, townName, warchiefSession, shamanSession string) error {
	tmpl, err := New()
	if err != nil {
		return err
	}
	...
	data := RoleData{
		...
		TownName:      townName,
		WorkDir:       warchiefDir,
		WarchiefSession:  warchiefSession,
		ShamanSession: shamanSession,
	}

	content, err := tmpl.RenderRole("warchief", data)
	if err != nil {
		return err
	}
...
// This ensures clan/raider workspaces have the handoff command and other utilities
