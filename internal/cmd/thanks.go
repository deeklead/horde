...
// hordeContributors maps HUMAN contributor names to their commit counts.
...
var thanksCmd = &cobra.Command{
	Use:     "thanks",
	Short:   "Thank the human contributors to Horde",
	GroupID: GroupDiag,
	Long: `Display acknowledgments to all the humans who have contributed
to the Horde project. This command celebrates the collaborative
...
func getContributorsSorted() []string {
	names := make([]string, 0, len(hordeContributors))
	for name := range hordeContributors {
		names = append(names, name)
	}

	slices.SortFunc(names, func(a, b string) int {
		// sort by commit count descending, then by name ascending for ties
		countCmp := cmp.Compare(hordeContributors[b], hordeContributors[a])
		if countCmp != 0 {
			return countCmp
		}
...
func printThanksPage() {
	...
	subtitle := thanksSubtitleStyle.Render("To all the humans who contributed to Horde")
	headerContent := title + "\n\n" + subtitle
...
func printThanksColumns(names []string, cols int) {
	...
	// print in rows, reading left to right (matches rl thanks)
