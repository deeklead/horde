...
func TestClaimPatternMatching(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "exact match",
			pattern: "horde/raiders/capable",
			caller:  "horde/raiders/capable",
			want:    true,
		},
		{
			name:    "exact match with different name",
			pattern: "horde/raiders/toast",
			caller:  "horde/raiders/capable",
			want:    false,
		},
		...
		// Wildcard at end
		{
			name:    "wildcard matches raider",
			pattern: "horde/raiders/*",
			caller:  "horde/raiders/capable",
			want:    true,
		},
		{
			name:    "wildcard matches different raider",
			pattern: "horde/raiders/*",
			caller:  "horde/raiders/toast",
			want:    true,
		},
		{
			name:    "wildcard doesn't match wrong warband",
			pattern: "horde/raiders/*",
			caller:  "relics/raiders/capable",
			want:    false,
		},
		{
			name:    "wildcard doesn't match nested path",
			pattern: "horde/raiders/*",
			caller:  "horde/raiders/sub/capable",
			want:    false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := relics.MatchClaimPattern(tt.pattern, tt.caller)
			if got != tt.want {
	...
	tests := []struct {
	...
	}{
		{
			...
			msgInfo: &queueMessageInfo{
				ID:        "hq-test1",
				Title:     "Test Message",
				ClaimedBy: "horde/raiders/nux",
				QueueName: "work-requests",
				Status:    "open",
			},
			caller:  "horde/raiders/nux",
			wantErr: false,
		},
		{
...
// TestAnnounceMessageParsing tests parsing of announce messages from relics output.
func TestAnnounceMessageParsing(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			labels: []string{"from:warchief/", "announce_channel:alerts"},
			want:   "warchief/",
		},
		{
			name:   "extracts from with warband path",
			labels: []string{"announce_channel:alerts", "from:horde/witness"},
			want:   "horde/witness",
		},
		{
			name:   "no from label",
