...
var agentStateCmd = &cobra.Command{
	...
	Short: "Get or set operational state on agent relics",
	Long: `Get or set label-based operational state on agent relics.

Agent relics store operational state (like idle cycle counts) as labels.
...
OPERATIONS:
  Get all labels (default):
    hd agent state <agent-bead>

  Set a label:
    hd agent state <agent-bead> --set idle=0
    hd agent state <agent-bead> --set idle=0 --set backoff=30s

  Increment a numeric label:
    hd agent state <agent-bead> --incr idle
    (Creates label with value 1 if not present)

  Delete a label:
    hd agent state <agent-bead> --del idle

COMMON LABELS:
  idle:<n>           - Consecutive idle scout cycles
  backoff:<duration> - Current backoff interval
  last_activity:<ts> - Last activity timestamp

EXAMPLES:
  # Check current idle count
  hd agent state gt-horde-witness

  # Reset idle counter after finding work
  hd agent state gt-horde-witness --set idle=0

  # Increment idle counter on timeout
  hd agent state gt-horde-witness --incr idle

  # Get state as JSON
  hd agent state gt-horde-witness --json`,
...
func runAgentState(cmd *cobra.Command, args []string) error {
	...
	relicsDir := relics.ResolveRelicsDir(cwd)
	if relicsDir == "" {
		return fmt.Errorf("not in a relics workspace")
	}
	...
	if hasSet || hasIncr || hasDel {
		// Modification mode
		return modifyAgentState(agentBead, relicsDir, hasIncr)
	}

	// Query mode
	return queryAgentState(agentBead, relicsDir)
}

// queryAgentState retrieves and displays labels from an agent bead.
func queryAgentState(agentBead, relicsDir string) error {
	labels, err := getAgentLabels(agentBead, relicsDir)
	if err != nil {
		return err
	}
...
func modifyAgentState(agentBead, relicsDir string, hasIncr bool) error {
	// Read current labels
	labels, err := getAgentLabels(agentBead, relicsDir)
	if err != nil {
		return err
	...
	allLabels, err := getAllAgentLabels(agentBead, relicsDir)
	if err != nil {
		return err
	}
...
func getAgentLabels(agentBead, relicsDir string) (map[string]string, error) {
	allLabels, err := getAllAgentLabels(agentBead, relicsDir)
	if err != nil {
		return nil, err
	}
...
func getAllAgentLabels(agentBead, relicsDir string) ([]string, error) {
