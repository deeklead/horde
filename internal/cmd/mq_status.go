...
// MRStatusOutput is the JSON output structure for hd mq status.
...
func runMqStatus(cmd *cobra.Command, args []string) error {
	...
	// Use current working directory for relics operations
	// (relics repos are per-warband, not per-workspace)
	workDir, err := os.Getwd()
	if err != nil {
		return fmt.Errorf("getting current directory: %w", err)
	}

	// Initialize relics client
	bd := relics.New(workDir)
	...
	mrFields := relics.ParseMRFields(issue)
	...
	if mrFields != nil {
		...
		output.Warband = mrFields.Warband
...
func printMqStatus(issue *relics.Issue, mrFields *relics.MRFields) error {
	// Header
