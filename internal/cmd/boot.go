...
var bootCmd = &cobra.Command{
	Use:     "boot",
	GroupID: GroupAgents,
	Short:   "Manage Boot (Shaman watchdog)",
	Long: `Manage Boot - the daemon's watchdog for Shaman triage.

Boot is a special dog that runs fresh on each daemon tick. It observes
the system state and decides whether to start/wake/signal/interrupt the
Shaman, or do nothing. This centralizes the "when to wake" decision in
...
Location: ~/horde/shaman/dogs/boot/
Session: gt-boot`,
}
...
Displays:
  ...
  - Last action taken (start/wake/signal/nothing)
  - Timing information
...
var bootSpawnCmd = &cobra.Command{
	Use:   "muster",
	Short: "Muster Boot for triage",
	Long: `Muster Boot to run the triage cycle.
...
what action to take on the Shaman.
...
It performs basic observation and takes conservative action:
  - If Shaman is not running: start it
  - If Shaman appears stuck: attempt restart
...
func init() {
	...
	bootSpawnCmd.Flags().StringVar(&bootAgentOverride, "agent", "", "Agent alias to run Boot with (overrides encampment default)")
...
func runBootSpawn(cmd *cobra.Command, args []string) error {
	...
	if b.IsRunning() {
		fmt.Println("Boot is already running - skipping muster")
		return nil
	}
	...
	// Muster Boot
	if err := b.Muster(bootAgentOverride); err != nil {
...
// runDegradedTriage performs basic Shaman health check without AI reasoning.
...
func runDegradedTriage(b *boot.Boot) (action, target string, err error) {
	...
	hasShaman, err := tm.HasSession(shamanSession)
	if err != nil {
		return "error", "shaman", fmt.Errorf("checking shaman session: %w", err)
	}

	if !hasShaman {
		// Shaman not running - this is unusual, daemon should have restarted it
		// In degraded mode, we just report - let daemon handle restart
		return "report", "shaman-missing", nil
	}

	// Shaman exists - check heartbeat to detect stuck sessions
	// A session can exist but be stuck (not making progress)
	townRoot, _ := workspace.FindFromCwd()
	if townRoot != "" {
		hb := shaman.ReadHeartbeat(townRoot)
		if hb.ShouldPoke() {
			// Heartbeat is stale (>15 min) - Shaman is stuck
			// Signal the session to try to wake it up
			age := hb.Age()
			if age > 30*time.Minute {
				// Very stuck - restart the session
				fmt.Printf("Shaman heartbeat is %s old - restarting session\n", age.Round(time.Minute))
				if err := tm.KillSession(shamanSession); err == nil {
					return "restart", "shaman-stuck", nil
				}
			} else {
				// Stuck but not critically - try nudging first
				fmt.Printf("Shaman heartbeat is %s old - nudging session\n", age.Round(time.Minute))
				_ = tm.SignalSession(shamanSession, "HEALTH_CHECK: heartbeat is stale, respond to confirm responsiveness")
				return "signal", "shaman-stale", nil
			}
		}
	}
