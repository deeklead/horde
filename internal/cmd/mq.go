...
var mqCmd = &cobra.Command{
	...
	Long: `Manage merge requests and the merge queue for a warband.

Alias: 'hd mr' is equivalent to 'hd mq' (merge request vs merge queue).

The merge queue tracks work branches from raiders waiting to be merged.
...
Creates a merge-request bead that will be processed by the Forge.

Auto-detection:
  - Branch: current git branch
  - Issue: parsed from branch name (e.g., raider/Nux/gp-xyz → gt-xyz)
  - Worker: parsed from branch name
  - Warband: detected from current directory
...
Raider auto-cleanup:
  When run from a raider work branch (raider/<worker>/<issue>), this command
  automatically triggers raider shutdown after submitting the MR. The raider
...
Examples:
  hd mq submit                           # Auto-detect everything + auto-cleanup
  hd mq submit --issue gp-abc            # Explicit issue
  hd mq submit --epic gt-xyz             # Target integration branch explicitly
  hd mq submit --priority 0              # Override priority (P0)
  hd mq submit --no-cleanup              # Submit without auto-cleanup`,
	RunE: runMqSubmit,
}

var mqRetryCmd = &cobra.Command{
	Use:   "retry <warband> <mr-id>",
	Short: "Retry a failed merge request",
	Long: `Retry a failed merge request.

Resets a failed MR so it can be processed again by the forge.
The MR must be in a failed state (open with an error).

Examples:
  hd mq retry greenplace gp-mr-abc123
  hd mq retry greenplace gp-mr-abc123 --now`,
...
Output format:
  ...
  gt-mr-001   ready        P0        raider/Nux/gp-xyz        Nux     5m
  gt-mr-002   in_progress  P1        raider/Toast/gt-abc      Toast   12m
  gt-mr-003   blocked      P1        raider/Capable/gt-def    Capable 8m
              (waiting on gt-mr-001)

Examples:
  hd mq list greenplace
  hd mq list greenplace --ready
  hd mq list greenplace --status=open
  hd mq list greenplace --worker=Nux`,
	Args: cobra.ExactArgs(1),
	RunE: runMQList,
}

var mqRejectCmd = &cobra.Command{
	Use:   "reject <warband> <mr-id-or-branch>",
...
Examples:
  hd mq reject greenplace raider/Nux/gp-xyz --reason "Does not meet requirements"
  hd mq reject greenplace mr-Nux-12345 --reason "Superseded by other work" --notify`,
	Args: cobra.ExactArgs(2),
	RunE: runMQReject,
}
...
Branch naming:
  ...
  Config:  Set merge_queue.integration_branch_template in warband settings
...
Examples:
  hd mq integration create gt-auth-epic
  # Creates integration/gt-auth-epic (default)

  hd mq integration create RA-123 --branch "klauern/PROJ-1234/{epic}"
...
Examples:
  hd mq integration land gt-auth-epic
  hd mq integration land gt-auth-epic --dry-run
  hd mq integration land gt-auth-epic --force --skip-tests`,
	Args: cobra.ExactArgs(1),
...
Example:
  hd mq integration status gt-auth-epic`,
	Args: cobra.ExactArgs(1),
...
func init() {
	...
	mqSubmitCmd.Flags().BoolVar(&mqSubmitNoCleanup, "no-cleanup", false, "Don't auto-cleanup after submit (for raiders)")

	// Retry flags
	mqRetryCmd.Flags().BoolVar(&mqRetryNow, "now", false, "Immediately process instead of waiting for forge loop")
	...
	mqRejectCmd.Flags().BoolVar(&mqRejectNotify, "notify", false, "Send drums notification to worker")
...
// findCurrentRig determines the current warband from the working directory.
// Returns the warband name and warband object, or an error if not in a warband.
func findCurrentRig(townRoot string) (string, *warband.Warband, error) {
	cwd, err := os.Getwd()
	if err != nil {
		return "", nil, fmt.Errorf("getting current directory: %w", err)
	}

	// Get relative path from encampment root to cwd
	relPath, err := filepath.Rel(townRoot, cwd)
	if err != nil {
		return "", nil, fmt.Errorf("computing relative path: %w", err)
	}

	// The first component of the relative path should be the warband name
	parts := strings.Split(relPath, string(filepath.Separator))
	if len(parts) == 0 || parts[0] == "" || parts[0] == "." {
		return "", nil, fmt.Errorf("not inside a warband directory")
	}

	rigName := parts[0]

	// Load warband manager and get the warband
	...
	if err != nil {
		return "", nil, fmt.Errorf("warband '%s' not found: %w", rigName, err)
	}
...
func runMQRetry(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		if err == forge.ErrMRNotFound {
			return fmt.Errorf("merge request '%s' not found in warband '%s'", mrID, rigName)
		}
	...
	if mqRetryNow {
		...
		fmt.Printf("  %s\n", style.Dim.Render("Will be processed on next forge cycle"))
	}
...
func runMQReject(cmd *cobra.Command, args []string) error {
	...
	if mqRejectNotify {
		fmt.Printf("  %s\n", style.Dim.Render("Worker notified via drums"))
	}
