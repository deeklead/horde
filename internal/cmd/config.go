...
var configCmd = &cobra.Command{
	Use:     "config",
	GroupID: GroupConfig,
	Short:   "Manage Horde configuration",
	RunE:    requireSubcommand,
	Long: `Manage Horde configuration settings.

This command allows you to view and modify configuration settings
for your Horde workspace, including agent aliases and defaults.

Commands:
  hd config agent list              List all agents (built-in and custom)
  hd config agent get <name>         Show agent configuration
  hd config agent set <name> <cmd>   Set custom agent command
  hd config agent remove <name>      Remove custom agent
  hd config default-agent [name]     Get or set default agent`,
}
...
custom agents defined in your encampment settings.

Examples:
  hd config agent list           # Text output
  hd config agent list --json    # JSON output`,
...
Examples:
  hd config agent get claude
  hd config agent get my-custom-agent`,
...
var configAgentSetCmd = &cobra.Command{
	...
	Long: `Set a custom agent command in encampment settings.
...
to all warbands in the encampment.
...
Examples:
  ...
  hd config agent set gemini-custom gemini --approval-mode yolo
  hd config agent set claude \"claude-glm\"  # Override built-in claude`,
...
var configAgentRemoveCmd = &cobra.Command{
	...
	Long: `Remove a custom agent definition from encampment settings.

This removes a custom agent from your encampment settings. Built-in agents
(claude, gemini, codex) cannot be removed.

Examples:
  hd config agent remove claude-glm`,
	Args: cobra.ExactArgs(1),
...
var configDefaultAgentCmd = &cobra.Command{
	...
	Long: `Get or set the default agent for the encampment.

With no arguments, shows the current default agent.
With an argument, sets the default agent to the specified name.

The default agent is used when a warband doesn't specify its own agent
setting. Can be a built-in preset (claude, gemini, codex) or a
custom agent name.

Examples:
  hd config default-agent           # Show current default
  hd config default-agent claude    # Set to claude
  hd config default-agent gemini    # Set to gemini
  hd config default-agent my-custom # Set to custom agent`,
...
When agents commit code via 'hd commit', their identity is converted
to a git email address. For example, "horde/clan/jack" becomes
"horde.clan.jack@{domain}".
...
Examples:
  hd config agent-email-domain                 # Show current domain
  hd config agent-email-domain horde.local   # Set to horde.local
  hd config agent-email-domain example.com     # Set custom domain`,
...
func runConfigAgentList(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		return fmt.Errorf("loading encampment settings: %w", err)
	}
...
func runConfigAgentGet(cmd *cobra.Command, args []string) error {
	...
	// Load encampment settings for custom agents
	settingsPath := config.TownSettingsPath(townRoot)
	townSettings, err := config.LoadOrCreateTownSettings(settingsPath)
	if err != nil {
		return fmt.Errorf("loading encampment settings: %w", err)
...
func runConfigAgentSet(cmd *cobra.Command, args []string) error {
	...
	if err := config.SaveTownSettings(settingsPath, townSettings); err != nil {
		return fmt.Errorf("saving encampment settings: %w", err)
	}
...
func runConfigAgentRemove(cmd *cobra.Command, args []string) error {
	...
	for _, builtin := range builtInAgents {
		if name == builtin {
			return fmt.Errorf("cannot remove built-in agent '%s' (use 'hd config agent set' to override it)", name)
		}
	}
...
func runConfigDefaultAgent(cmd *cobra.Command, args []string) error {
	...
	if !isValid {
		return fmt.Errorf("agent '%s' not found (use 'hd config agent list' to see available agents)", name)
	}
...
func runConfigAgentEmailDomain(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 0 {
		...
		fmt.Printf("\nExample: horde/clan/jack → horde.clan.jack@%s\n", domain)
		return nil
	}
