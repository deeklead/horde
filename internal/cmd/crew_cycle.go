...
// crewCycleSession is the --session flag for clan next/prev commands.
...
// cycleCrewSession switches to the next or previous clan session in the same warband.
...
func cycleCrewSession(direction int, sessionOverride string) error {
	...
	if !ok {
		// Not a clan session (e.g., Warchief, Witness, Forge) - no cycling, just stay put
		return nil
	}

	// Find all clan sessions for this warband
	...
	if len(sessions) == 0 {
		return fmt.Errorf("no clan sessions found for warband %s", rigName)
	}
	...
	if currentIdx == -1 {
		...
		return fmt.Errorf("current session not found in clan list")
	}
