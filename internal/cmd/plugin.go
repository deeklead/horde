...
var pluginCmd = &cobra.Command{
	...
	Long: `Manage plugins that run during Shaman scout cycles.

Plugins are periodic automation tasks defined by plugin.md files with TOML frontmatter.

PLUGIN LOCATIONS:
  ~/horde/plugins/           Encampment-level plugins (universal, apply everywhere)
  <warband>/plugins/          Warband-level plugins (project-specific)
...
Examples:
  hd plugin list                    # List all discovered plugins
  hd plugin show <name>             # Show plugin details
  hd plugin list --json             # JSON output`,
...
var pluginListCmd = &cobra.Command{
	...
	Long: `List all plugins from encampment and warband plugin directories.

Plugins are discovered from:
  - ~/horde/plugins/ (encampment-level)
  - <warband>/plugins/ for each registered warband

When a plugin exists at both levels, the warband-level version takes precedence.

Examples:
  hd plugin list              # Human-readable output
  hd plugin list --json       # JSON output for scripting`,
	RunE: runPluginList,
}
...
Examples:
  hd plugin show rebuild-gt
  hd plugin show rebuild-gt --json`,
...
Examples:
  hd plugin run rebuild-gt              # Run if gate allows
  hd plugin run rebuild-gt --force      # Bypass gate check
  hd plugin run rebuild-gt --dry-run    # Show what would happen`,
	Args: cobra.ExactArgs(1),
	RunE: runPluginRun,
}
...
Queries ephemeral relics (wisps) that record plugin runs.

Examples:
  hd plugin history rebuild-gt
  hd plugin history rebuild-gt --json
  hd plugin history rebuild-gt --limit 20`,
	Args: cobra.ExactArgs(1),
...
// getPluginScanner creates a scanner with encampment root and all warband names.
func getPluginScanner() (*plugin.Scanner, string, error) {
	...
	// Load warbands config to get warband names
	...
	// Extract warband names
	rigNames := make([]string, 0, len(rigsConfig.Warbands))
...
func outputPluginListText(plugins []*plugin.Plugin, townRoot string) error {
	...
	// Print encampment-level plugins
	if len(townPlugins) > 0 {
		fmt.Printf("  %s\n", style.Bold.Render("Encampment-level plugins:"))
	...
	// Print warband-level plugins by warband
	...
	for _, rigName := range rigNames {
		fmt.Printf("  %s\n", style.Bold.Render(fmt.Sprintf("Warband %s:", rigName)))
...
func runPluginRun(cmd *cobra.Command, args []string) error {
	...
	beadID, err := recorder.RecordRun(plugin.PluginRunRecord{
		...
		Body:       "Manual run via hd plugin run",
	})
	if err != nil {
