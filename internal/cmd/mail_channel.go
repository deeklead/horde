...
var mailChannelCmd = &cobra.Command{
	Use:   "channel [name]",
	Short: "Manage and view relics-native channels",
	Long: `View and manage relics-native broadcast channels.
...
Examples:
  hd drums channel              # List all channels
  hd drums channel alerts       # View messages from 'alerts' channel
  hd drums channel list         # Alias for listing channels
  hd drums channel show alerts  # Same as: hd drums channel alerts
  hd drums channel create alerts --retain-count=100
  hd drums channel delete alerts`,
...
func runChannelList(cmd *cobra.Command, args []string) error {
	...
	if len(channels) == 0 {
		...
		fmt.Println("\nCreate one with: hd drums channel create <name>")
		return nil
	}
...
// listChannelMessages lists messages from a relics-native channel.
