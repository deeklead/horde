...
The priority scoring function considers:
  - Raid age: Older raids get higher priority (starvation prevention)
  - Issue priority: P0 > P1 > P2 > P3 > P4
  - Retry count: MRs that fail repeatedly get deprioritized
  - MR age: FIFO tiebreaker for same priority/raid

Use --strategy=fifo for first-in-first-out ordering instead.

Examples:
  hd mq next horde                    # Show highest-priority MR
  hd mq next horde --strategy=fifo    # Show oldest MR instead
  hd mq next horde --quiet            # Just print the MR ID
  hd mq next horde --json             # Output as JSON`,
...
func runMQNext(cmd *cobra.Command, args []string) error {
	...
	// Create relics wrapper for the warband
	...
	// Get the top MR
	next := ready[0]
	fields := relics.ParseMRFields(next)
