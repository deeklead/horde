...
var auditCmd = &cobra.Command{
	...
	Long: `Query provenance data across git commits, relics, and events.

Shows a unified timeline of work performed by an actor including:
  - Git commits authored by the actor
  - Relics (issues) created by the actor
  - Relics closed by the actor (via assignee)
  - Encampment log events (muster, done, handoff, etc.)
  - Activity feed events

Examples:
  hd audit --actor=greenplace/clan/joe       # Show all work by joe
  hd audit --actor=greenplace/raiders/toast # Show raider toast's work
  hd audit --actor=warchief                  # Show warchief's activity
  hd audit --since=24h                    # Show all activity in last 24h
  hd audit --actor=joe --since=1h         # Combined filters
  hd audit --json                         # Output as JSON`,
	RunE: runAudit,
}
...
type AuditEntry struct {
	...
	Source    string    `json:"source"` // "git", "relics", "encampmentlog", "events"
	Type      string    `json:"type"`   // "commit", "bead_created", "bead_closed", "muster", etc.
...
func runAudit(cmd *cobra.Command, args []string) error {
	...
	// 2. Relics (created_by, assignee)
	relicsEntries, err := collectRelicsActivity(townRoot, auditActor, sinceTime)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Warning: could not query relics: %v\n", err)
	}
	allEntries = append(allEntries, relicsEntries...)

	// 3. Encampment log events
	encampmentlogEntries, err := collectTownlogEvents(townRoot, auditActor, sinceTime)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Warning: could not query encampment log: %v\n", err)
	}
	allEntries = append(allEntries, encampmentlogEntries...)
...
func collectGitCommits(townRoot, actor string, since time.Time) ([]AuditEntry, error) { //nolint:unparam // error return kept for future use
	...
	if actor != "" {
		...
		// Actor format might be "greenplace/clan/joe" - extract "joe" as the author name
...
func extractAuthorName(actor string) string {
	// Actor format: "greenplace/clan/joe" -> "joe"
	// Or: "warchief" -> "warchief"
...
// collectRelicsActivity queries relics for issues created or closed by the actor.
func collectRelicsActivity(townRoot, actor string, since time.Time) ([]AuditEntry, error) {
	var entries []AuditEntry

	// Find the horde relics path (where gt- prefix issues live)
	hordeRelicsPath := filepath.Join(townRoot, "horde", "warchief", "warband")
	b := relics.New(hordeRelicsPath)
	...
	for _, issue := range issues {
		...
		if issue.CreatedBy != "" {
			if actor == "" || matchesActor(issue.CreatedBy, actor) {
				ts := parseRelicsTimestamp(issue.CreatedAt)
		...
		if issue.Status == "closed" && issue.Assignee != "" {
			if actor == "" || matchesActor(issue.Assignee, actor) {
				ts := parseRelicsTimestamp(issue.ClosedAt)
				if ts.IsZero() {
					ts = parseRelicsTimestamp(issue.UpdatedAt)
				}
				if !since.IsZero() && ts.Before(since) {
					continue
				}
...
// parseRelicsTimestamp parses a relics timestamp string.
func parseRelicsTimestamp(s string) time.Time {
	// Try various formats
	formats := []string{
		time.RFC3339,
		"2006-01-02 15:04",
...
// collectTownlogEvents queries the encampment log for agent lifecycle events.
func collectTownlogEvents(townRoot, actor string, since time.Time) ([]AuditEntry, error) {
	var entries []AuditEntry

	allEvents, err := encampmentlog.ReadEvents(townRoot)
	if err != nil {
		return nil, err
	}
...
// formatTownlogSummary creates a readable summary from a encampment log event.
func formatTownlogSummary(e encampmentlog.Event) string {
	switch e.Type {
...
func formatFeedSummary(e events.Event) string {
	...
	case events.TypeSling:
		if bead, ok := e.Payload["bead"].(string); ok {
			return fmt.Sprintf("Charged %s", bead)
		}
		return "Charged work"
	...
	case events.TypeMail:
		if to, ok := e.Payload["to"].(string); ok {
			return fmt.Sprintf("Sent drums to %s", to)
		}
		return "Sent drums"
	default:
		return e.Type
...
func formatSource(source string) string {
	...
	case "relics":
		return style.Success.Render("[relics]")
	case "encampmentlog":
...
func formatType(t string) string {
	...
	case "muster":
		return style.Success.Render("muster")
	case "done":
