...
func init() {
	...
	feedCmd.Flags().StringVar(&feedMol, "mol", "", "Filter by totem/issue ID prefix")
	feedCmd.Flags().StringVar(&feedType, "type", "", "Filter by event type (create, update, delete, comment)")
	feedCmd.Flags().StringVar(&feedRig, "warband", "", "Run from specific warband's relics directory")
...
var feedCmd = &cobra.Command{
	Use:     "feed",
	GroupID: GroupDiag,
	Short:   "Show real-time activity feed from relics and hd events",
	Long: `Display a real-time feed of issue changes and agent activity.

By default, launches an interactive TUI warmap with:
  - Agent tree (top): Shows all agents organized by role with latest activity
  - Raid panel (middle): Shows in-progress and recently landed raids
...
The feed combines multiple event sources:
  - Relics activity: Issue creates, updates, completions (from rl activity)
  - GT events: Agent activity like scout, charge, handoff (from .events.jsonl)
  - Raid status: In-progress and recently-landed raids (refreshes every 10s)

Use --plain for simple text output (wraps rl activity only).
...
Event symbols:
  +  created/bonded    - New issue or totem created
  ...
  🦉  patrol_started   - Witness began scout cycle
  ⚡  raider_nudged   - Worker was nudged
  🎯  charge            - Work was charged to worker
  🤝  handoff          - Session handed off

MQ (Merge Queue) event symbols:
  ⚙  merge_started   - Forge began processing an MR
...
Examples:
  hd feed                       # Launch TUI warmap
  hd feed --plain               # Plain text output (bd activity)
  hd feed --window              # Open in dedicated tmux window
  hd feed --since 1h            # Events from last hour
  hd feed --warband greenplace         # Use horde warband's relics`,
	RunE: runFeed,
}

func runFeed(cmd *cobra.Command, args []string) error {
	// Must be in a Horde workspace
	townRoot, err := workspace.FindFromCwdOrError()
	if err != nil {
		return fmt.Errorf("not in a Horde workspace (run from ~/horde or a warband directory)")
	}
	...
	// If --warband specified, find that warband's relics directory
	if feedRig != "" {
		// Try common relics locations for the warband
		candidates := []string{
			fmt.Sprintf("%s/%s/warchief/warband", townRoot, feedRig),
			fmt.Sprintf("%s/%s", townRoot, feedRig),
		}

		found := false
		for _, candidate := range candidates {
			if _, err := os.Stat(candidate + "/.relics"); err == nil {
				workDir = candidate
				found = true
				break
			}
		...
		if !found {
			return fmt.Errorf("warband '%s' not found or has no .relics directory", feedRig)
		}
	}

	// Build rl activity command (without argv[0] for buildFeedCommand)
	...
	// Plain mode: exec rl activity directly
	return runFeedDirect(workDir, bdArgs)
}

// buildFeedArgs builds the rl activity arguments based on flags.
...
// runFeedDirect runs rl activity in the current terminal.
func runFeedDirect(workDir string, bdArgs []string) error {
	...
	fullArgs := append([]string{"rl", "activity"}, bdArgs...)
...
func runFeedTUI(workDir string) error {
	...
	// Create event source from rl activity
	bdSource, err := feed.NewBdActivitySource(workDir)
	if err != nil {
		return fmt.Errorf("creating rl activity source: %w", err)
	}
...
func runFeedInWindow(workDir string, bdArgs []string) error {
	...
	feedCmd := fmt.Sprintf("cd %s && rl activity --follow", workDir)
	if len(bdArgs) > 0 {
		...
		if len(filteredArgs) > 0 {
			feedCmd = fmt.Sprintf("cd %s && rl activity --follow %s", workDir, strings.Join(filteredArgs, " "))
		}
	}
	...
	// Create new window named 'feed' with the rl activity command
