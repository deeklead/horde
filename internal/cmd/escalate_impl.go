...
func runEscalate(cmd *cobra.Command, args []string) error {
	...
	if escalateDryRun {
		...
		fmt.Printf("  Drums targets: %s\n", strings.Join(targets, ", "))
		return nil
	}

	// Create escalation bead
	bd := relics.New(relics.ResolveRelicsDir(townRoot))
	fields := &relics.EscalationFields{
		Severity:    severity,
	...
	// Send drums to each target (actions with "drums:" prefix)
	...
	for _, target := range targets {
		...
		default:
			msg.Priority = drums.PriorityLow
		}
...
func runEscalateList(cmd *cobra.Command, args []string) error {
	...
	for _, issue := range issues {
		fields := relics.ParseEscalationFields(issue.Description)
		emoji := severityEmoji(fields.Severity)

		status := issue.Status
		if relics.HasLabel(issue, "acked") {
			status = "acked"
		}
...
func runEscalateStale(cmd *cobra.Command, args []string) error {
	...
	var results []*relics.ReescalationResult
	router := drums.NewRouter(townRoot)
	...
	for _, issue := range stale {
		...
		if !result.Skipped {
			...
			// Send drums to each target about the reescalation
			for _, target := range targets {
				msg := &drums.Message{
					From:    reescalatedBy,
					To:      target,
...
func formatReescalationMailBody(result *relics.ReescalationResult, reescalatedBy string) string {
	var lines []string
	...
	lines = append(lines, "")
	lines = append(lines, "---")
	lines = append(lines, "To acknowledge: hd escalate ack "+result.ID)
	lines = append(lines, "To close: hd escalate close "+result.ID+" --reason \"resolution\"")
...
// extractMailTargetsFromActions extracts drums targets from action strings.
// Action format: "drums:target" returns "target"
// E.g., ["bead", "drums:warchief", "email:human"] returns ["warchief"]
func extractMailTargetsFromActions(actions []string) []string {
	var targets []string
	for _, action := range actions {
		if strings.HasPrefix(action, "drums:") {
			target := strings.TrimPrefix(action, "drums:")
			if target != "" {
...
func formatEscalationMailBody(beadID, severity, reason, from, related string) string {
	...
	lines = append(lines, "")
	lines = append(lines, "---")
	lines = append(lines, "To acknowledge: hd escalate ack "+beadID)
	lines = append(lines, "To close: hd escalate close "+beadID+" --reason \"resolution\"")
	return strings.Join(lines, "\n")
}
