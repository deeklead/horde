...
import (
	...
	"github.com/deeklead/horde/internal/plugin"
...
var dogCmd = &cobra.Command{
	...
	Aliases: []string{"dogs"},
	GroupID: GroupAgents,
	Short:   "Manage dogs (Shaman's helper workers)",
	Long: `Manage dogs in the kennel.

Dogs are reusable helper workers managed by the Shaman for infrastructure
and cleanup tasks. Unlike raiders (single-warband, ephemeral), dogs handle
cross-warband infrastructure work with worktrees into each warband.

The kennel is located at ~/horde/shaman/dogs/.`,
}

var dogAddCmd = &cobra.Command{
	Use:   "add <name>",
	Short: "Create a new dog in the kennel",
	Long: `Create a new dog in the kennel with multi-warband worktrees.

Each dog gets a worktree per configured warband (e.g., horde, relics).
The dog starts in idle state, ready to receive work from the Shaman.
...
Examples:
  hd dog remove alpha
  hd dog remove alpha bravo
  hd dog remove --all
  hd dog remove alpha --force`,
...
This is the formalized command for sending plugin work to dogs. The Shaman
uses this during scout cycles to dispatch plugins with open gates.

The command:
1. Finds the plugin definition (plugin.md)
2. Assigns work to an idle dog (marks as working)
3. Sends drums with plugin instructions to the dog
4. Returns immediately (non-blocking)

The dog discovers the work via its drums inbox and executes the plugin
instructions. On completion, the dog sends DOG_DONE drums to shaman/.

Examples:
  hd dog dispatch --plugin rebuild-gt
  hd dog dispatch --plugin rebuild-gt --warband horde
  hd dog dispatch --plugin rebuild-gt --dog alpha
  hd dog dispatch --plugin rebuild-gt --create
  hd dog dispatch --plugin rebuild-gt --dry-run
  hd dog dispatch --plugin rebuild-gt --json`,
	RunE: runDogDispatch,
}
...
func init() {
	...
	dogDispatchCmd.Flags().StringVar(&dogDispatchRig, "warband", "", "Limit plugin search to specific warband")
...
// getDogManager creates a dog.Manager with the current encampment root.
func getDogManager() (*dog.Manager, error) {
	townRoot, err := workspace.FindFromCwd()
	if err != nil {
		return nil, fmt.Errorf("finding encampment root: %w", err)
	}
...
func runDogAdd(cmd *cobra.Command, args []string) error {
	...
	if townRoot != "" {
		...
		location := filepath.Join("shaman", "dogs", name)
...
func showPackStatus(mgr *dog.Manager) error {
	...
	if dogStatusJSON {
		...
		status := PackStatus{
			Total:     len(dogs),
			KennelDir: filepath.Join(townRoot, "shaman", "dogs"),
		}
		for _, d := range dogs {
			if d.State == dog.StateIdle {
				status.Idle++
	...
	if len(dogs) == 0 {
		fmt.Println("  No dogs in kennel")
		fmt.Println()
		fmt.Println("  Use 'hd dog add <name>' to add a dog")
		return nil
	}
	...
	if idleCount > 0 {
		fmt.Println()
		fmt.Println(style.Dim.Render("  Ready for work. Use 'hd dog call' to wake."))
	}
...
func runDogDispatch(cmd *cobra.Command, args []string) error {
	...
	// Get warband names for plugin scanner
	rigsConfigPath := filepath.Join(townRoot, "warchief", "warbands.json")
	rigsConfig, err := config.LoadRigsConfig(rigsConfigPath)
	if err != nil {
	...
	// If --warband specified, search only that warband
	if dogDispatchRig != "" {
		...
		if targetDog == nil {
			if dogDispatchCreate {
				...
				if dogDispatchDryRun {
					...
					// Create agent bead for the dog
					b := relics.New(townRoot)
					location := filepath.Join("shaman", "dogs", newName)
	...
	if dogDispatchDryRun {
		...
		if p.RigName != "" {
			...
			fmt.Printf("  Location: plugins/%s (encampment-level)\n", p.Name)
		}
	...
	// Assign work FIRST (before sending drums) to prevent race condition
	// If this fails, we haven't sent any drums yet
	if err := mgr.AssignWork(targetDog.Name, workDesc); err != nil {
		return fmt.Errorf("assigning work to dog: %w", err)
	}

	// Create and send drums message with plugin instructions
	dogAddress := fmt.Sprintf("shaman/dogs/%s", targetDog.Name)
	subject := fmt.Sprintf("Plugin: %s", p.Name)
	body := formatPluginMailBody(p)

	router := drums.NewRouterWithTownRoot(townRoot, townRoot)
	msg := &drums.Message{
		From:      "shaman/",
		To:        dogAddress,
		Subject:   subject,
	...
	if err := router.Send(msg); err != nil {
		// Rollback: clear work assignment since drums failed
		...
		return fmt.Errorf("sending plugin drums to dog: %w", err)
	}
...
// dogDispatchResult is the JSON output for hd dog dispatch.
...
func formatPluginMailBody(p *plugin.Plugin) string {
	...
	sb.WriteString("2. Send DOG_DONE drums to shaman/\n")
