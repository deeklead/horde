...
func runMailClaim(cmd *cobra.Command, args []string) error {
	...
	// Get caller identity
	caller := detectSender()
	relicsDir := relics.ResolveRelicsDir(townRoot)
	bd := relics.NewWithRelicsDir(townRoot, relicsDir)

	var queueName string
	var queueFields *relics.QueueFields
	...
	if len(args) > 0 {
		...
		queueID := relics.QueueBeadID(queueName, true) // Try encampment-level first
		...
		if issue == nil {
			// Try warband-level
			queueID = relics.QueueBeadID(queueName, false)
			issue, fields, err = bd.GetQueueBead(queueID)
			if err != nil {
				return fmt.Errorf("looking up queue: %w", err)
			}
		...
		if !relics.MatchClaimPattern(queueFields.ClaimPattern, caller) {
	...
	messages, err := listUnclaimedQueueMessages(relicsDir, queueName)
	if err != nil {
	...
	if err := claimQueueMessage(relicsDir, oldest.ID, caller); err != nil {
...
func listUnclaimedQueueMessages(relicsDir, queueName string) ([]queueMessage, error) {
	// Use rl list to find messages with queue:<name> label and status=open
	args := []string{"list",
	...
	if err := json.Unmarshal(stdout.Bytes(), &issues); err != nil {
		// If no messages, rl might output empty or error
...
func claimQueueMessage(relicsDir, messageID, claimant string) error {
...
func runMailRelease(cmd *cobra.Command, args []string) error {
	...
	msgInfo, err := getQueueMessageInfo(relicsDir, messageID)
	if err != nil {
	...
	if err := releaseQueueMessage(relicsDir, messageID, caller); err != nil {
...
func getQueueMessageInfo(relicsDir, messageID string) (*queueMessageInfo, error) {
	...
	// Parse JSON output - rl show --json returns an array
	var issues []struct {
		ID       string   `json:"id"`
		Title    string   `json:"title"`
		Labels   []string `json:"labels"`
...
func releaseQueueMessage(relicsDir, messageID, actor string) error {
	// Get current message info to find the exact claim labels
	info, err := getQueueMessageInfo(relicsDir, messageID)
	if err != nil {
		return err
	}
...
// Queue management commands (relics-native)
...
var mailQueueCmd = &cobra.Command{
	Use:   "queue",
	Short: "Manage drums queues",
	Long: `Manage relics-native drums queues.
...
Examples:
  hd drums queue create work --claimers 'horde/raiders/*'
  hd drums queue show work
  hd drums queue list
  hd drums queue delete work`,
...
var mailQueueCreateCmd = &cobra.Command{
	...
	Long: `Create a new relics-native drums queue.

The --claimers flag specifies a pattern for who can claim messages from this queue.
Patterns support wildcards: 'horde/raiders/*' matches any raider in horde warband.

Examples:
  hd drums queue create work --claimers 'horde/raiders/*'
  hd drums queue create dispatch --claimers 'horde/clan/*'
  hd drums queue create urgent --claimers '*'`,
...
var mailQueueShowCmd = &cobra.Command{
	...
	Long: `Show details about a drums queue.

Displays the queue's claim pattern, status, and message counts.

Examples:
  hd drums queue show work
  hd drums queue show dispatch --json`,
	Args: cobra.ExactArgs(1),
...
var mailQueueListCmd = &cobra.Command{
	...
	Long: `List all relics-native drums queues.

Shows queue names, claim patterns, and status.

Examples:
  hd drums queue list
  hd drums queue list --json`,
...
func init() {
	...
	// Add queue command to drums
	mailCmd.AddCommand(mailQueueCmd)
}

// runMailQueueCreate creates a new relics-native queue.
func runMailQueueCreate(cmd *cobra.Command, args []string) error {
	...
	// Create queue bead
	b := relics.NewWithRelicsDir(townRoot, relics.ResolveRelicsDir(townRoot))

	// Generate queue bead ID (encampment-level: hq-q-<name>)
	queueID := relics.QueueBeadID(queueName, true)
	...
	// Create queue fields
	fields := &relics.QueueFields{
		Name:         queueName,
		ClaimPattern: mailQueueClaimers,
		Status:       relics.QueueStatusActive,
		CreatedBy:    caller,
...
func runMailQueueList(cmd *cobra.Command, args []string) error {
	...
	if mailQueueJSON {
		...
		for _, issue := range queues {
			fields := relics.ParseQueueFields(issue.Description)
