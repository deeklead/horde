...
var moleculeAwaitSignalCmd = &cobra.Command{
	...
	Long: `Wait for any activity on the relics feed, with optional backoff.

This command is the primary wake mechanism for scout agents. It subscribes
to 'bd activity --follow' and returns immediately when any line of output
is received (indicating relics activity).
...
EXAMPLES:
  ...
  hd mol await-signal --timeout 60s

  # Backoff mode with agent bead tracking:
  hd mol await-signal --agent-bead gt-horde-witness \
    --backoff-base 30s --backoff-mult 2 --backoff-max 5m

  # On timeout, the agent bead's idle:N label is auto-incremented
  # On signal, caller should reset: hd agent state gt-horde-witness --set idle=0

  # Quiet mode (no output, for scripting)
  hd mol await-signal --timeout 30s --quiet`,
...
func runMoleculeAwaitSignal(cmd *cobra.Command, args []string) error {
	...
	if awaitSignalAgentBead != "" {
		labels, err := getAgentLabels(awaitSignalAgentBead, relicsDir)
		if err != nil {
	...
	if result.Reason == "timeout" && awaitSignalAgentBead != "" {
		...
		if err := setAgentIdleCycles(awaitSignalAgentBead, relicsDir, newIdleCycles); err != nil {
			if !awaitSignalQuiet {
...
// If backoff parameters are provided, uses exponential backoff ritual:
...
// waitForActivitySignal starts rl activity --follow and waits for any output.
...
func waitForActivitySignal(ctx context.Context, workDir string) (*AwaitSignalResult, error) {
	...
	if err := cmd.Start(); err != nil {
		return nil, fmt.Errorf("starting rl activity: %w", err)
	}
...
// This is used by scout agents to get the timeout for await-signal.
func GetCurrentStepBackoff(workDir string) (*relics.BackoffConfig, error) {
	b := relics.New(workDir)

	// Get current agent's hook
	// This would need to query the pinned/bannered bead and parse its description
...
func setAgentIdleCycles(agentBead, relicsDir string, cycles int) error {
	...
	// Use rl update with --set-labels to replace all labels
