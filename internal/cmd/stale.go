...
var staleCmd = &cobra.Command{
	Use:     "stale",
	GroupID: GroupDiag,
	Short:   "Check if the hd binary is stale",
	Long: `Check if the hd binary was built from an older commit than the current repo HEAD.

This command compares the commit hash embedded in the binary at build time
with the current HEAD of the horde repository.

Examples:
  hd stale              # Human-readable output
  hd stale --json       # Machine-readable JSON output
  hd stale --quiet      # Exit code only (0=stale, 1=fresh)
...
func runStale(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("cannot find horde repo: %w", err)
	}
...
func outputStaleText(output StaleOutput) error {
	if output.Stale {
		...
		fmt.Printf("\n  Run 'go install ./cmd/hd' to rebuild\n")
	} else {
