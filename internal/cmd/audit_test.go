...
func TestExtractAuthorName(t *testing.T) {
	tests := []struct {
		...
		{"horde/clan/joe", "joe"},
		{"horde/raiders/toast", "toast"},
		{"warchief", "warchief"},
		{"horde/witness", "witness"},
		{"", ""},
	}
...
func TestParseRelicsTimestamp(t *testing.T) {
	tests := []struct {
		input    string
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			got := parseRelicsTimestamp(tt.input)
			if tt.isZero {
				if !got.IsZero() {
					t.Errorf("parseRelicsTimestamp(%q) expected zero time, got %v", tt.input, got)
				}
				return
			}
			gotStr := got.Format("2006-01-02 15:04")
			if gotStr != tt.expected {
				t.Errorf("parseRelicsTimestamp(%q) = %q, want %q", tt.input, gotStr, tt.expected)
			}
		})
	}
}
...
func TestFormatSource(t *testing.T) {
	...
	sources := []string{"git", "relics", "encampmentlog", "events", "unknown"}
...
func TestFormatType(t *testing.T) {
	// Just verify it doesn't panic and returns non-empty strings
	types := []string{"commit", "bead_created", "bead_closed", "muster", "done", "handoff", "crash", "kill", "merged", "merge_failed", "unknown"}
