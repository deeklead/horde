...
// This file implements the hd warband config commands for viewing and manipulating
// warband configuration across property layers.
package cmd
...
var rigConfigCmd = &cobra.Command{
	Use:   "config",
	Short: "View and manage warband configuration",
	Long: `View and manage warband configuration across property layers.

Configuration is looked up through multiple layers:
1. Wisp layer (transient, local) - .relics-wisp/config/
2. Bead layer (persistent, synced) - warband identity bead labels
3. Encampment defaults - ~/horde/settings/config.json
...
Examples:
  hd warband config set horde status parked           # Wisp layer
  hd warband config set horde status docked --global  # Bead layer
  hd warband config set horde auto_restart --block    # Block inheritance`,
	Args: cobra.RangeArgs(2, 3),
	RunE: runRigConfigSet,
}

var rigConfigUnsetCmd = &cobra.Command{
	Use:   "unset <warband> <key>",
...
Example:
  hd warband config unset horde status`,
...
func runRigConfigShow(cmd *cobra.Command, args []string) error {
	...
	if rigConfigShowLayers {
		...
		for _, key := range allKeys {
			...
			if result.Source == warband.SourceBlocked {
				valueStr = "(blocked)"
			}
		...
		for _, key := range allKeys {
			result := r.GetConfigWithSource(key)
			if result.Source == warband.SourceNone {
...
func runRigConfigSet(cmd *cobra.Command, args []string) error {
	...
	if rigConfigSetBlock {
		...
		fmt.Printf("%s Blocked %s for warband %s\n", style.Success.Render("✓"), key, rigName)
		return nil
	}
	...
	if rigConfigSetGlobal {
		...
		fmt.Printf("%s Set %s=%s in wisp layer for warband %s\n", style.Success.Render("✓"), key, value, rigName)
	}
...
func runRigConfigUnset(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("%s Unset %s from wisp layer for warband %s\n", style.Success.Render("✓"), key, rigName)
	return nil
}

// getConfigKeys returns all known configuration keys, sorted.
func getConfigKeys(townRoot string, r *warband.Warband) []string {
	keySet := make(map[string]bool)

	// System defaults
	for k := range warband.SystemDefaults {
		keySet[k] = true
	}
	...
	// Bead labels (from warband identity bead)
	prefix := "hd"
...
// setBeadLabel sets a label on the warband identity bead.
func setBeadLabel(townRoot string, r *warband.Warband, key, value string) error {
	prefix := "hd"
	if r.Config != nil && r.Config.Prefix != "" {
		prefix = r.Config.Prefix
	}
	...
	if err != nil {
		return fmt.Errorf("warband identity bead %s not found (run 'hd warband add' to create it)", rigBeadID)
	}
	...
	// Update the bead
	return bd.Update(rigBeadID, relics.UpdateOptions{
		SetLabels: newLabels,
	})
}
