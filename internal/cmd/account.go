...
var accountCmd = &cobra.Command{
	...
	Long: `Manage multiple Claude Code accounts for Horde.

This enables switching between accounts (e.g., personal vs work) with
easy account selection per muster or globally.

Commands:
  hd account list              List registered accounts
  hd account add <handle>      Add a new account
  hd account default <handle>  Set the default account
  hd account status            Show current account info`,
}
...
Examples:
  hd account list           # Text output
  hd account list --json    # JSON output`,
	RunE: runAccountList,
}
...
Examples:
  hd account add work
  hd account add work --email steve@company.com
  hd account add work --email steve@company.com --desc "Work account"`,
...
Examples:
  hd account default work
  hd account default personal`,
	Args: cobra.ExactArgs(1),
...
func runAccountList(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		...
		fmt.Println("\nTo add an account:")
		fmt.Println("  hd account add <handle>")
		return nil
	}
...
Examples:
  hd account switch work       # Switch to work account
  hd account switch personal   # Switch to personal account`,
	Args: cobra.ExactArgs(1),
...
func runAccountSwitch(cmd *cobra.Command, args []string) error {
	...
	if err == nil && fileInfo.Mode()&os.ModeSymlink == 0 && fileInfo.IsDir() {
		...
		if currentHandle != "" {
			...
			return fmt.Errorf("~/.claude is a directory but no default account is set. Please set a default account first with 'hd account default <handle>'")
		}
