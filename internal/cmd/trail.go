...
Subcommands:
  ...
  relics      Recent relics (work items)
...
Examples:
  hd trail                     # Recent commits (default)
  hd trail commits             # Same as above
  hd trail commits --since 1h  # Last hour
  hd trail relics               # Recent relics
  hd trail hooks               # Recent hook activity
  hd recent                    # Alias for hd trail
  hd recap --since 24h         # Activity from last 24 hours`,
...
Examples:
  hd trail commits              # Recent agent commits
  hd trail commits --since 1h   # Last hour of commits
  hd trail commits --all        # All commits (including non-agents)
  hd trail commits --json       # JSON output`,
	RunE: runTrailCommits,
}

var trailRelicsCmd = &cobra.Command{
	Use:   "relics",
	Short: "Show recent relics",
	Long: `Show recently created or modified relics (work items).

Examples:
  hd trail relics              # Recent relics
  hd trail relics --since 24h  # Last 24 hours of relics
  hd trail relics --json       # JSON output`,
	RunE: runTrailRelics,
}
...
Examples:
  hd trail hooks              # Recent hook activity
  hd trail hooks --since 1h   # Last hour of hook activity
  hd trail hooks --json       # JSON output`,
	RunE: runTrailHooks,
}
...
func init() {
	...
	trailCmd.AddCommand(trailRelicsCmd)
...
func runTrailRelics(cmd *cobra.Command, args []string) error {
	// Find relics directory
	relicsDir, err := findRelicsDir()
	if err != nil {
		return fmt.Errorf("finding relics: %w", err)
	}

	// Use relics query to get recent relics
	relicsArgs := []string{
		"query",
	...
	if trailSince != "" {
		...
		since := time.Now().Add(-duration)
		relicsArgs = append(relicsArgs, "--since", since.Format(time.RFC3339))
	}

	relicsCmd := exec.Command("relics", relicsArgs...)
	relicsCmd.Dir = relicsDir
	relicsCmd.Env = append(os.Environ(), "RELICS_DIR="+relicsDir+"/.relics")
	output, err := relicsCmd.Output()
	if err != nil {
		// Fallback: relics might not support all these flags
		// Try a simpler approach
		return runTrailRelicsSimple(relicsDir)
	}

	// Parse output
	var relics []BeadEntry
	lines := strings.Split(strings.TrimSpace(string(output)), "\n")
	for _, line := range lines {
		if line == "" {
			continue
		...
		relics = append(relics, BeadEntry{
			ID:        parts[0],
			Title:     parts[1],
			Status:    parts[2],
			Agent:     parts[3],
	...
	fmt.Printf("%s\n\n", style.Bold.Render("Recent Relics"))
	for _, b := range relics {
...
func runTrailRelicsSimple(relicsDir string) error {
	// Simple fallback using relics list
	relicsCmd := exec.Command("relics", "list", "--limit", fmt.Sprintf("%d", trailLimit))
	relicsCmd.Dir = relicsDir
	relicsCmd.Env = append(os.Environ(), "RELICS_DIR="+relicsDir+"/.relics")
	relicsCmd.Stdout = os.Stdout
	relicsCmd.Stderr = os.Stderr
	return relicsCmd.Run()
}
...
func findRelicsDir() (string, error) {
	// Try local relics dir first
	dir, err := findLocalRelicsDir()
	if err == nil {
		return dir, nil
	}

	// Fall back to encampment root
