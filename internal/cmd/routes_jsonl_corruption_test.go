...
// TestRoutesJSONLCorruption tests that routes.jsonl is not corrupted by rl auto-export.
func TestRoutesJSONLCorruption(t *testing.T) {
	...
	t.Run("TownLevelRoutesNotCorrupted", func(t *testing.T) {
		// Test that hd install creates issues.jsonl before routes.jsonl
		// so that rl auto-export doesn't corrupt routes.jsonl
		tmpDir := t.TempDir()
		...
		// Install encampment
		cmd := exec.Command(gtBinary, "install", townRoot, "--name", "test-encampment")
		...
		issuesPath := filepath.Join(townRoot, ".relics", "issues.jsonl")
		if _, err := os.Stat(issuesPath); os.IsNotExist(err) {
			t.Error("issues.jsonl should exist after hd install")
		}

		// Verify routes.jsonl exists and has valid content
		routesPath := filepath.Join(townRoot, ".relics", "routes.jsonl")
		...
		cmd = exec.Command("rl", "--no-daemon", "-q", "create", "--type", "task", "--title", "test issue")
		cmd.Dir = townRoot
		if output, err := cmd.CombinedOutput(); err != nil {
		...
		if strings.Contains(string(routesContent), `"title"`) {
			t.Errorf("routes.jsonl was corrupted with issue data after rl create: %s", routesContent)
		}
		if !strings.Contains(string(routesContent), `"prefix"`) {
	...
	t.Run("RigLevelNoRoutesJSONL", func(t *testing.T) {
		// Test that hd warband add does NOT create routes.jsonl in warband relics
		// (warband-level routes.jsonl breaks bd's walk-up routing to encampment routes)
		tmpDir := t.TempDir()
		townRoot := filepath.Join(tmpDir, "test-encampment")
		...
		// Add a warband
		cmd = exec.Command(gtBinary, "warband", "add", "testrig", repoDir)
		cmd.Dir = townRoot
		cmd.Env = append(os.Environ(), "HOME="+tmpDir)
		if output, err := cmd.CombinedOutput(); err != nil {
		...
		// Verify warband relics directory exists
		rigRelicsDir := filepath.Join(townRoot, "testrig", ".relics")
		if _, err := os.Stat(rigRelicsDir); os.IsNotExist(err) {
			t.Fatal("warband .relics directory should exist")
		}

		// Verify issues.jsonl exists in warband relics
		rigIssuesPath := filepath.Join(rigRelicsDir, "issues.jsonl")
		if _, err := os.Stat(rigIssuesPath); os.IsNotExist(err) {
			t.Error("issues.jsonl should exist in warband relics")
		}

		// Verify routes.jsonl does NOT exist in warband relics
		rigRoutesPath := filepath.Join(rigRelicsDir, "routes.jsonl")
		if _, err := os.Stat(rigRoutesPath); err == nil {
			t.Error("routes.jsonl should NOT exist in warband relics (breaks walk-up routing)")
		}
	})

	t.Run("CorruptionReproduction", func(t *testing.T) {
		// This test reproduces the bug: if issues.jsonl doesn't exist,
		// rl auto-export writes to routes.jsonl
		tmpDir := t.TempDir()
		relicsDir := filepath.Join(tmpDir, ".relics")
		os.MkdirAll(relicsDir, 0755)
		...
		cmd = exec.Command("rl", "--no-daemon", "-q", "create", "--type", "task", "--title", "bug reproduction")
		cmd.Dir = tmpDir
