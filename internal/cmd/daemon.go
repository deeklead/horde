...
var daemonCmd = &cobra.Command{
	...
	Short:   "Manage the Horde daemon",
	RunE:    requireSubcommand,
	Long: `Manage the Horde background daemon.
...
var daemonStartCmd = &cobra.Command{
	...
	Long: `Start the Horde daemon in the background.

The daemon will run until stopped with 'hd daemon stop'.`,
	RunE: runDaemonStart,
}
...
var daemonStopCmd = &cobra.Command{
	...
	Long:  `Stop the running Horde daemon.`,
	RunE:  runDaemonStop,
}
...
var daemonStatusCmd = &cobra.Command{
	...
	Long:  `Show the current status of the Horde daemon.`,
...
func runDaemonStart(cmd *cobra.Command, args []string) error {
	...
	// We use 'hd daemon run' as the actual daemon process
	...
	if !running {
		return fmt.Errorf("daemon failed to start (check logs with 'hd daemon logs')")
	}
...
func runDaemonStatus(cmd *cobra.Command, args []string) error {
	...
	if running {
			...
			if binaryModTime, err := getBinaryModTime(); err == nil {
				...
				if binaryModTime.After(state.StartedAt) {
					fmt.Printf("  %s Binary is newer than process - consider '%s'\n",
						style.Bold.Render("⚠"),
						style.Dim.Render("hd daemon stop && hd daemon start"))
				}
			}
		}
	} else {
		fmt.Printf("%s Daemon is %s\n",
			style.Dim.Render("○"),
			"not running")
		fmt.Printf("\nStart with: %s\n", style.Dim.Render("hd daemon start"))
	}
