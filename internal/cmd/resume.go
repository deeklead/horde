...
By default, this command checks for parked work (from 'hd park') and whether
...
Examples:
  hd resume              # Check for and resume parked work
  hd resume --status     # Just show parked work status without resuming
  hd resume --handoff    # Check inbox for handoff messages`,
	RunE: runResume,
}
...
func runResume(cmd *cobra.Command, args []string) error {
	...
	if parked == nil {
		...
		fmt.Printf("  Use 'hd park <gate-id>' to park work on a gate\n")
		return nil
	}

	// Check gate status
	gateCheck := exec.Command("rl", "gate", "show", parked.GateID, "--json")
	...
	if parked.BeadID != "" {
		pinCmd := exec.Command("rl", "update", parked.BeadID, "--status=pinned", "--assignee="+agentID)
...
func displayResumeStatus(status ResumeStatus, parked *ParkedWork) error {
	...
	if status.GateClosed {
		fmt.Printf("\n%s Gate cleared! Run 'hd resume' (without --status) to restore work.\n",
			style.Bold.Render("→"))
	}
...
func checkHandoffMessages() error {
	...
	inboxCmd := exec.Command("hd", "drums", "inbox", "--json")
	output, err := inboxCmd.Output()
	if err != nil {
		// Fallback to non-JSON if --json not supported
		inboxCmd = exec.Command("hd", "drums", "inbox")
		output, err = inboxCmd.Output()
		if err != nil {
	...
	if len(handoffs) == 0 {
		fmt.Printf("%s No handoff messages in inbox\n", style.Dim.Render("○"))
		fmt.Printf("  Handoff messages have 'HANDOFF' in the subject.\n")
		fmt.Printf("  Use 'hd handoff -s \"...\"' to create one when handing off.\n")
		return nil
	}
	...
	if len(handoffs) == 1 {
		fmt.Printf("%s Read full message: hd drums read %s\n", style.Bold.Render("→"), handoffs[0].ID)
	} else {
		fmt.Printf("%s Read messages: hd drums read <id>\n", style.Bold.Render("→"))
	}
	fmt.Printf("%s Clear after reading: hd drums close <id>\n", style.Dim.Render("💡"))
