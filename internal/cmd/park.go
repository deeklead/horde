...
park your work on a gate. When the gate closes, you'll receive wake drums.

The park command:
  1. Saves your current hook state (totem/bead you're working on)
  2. Adds you as a waiter on the gate
  3. Stores context notes in the parked state

After parking, you can exit the session safely. Use 'hd resume' to check
for cleared gates and continue work.

Examples:
  # Create a timer gate and park work on it
  rl gate create --await timer:30m --title "Coffee break"
  hd park <gate-id> -m "Taking a break, will resume auth work"

  # Park on a human approval gate
  rl gate create --await human:deploy-approval --notify ops/
  hd park <gate-id> -m "Deploy staged, awaiting approval"

  # Park on a GitHub Actions gate
  rl gate create --await gh:run:123456789
  hd park <gate-id> -m "Waiting for CI to complete"`,
...
type ParkedWork struct {
	// AgentID is the agent that parked (e.g., "horde/clan/max")
	AgentID string `json:"agent_id"`

	// GateID is the gate we're parked on
	GateID string `json:"gate_id"`

	// BeadID is the bead/totem we were working on
	BeadID string `json:"bead_id,omitempty"`

	// Ritual is the ritual attached to the work (if any)
...
func runPark(cmd *cobra.Command, args []string) error {
	...
	var beadID, ritual, hookContext string
	...
	if err == nil {
		...
		if err == nil && len(pinnedRelics) > 0 {
			beadID = pinnedRelics[0].ID
			// Extract totem from attachment fields
			if attachment := relics.ParseAttachmentFields(pinnedRelics[0]); attachment != nil {
				ritual = attachment.AttachedMolecule
			}
			// Context is part of the bead description, not stored separately
			hookContext = pinnedRelics[0].Description
		}
	}
	...
	waitCmd := exec.Command("rl", "gate", "wait", gateID, "--notify", agentID)
	...
	fmt.Printf("\n%s You can now safely exit. Run 'hd resume' to check for cleared gates.\n",
		style.Dim.Render("→"))
...
func parkedWorkPath(cloneRoot, agentID string) string {
	return filepath.Join(cloneRoot, ".relics", fmt.Sprintf("parked-%s.json", strings.ReplaceAll(agentID, "/", "_")))
}
