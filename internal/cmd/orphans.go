...
Examples:
  hd orphans              # Last 7 days (default)
  hd orphans --days=14    # Last 2 weeks
  hd orphans --all        # Show all orphans (no date filter)`,
	RunE: runOrphans,
}
...
The command will:
1. Find orphaned commits (same as 'hd orphans')
2. Show what will be removed
3. Ask for confirmation (unless --force)
4. Run git gc --prune=now

Examples:
  hd orphans kill              # Kill orphans from last 7 days (default)
  hd orphans kill --days=14    # Kill orphans from last 2 weeks
  hd orphans kill --all        # Kill all orphans
  hd orphans kill --dry-run    # Preview without deleting
  hd orphans kill --force      # Skip confirmation prompt`,
	RunE: runOrphansKill,
}
...
Examples:
  hd orphans procs        # List orphaned Claude processes
  hd orphans procs list   # Same as above
  hd orphans procs kill   # Kill orphaned processes`,
...
Examples:
  hd orphans procs list      # Show all orphan Claude processes`,
...
Examples:
  hd orphans procs kill      # Kill with confirmation
  hd orphans procs kill -f   # Force kill without confirmation`,
...
func runOrphans(cmd *cobra.Command, args []string) error {
	// Find workspace to determine warband root
	...
	if err != nil {
		return fmt.Errorf("determining warband: %w", err)
	}

	// We need to run from the warchief's clone (main git repo for the warband)
	warchiefPath := r.Path + "/warchief/warband"

	fmt.Printf("Scanning for orphaned commits in %s...\n\n", rigName)

	// Run git fsck
	orphans, err := findOrphanCommits(warchiefPath)
	if err != nil {
...
func isNoiseCommit(subject string) bool {
	...
	noisePrefixes := []string{
		...
		"bd sync:",         // Relics sync commits (routine)
		"bd sync: ",        // Relics sync commits (routine)
	}
...
func runOrphansListProcesses(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("\n%s\n", style.Dim.Render("Use 'hd orphans procs kill' to terminate these processes"))
