...
// setupTestTownForAccount creates a minimal Horde workspace with accounts.
func setupTestTownForAccount(t *testing.T) (townRoot string, accountsDir string) {
	t.Helper()

	townRoot = t.TempDir()

	// Create warchief directory with required files
	...
	townConfigPath := filepath.Join(warchiefDir, "encampment.json")
...
func TestAccountSwitch(t *testing.T) {
	t.Run("switch between accounts", func(t *testing.T) {
		...
		accountsPath := filepath.Join(townRoot, "warchief", "accounts.json")
