...
Examples:
  hd hooks              # List all hooks in workspace
  hd hooks --verbose    # Show hook commands
  hd hooks --json       # Output as JSON`,
	RunE: runHooks,
}
...
type HookInfo struct {
	...
	Agent    string   `json:"agent"`    // Agent that owns this hook (e.g., "raider/nux")
...
func discoverHooks(townRoot string) ([]HookInfo, error) {
	...
	// NOTE: Warchief settings are at ~/horde/warchief/.claude/, NOT ~/horde/.claude/
	// Settings at encampment root would pollute all child workspaces.
	locations := []struct {
		path  string
		agent string
	}{
		{filepath.Join(townRoot, "warchief", ".claude", "settings.json"), "warchief/"},
		{filepath.Join(townRoot, "shaman", ".claude", "settings.json"), "shaman/"},
	}
	...
	for _, entry := range entries {
		...
		locations = append(locations, struct {
			path  string
			agent string
		}{filepath.Join(rigPath, ".claude", "settings.json"), fmt.Sprintf("%s/warband", rigName)})

		// Raiders
		raidersDir := filepath.Join(rigPath, "raiders")
		if raiders, err := os.ReadDir(raidersDir); err == nil {
			for _, p := range raiders {
				if p.IsDir() && !strings.HasPrefix(p.Name(), ".") {
					locations = append(locations, struct {
						path  string
						agent string
					}{filepath.Join(raidersDir, p.Name(), ".claude", "settings.json"), fmt.Sprintf("%s/%s", rigName, p.Name())})
				}
			}
		}
		...
		if clan, err := os.ReadDir(crewDir); err == nil {
			for _, c := range clan {
				if c.IsDir() {
					locations = append(locations, struct {
						path  string
						agent string
					}{filepath.Join(crewDir, c.Name(), ".claude", "settings.json"), fmt.Sprintf("%s/clan/%s", rigName, c.Name())})
				}
			}
		}
		...
		// Forge
		forgePath := filepath.Join(rigPath, "forge", ".claude", "settings.json")
		locations = append(locations, struct {
			path  string
			agent string
		}{forgePath, fmt.Sprintf("%s/forge", rigName)})
	}
