...
// Priority: CLI flag > warband config > default
func getIntegrationBranchTemplate(rigPath, cliOverride string) string {
	if cliOverride != "" {
		return cliOverride
	}

	// Try to load warband settings
...
func runMqIntegrationCreate(cmd *cobra.Command, args []string) error {
	...
	// Initialize relics for the warband
	bd := relics.New(r.Path)

	// 1. Verify epic exists
	epic, err := bd.Show(epicID)
	if err != nil {
		if err == relics.ErrNotFound {
	...
	// Initialize git for the warband
	g := git.NewGit(r.Path)
	...
	if newDesc != epic.Description {
		if err := bd.Update(epicID, relics.UpdateOptions{Description: &newDesc}); err != nil {
	...
	fmt.Printf("    hd mq submit --epic %s\n", epicID)
...
func runMqIntegrationLand(cmd *cobra.Command, args []string) error {
	...
	// Initialize relics and git for the warband
	bd := relics.New(r.Path)
	g := git.NewGit(r.Path)
...
func findOpenMRsForIntegration(bd *relics.Relics, targetBranch string) ([]*relics.Issue, error) {
...
func filterMRsByTarget(mrs []*relics.Issue, targetBranch string) []*relics.Issue {
	var result []*relics.Issue
	for _, mr := range mrs {
		fields := relics.ParseMRFields(mr)
...
// getTestCommand returns the test command from warband settings.
...
func runMqIntegrationStatus(cmd *cobra.Command, args []string) error {
	...
	allMRs, err := bd.List(relics.ListOptions{
		Type:   "merge-request",
		Status: "", // all statuses
	})
	if err != nil {
	...
	var mergedMRs, pendingMRs []*relics.Issue
