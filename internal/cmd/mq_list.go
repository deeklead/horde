...
func runMQList(cmd *cobra.Command, args []string) error {
	...
	_, r, _, err := getForgeManager(rigName)
	if err != nil {
		return err
	}

	// Create relics wrapper for the warband - use RelicsPath() to get the git-synced location
	...
	table := style.NewTable(
		...
		style.Column{Name: "RAID", Width: 12},
	...
	for _, item := range scored {
		...
		// Format raid column
		raidDisplay := style.Dim.Render("(none)")
		if raidID != "" {
			// Truncate raid ID for display
			if len(raidID) > 12 {
				raidID = raidID[:12]
			}
			raidDisplay = raidID
		...
		table.AddRow(displayID, scoreStr, priority, raidDisplay, branch, styledStatus, style.Dim.Render(age))
	}
...
// calculateMRScore computes the priority score for an MR using the forge scoring function.
// Higher scores mean higher priority (process first).
func calculateMRScore(issue *relics.Issue, fields *relics.MRFields, now time.Time) float64 {
	...
	if fields != nil {
		...
		if fields.RaidCreatedAt != "" {
			if raidTime, err := time.Parse(time.RFC3339, fields.RaidCreatedAt); err == nil {
				input.RaidCreatedAt = &raidTime
			}
		}
	}

	return forge.ScoreMRWithDefaults(input)
}
