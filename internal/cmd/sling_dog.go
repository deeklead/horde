...
// Patterns:
//   - "shaman/dogs" -> ("", true) - dispatch to any idle dog
//   - "shaman/dogs/alpha" -> ("alpha", true) - dispatch to specific dog
func IsDogTarget(target string) (dogName string, isDog bool) {
	target = strings.ToLower(target)

	// Check for exact "shaman/dogs" (pool dispatch)
	if target == "shaman/dogs" {
		return "", true
	}

	// Check for "shaman/dogs/<name>" (specific dog)
	if strings.HasPrefix(target, "shaman/dogs/") {
		name := strings.TrimPrefix(target, "shaman/dogs/")
...
type DogDispatchInfo struct {
	...
	AgentID string // Agent ID format (shaman/dogs/<name>)
...
func DispatchToDog(dogName string, create bool) (*DogDispatchInfo, error) {
	...
	// Build agent ID
	agentID := fmt.Sprintf("shaman/dogs/%s", targetDog.Name)

	// Try to find tmux session for the dog (dogs may run in tmux like raiders)
	// Dogs use the pattern gt-{encampment}-shaman-{name}
	townName, _ := workspace.GetTownName(townRoot)
	sessionName := fmt.Sprintf("gt-%s-shaman-%s", townName, targetDog.Name)
	t := tmux.NewTmux()
	var pane string
