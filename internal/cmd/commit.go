...
const DefaultAgentEmailDomain = "horde.local"
...
The email domain is configurable in encampment settings (agent_email_domain).
Default: horde.local

Examples:
  hd commit -m "Fix bug"              # Commit as current agent
  hd commit -am "Quick fix"           # Stage all and commit
  hd commit -- --amend                # Amend last commit
...
func runCommit(cmd *cobra.Command, args []string) error {
	...
	// Load agent email domain from encampment settings
	...
	// "horde/clan/jack" → "horde.clan.jack@domain"
...
// identityToEmail converts a Horde identity to a git email address.
// "horde/clan/jack" → "horde.clan.jack@domain"
// "warchief/" → "warchief@domain"
