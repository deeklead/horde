...
func TestListRaiderWorktrees_SkipsHiddenDirs(t *testing.T) {
	tmpDir := t.TempDir()
	raidersDir := filepath.Join(tmpDir, "some-warband", "raiders")

	if err := os.MkdirAll(filepath.Join(raidersDir, ".claude"), 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(raidersDir, "furiosa"), 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(raidersDir, "not-a-dir.txt"), []byte("x"), 0644); err != nil {
		t.Fatal(err)
	}

	raiders, err := listRaiderWorktrees(raidersDir)
	if err != nil {
		t.Fatalf("listRaiderWorktrees returned error: %v", err)
	}

	if slices.Contains(raiders, ".claude") {
		t.Fatalf("expected hidden dir .claude to be ignored, got %v", raiders)
	}
	if !slices.Contains(raiders, "furiosa") {
		t.Fatalf("expected furiosa to be included, got %v", raiders)
	}
}

// NOTE: TestIsWitnessSession removed - isWitnessSession function was deleted
// as part of ZFC cleanup. Witness poking is now Shaman's responsibility.
