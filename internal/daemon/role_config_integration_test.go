...
func runBd(t *testing.T, dir string, args ...string) string {
	t.Helper()
	cmd := exec.Command("rl", args...) //nolint:gosec // rl is a trusted internal tool in this repo
	cmd.Dir = dir
...
func TestGetRoleConfigForIdentity_PrefersTownRoleBead(t *testing.T) {
	...
	runBd(t, townRoot, "config", "set", "types.custom", "agent,role,warband,raid,event")
...
func TestGetRoleConfigForIdentity_FallsBackToLegacyRoleBead(t *testing.T) {
	...
	townRoot := t.TempDir()
	runBd(t, townRoot, "init", "--quiet", "--prefix", "hd")
