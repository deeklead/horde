// Package daemon provides the encampment-level background service for Horde.
//
...
type Config struct {
	...
	// TownRoot is the Horde workspace root.
...
func DefaultConfig(townRoot string) *Config {
	...
	return &Config{
		HeartbeatInterval: 5 * time.Minute, // Shaman wakes on drums too, no need to poke often
		TownRoot:          townRoot,
...
type LifecycleRequest struct {
	// From is the agent requesting the action (e.g., "warchief/", "horde/witness").
