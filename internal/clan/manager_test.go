...
func TestManagerAddAndGet(t *testing.T) {
	...
	tmpDir, err := os.MkdirTemp("", "clan-test-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer func() { _ = os.RemoveAll(tmpDir) }()

	// Create a mock warband
	rigPath := filepath.Join(tmpDir, "test-warband")
	if err := os.MkdirAll(rigPath, 0755); err != nil {
		t.Fatalf("failed to create warband dir: %v", err)
	}

	// Initialize git repo for the warband
	...
	if worker.Warband != "test-warband" {
		t.Errorf("expected warband 'test-warband', got '%s'", worker.Warband)
	}
	...
	crewDir := filepath.Join(rigPath, "clan", "dave")
	if _, err := os.Stat(crewDir); os.IsNotExist(err) {
		t.Error("clan directory was not created")
	}

	mailDir := filepath.Join(crewDir, "drums")
	if _, err := os.Stat(mailDir); os.IsNotExist(err) {
		t.Error("drums directory was not created")
	}
...
func TestManagerAddUsesLocalRepoReference(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "clan-test-local-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer func() { _ = os.RemoveAll(tmpDir) }()
...
func TestManagerAddWithBranch(t *testing.T) {
	// Create temp directory for test
	tmpDir, err := os.MkdirTemp("", "clan-test-branch-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer func() { _ = os.RemoveAll(tmpDir) }()
	...
	if worker.Branch != "clan/emma" {
		t.Errorf("expected branch 'clan/emma', got '%s'", worker.Branch)
	}
}

func TestManagerList(t *testing.T) {
	// Create temp directory for test
	tmpDir, err := os.MkdirTemp("", "clan-test-list-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer func() { _ = os.RemoveAll(tmpDir) }()
...
func TestManagerRemove(t *testing.T) {
	// Create temp directory for test
	tmpDir, err := os.MkdirTemp("", "clan-test-remove-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer func() { _ = os.RemoveAll(tmpDir) }()
