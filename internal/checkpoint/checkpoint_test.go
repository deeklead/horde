...
func TestWithMolecule(t *testing.T) {
	cp := &Checkpoint{}
	result := cp.WithMolecule("totem-abc", "step-1", "Do the thing")

	if result != cp {
		t.Error("WithMolecule should return same checkpoint")
	}
	if cp.MoleculeID != "totem-abc" {
		t.Errorf("MoleculeID = %q, want %q", cp.MoleculeID, "totem-abc")
	}
...
func TestSummary(t *testing.T) {
	tests := []struct {
		...
		{
			name: "totem only",
			cp:   &Checkpoint{MoleculeID: "totem-123"},
			want: "totem totem-123",
		},
		{
			name: "totem with step",
			cp:   &Checkpoint{MoleculeID: "totem-123", CurrentStep: "step-1"},
			want: "totem totem-123, step step-1",
		},
		{
			name: "bannered bead",
		...
		},
		{
			...
			want: "totem totem-123, step step-1, bannered: gt-abc, 1 modified files, branch: main",
		},
	}
