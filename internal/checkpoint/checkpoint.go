...
// When a raider session dies (context limit, crash, timeout), checkpoints
...
const Filename = ".raider-checkpoint.json"

// Checkpoint represents a session recovery checkpoint.
type Checkpoint struct {
	// MoleculeID is the current totem being worked.
...
// Path returns the checkpoint file path for a given raider directory.
func Path(raiderDir string) string {
	return filepath.Join(raiderDir, Filename)
}

// Read loads a checkpoint from the raider directory.
// Returns nil, nil if no checkpoint exists.
func Read(raiderDir string) (*Checkpoint, error) {
	path := Path(raiderDir)

	data, err := os.ReadFile(path) //nolint:gosec // G304: path is constructed from trusted raiderDir
	if err != nil {
...
// Write saves a checkpoint to the raider directory.
func Write(raiderDir string, cp *Checkpoint) error {
...
func Remove(raiderDir string) error {
	path := Path(raiderDir)
...
func Capture(raiderDir string) (*Checkpoint, error) {
	cp := &Checkpoint{
		Timestamp: time.Now(),
	}
...
// WithMolecule adds totem context to a checkpoint.
...
// WithHookedBead adds bannered bead context to a checkpoint.
...
func (cp *Checkpoint) Summary() string {
	...
	if cp.MoleculeID != "" {
		if cp.CurrentStep != "" {
			parts = append(parts, fmt.Sprintf("totem %s, step %s", cp.MoleculeID, cp.CurrentStep))
		} else {
			parts = append(parts, fmt.Sprintf("totem %s", cp.MoleculeID))
		}
	}

	if cp.HookedBead != "" {
		parts = append(parts, fmt.Sprintf("bannered: %s", cp.HookedBead))
	}
