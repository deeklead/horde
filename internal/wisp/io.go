...
// EnsureDir ensures the .relics directory exists in the given root.
func EnsureDir(root string) (string, error) {
	dir := filepath.Join(root, WispDir)
	if err := os.MkdirAll(dir, 0755); err != nil {
		return "", fmt.Errorf("create relics dir: %w", err)
	}
	return dir, nil
}

// WispPath returns the full path to a file in the relics directory.
