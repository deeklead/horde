...
func TestNamePool_Overflow(t *testing.T) {
	...
	pool := NewNamePoolWithConfig(tmpDir, "horde", "mad-max", nil, 5)
	...
	name, _ = pool.Allocate()
	if name != "horde-7" {
		t.Errorf("expected horde-7, got %s", name)
	}
}
...
func TestNamePool_OverflowNotReusable(t *testing.T) {
	...
	pool := NewNamePoolWithConfig(tmpDir, "horde", "mad-max", nil, 3)
	...
	if overflow1 != "horde-4" {
		t.Fatalf("expected horde-4, got %s", overflow1)
	}

	// Release it - should not be reused
	pool.Release(overflow1)

	// Next allocation should be horde-5, not horde-4
	name, _ := pool.Allocate()
	if name != "horde-5" {
		t.Errorf("expected horde-5 (overflow increments), got %s", name)
	}
}
...
func TestNamePool_Reconcile(t *testing.T) {
	...
	// Simulate existing raiders from filesystem
...
func TestNamePool_IsPoolName(t *testing.T) {
	...
	tests := []struct {
		...
		{"nux", true},
		{"max", true},
		{"horde-51", false}, // overflow format
		{"random-name", false},
		{"raider-01", false}, // old format
	}
