...
const (
	...
	// NOTE: This is a pool of NAMES, not raiders. Raiders are spawned fresh
	// for each task and nuked when done - there is no idle pool of raiders.
	// Only the name slots are reused when a raider is nuked and a new one spawned.
	DefaultPoolSize = 50

	// DefaultTheme is the default theme for new warbands.
	DefaultTheme = "mad-max"
)

// Built-in themes with themed raider names.
var BuiltinThemes = map[string][]string{
	"mad-max": {
		...
		"rally", "vuvalini", "rockryder", "wretched", "buzzard",
		"horde", "bullet-farmer", "citadel", "wasteland", "fury",
		"road-warrior", "interceptor", "blackfinger", "wraith", "witness",
		"chrome", "shiny", "mediocre", "fuel", "aqua-cola",
	},
	"minerals": {
...
// NamePool manages a bounded pool of reusable raider NAME SLOTS.
// IMPORTANT: This pools NAMES, not raiders. Raiders are spawned fresh for each
// task and nuked when done - there is no idle pool of raider instances waiting
// for work. When a raider is nuked, its name slot becomes available for the next
// freshly-spawned raider.
//
...
type NamePool struct {
	...
	// RigName is the warband this pool belongs to.
	...
	// Never persist - always discover from existing raider directories.
...
// NewNamePool creates a new name pool for a warband.
...
// Called when a raider is nuked - the name becomes available for new raiders.
// NOTE: This releases the NAME, not the raider. The raider is gone (nuked).
...
// MarkInUse marks a name as in use (for reconciling with existing raiders).
...
// Reconcile updates the pool state based on existing raider directories.
// This should be called on startup to sync pool state with reality.
func (p *NamePool) Reconcile(existingRaiders []string) {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	// Mark all existing raiders as in use
	for _, name := range existingRaiders {
		if p.isThemedName(name) {
			p.InUse[name] = true
		}
	}
