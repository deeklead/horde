...
func TestClassifyMessage(t *testing.T) {
	tests := []struct {
		...
		{"RAIDER_DONE nux", ProtoRaiderDone},
		{"RAIDER_DONE ace", ProtoRaiderDone},
		...
		{"🤝 HANDOFF: Scout context", ProtoHandoff},
...
func TestParseRaiderDone(t *testing.T) {
	subject := "RAIDER_DONE nux"
	body := `Exit: MERGED
Issue: gt-abc123
MR: gt-mr-xyz
Branch: feature-branch`

	payload, err := ParseRaiderDone(subject, body)
	if err != nil {
		t.Fatalf("ParseRaiderDone() error = %v", err)
	}

	if payload.RaiderName != "nux" {
		t.Errorf("RaiderName = %q, want %q", payload.RaiderName, "nux")
	}
...
func TestParseRaiderDone_MinimalBody(t *testing.T) {
	subject := "RAIDER_DONE ace"
	body := "Exit: DEFERRED"
	...
	if payload.RaiderName != "ace" {
		t.Errorf("RaiderName = %q, want %q", payload.RaiderName, "ace")
	}
...
func TestParseRaiderDone_InvalidSubject(t *testing.T) {
	_, err := ParseRaiderDone("Invalid subject", "body")
	if err == nil {
		t.Error("ParseRaiderDone() expected error for invalid subject")
	}
}

func TestParseHelp(t *testing.T) {
	subject := "HELP: Tests failing on CI"
	body := `Agent: horde/raiders/nux
Issue: gt-abc123
...
func TestCleanupWispLabels(t *testing.T) {
	...
	expected := []string{"cleanup", "raider:nux", "state:pending"}
