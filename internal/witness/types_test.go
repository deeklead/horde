...
func TestWitness_JSONMarshaling(t *testing.T) {
	...
	w := Witness{
		...
		PID:        12345,
		StartedAt:  &now,
		MonitoredRaiders: []string{"keeper", "valkyrie"},
...
func TestWitnessConfig_OmitEmpty(t *testing.T) {
	...
	requiredFields := []string{"max_workers", "muster_delay_ms", "auto_spawn"}
...
func TestWitness_OmitEmpty(t *testing.T) {
	w := Witness{
		RigName: "horde",
		State:   StateRunning,
		// PID, StartedAt, MonitoredRaiders, SpawnedIssues left empty/nil
	}
	...
	if _, exists := raw["monitored_raiders"]; exists {
		t.Error("Field 'monitored_raiders' should be omitted when nil/empty")
	}
...
func TestWitness_WithMonitoredRaiders(t *testing.T) {
	w := Witness{
		RigName:    "horde",
		State:      StateRunning,
		MonitoredRaiders: []string{"keeper", "valkyrie", "nux"},
	}
	...
	if len(unmarshaled.MonitoredRaiders) != 3 {
		t.Errorf("After round-trip: MonitoredRaiders length = %d, want 3", len(unmarshaled.MonitoredRaiders))
	}
}
