...
var (
	// RAIDER_DONE <name> - raider signaling work completion
	PatternRaiderDone = regexp.MustCompile(`^RAIDER_DONE\s+(\S+)`)

	// LIFECYCLE:Shutdown <name> - daemon-triggered raider shutdown
	PatternLifecycleShutdown = regexp.MustCompile(`^LIFECYCLE:Shutdown\s+(\S+)`)

	// HELP: <topic> - raider requesting intervention
	PatternHelp = regexp.MustCompile(`^HELP:\s+(.+)`)

	// MERGED <name> - forge confirms branch merged
	PatternMerged = regexp.MustCompile(`^MERGED\s+(\S+)`)

	// MERGE_FAILED <name> - forge reporting merge failure
	...
	// SWARM_START - warchief initiating batch work
...
const (
	ProtoRaiderDone       ProtocolType = "raider_done"
...
// RaiderDonePayload contains parsed data from a RAIDER_DONE message.
type RaiderDonePayload struct {
	RaiderName string
...
// ParseRaiderDone extracts payload from a RAIDER_DONE message.
...
//	Gate: <gate-id>
//	Branch: <branch>
func ParseRaiderDone(subject, body string) (*RaiderDonePayload, error) {
	matches := PatternRaiderDone.FindStringSubmatch(subject)
	if len(matches) < 2 {
		return nil, fmt.Errorf("invalid RAIDER_DONE subject: %s", subject)
	}

	payload := &RaiderDonePayload{
...
// Body format is JSON: {"swarm_id": "batch-123", "relics": ["bd-a", "bd-b"]}
...
func CleanupWispLabels(raiderName, state string) []string {
	return []string{
		"cleanup",
		fmt.Sprintf("raider:%s", raiderName),
...
func AssessHelpRequest(payload *HelpPayload) *HelpAssessment {
	...
	if strings.Contains(topic, "unclear") || strings.Contains(problem, "requirement") ||
		...
		assessment.CanHelp = false
		assessment.NeedsEscalation = true
		assessment.EscalationReason = "Requirements clarification needed from Warchief"
	}
