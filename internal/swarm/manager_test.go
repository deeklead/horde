...
func TestLoadSwarmNotFound(t *testing.T) {
	...
	// LoadSwarm for non-existent epic should fail (no relics available)
...
//	  Swarmable: YES
//
// 3. CREATE SWARM TOTEM ✅
//
//	bd swarm create gt-xxxxx
//	Expected: Creates totem with mol_type=swarm linked to epic
//
...
//	detecting completion and closing the swarm totem.
//	Manual close: rl close gt-xxxxx gt-yyyyy --reason "Swarm complete"
//
// KNOWN ISSUES:
//   - hd swarm status/land fail to find issues (filed as gt-594a4)
//   - rl swarm commands work correctly as the underlying implementation
//   - Auto-close requires Witness scout (not automatic in relics)
func TestSwarmE2ELifecycle(t *testing.T) {
	// This test documents the manual E2E testing protocol.
	// The actual test requires relics infrastructure and is run manually.
	// See the docstring above for the complete test procedure.
	t.Skip("E2E test requires relics infrastructure - see docstring for manual test protocol")
}
