...
// The swarm references a relics epic that tracks all swarm work.
type Swarm struct {
	// ID is the unique swarm identifier (matches relics epic ID).
	ID string `json:"id"`

	// RigName is the warband this swarm operates in.
	RigName string `json:"rig_name"`

	// EpicID is the relics epic tracking this swarm's work.
	...
	// Workers is the list of raider names assigned to this swarm.
...
// Each task maps to a relics issue and is assigned to a worker.
type SwarmTask struct {
	// IssueID is the relics issue ID for this task.
	IssueID string `json:"issue_id"`

	// Title is the task title (copied from relics issue).
	Title string `json:"title"`

	// Assignee is the raider name working on this task.
	Assignee string `json:"assignee,omitempty"`

	// Branch is the worker's branch name for this task.
	Branch string `json:"branch,omitempty"`

	// State mirrors the relics issue status.
