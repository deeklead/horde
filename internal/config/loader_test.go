...
func TestTownConfigRoundTrip(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "warchief", "encampment.json")
...
func TestRigsConfigRoundTrip(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "warchief", "warbands.json")
	...
	if len(loaded.Warbands) != 1 {
		t.Errorf("Warbands count = %d, want 1", len(loaded.Warbands))
	}

	warband, ok := loaded.Warbands["horde"]
	if !ok {
		t.Fatal("missing 'horde' warband")
	}
	if warband.RelicsConfig == nil || warband.RelicsConfig.Prefix != "hd-" {
		t.Errorf("RelicsConfig.Prefix = %v, want 'gt-'", warband.RelicsConfig)
	}
	if warband.LocalRepo != "/tmp/local-repo" {
		t.Errorf("LocalRepo = %q, want %q", warband.LocalRepo, "/tmp/local-repo")
	}
}
...
func TestValidationErrors(t *testing.T) {
	t.Parallel()
	// Missing name
	tc := &TownConfig{Type: "encampment", Version: 1}
...
func TestRigConfigRoundTrip(t *testing.T) {
	...
	original := NewRigConfig("horde", "git@github.com:test/horde.git")
	original.CreatedAt = time.Now().Truncate(time.Second)
	original.Relics = &RelicsConfig{Prefix: "hd-"}
	...
	if loaded.Type != "warband" {
		t.Errorf("Type = %q, want 'warband'", loaded.Type)
	}
	if loaded.Version != CurrentRigConfigVersion {
		t.Errorf("Version = %d, want %d", loaded.Version, CurrentRigConfigVersion)
	}
	if loaded.Name != "horde" {
		t.Errorf("Name = %q, want 'horde'", loaded.Name)
	}
	if loaded.GitURL != "git@github.com:test/horde.git" {
	...
	if loaded.Relics == nil || loaded.Relics.Prefix != "hd-" {
		t.Error("Relics.Prefix not preserved")
	}
}
...
func TestRigSettingsRoundTrip(t *testing.T) {
	...
	if loaded.Type != "warband-settings" {
		t.Errorf("Type = %q, want 'warband-settings'", loaded.Type)
	}
...
func TestWarchiefConfigRoundTrip(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "warchief", "config.json")

	original := NewWarchiefConfig()
	...
	if err := SaveWarchiefConfig(path, original); err != nil {
		t.Fatalf("SaveWarchiefConfig: %v", err)
	}

	loaded, err := LoadWarchiefConfig(path)
	if err != nil {
		t.Fatalf("LoadWarchiefConfig: %v", err)
	}

	if loaded.Type != "warchief-config" {
		t.Errorf("Type = %q, want 'warchief-config'", loaded.Type)
	}
	if loaded.Version != CurrentWarchiefConfigVersion {
		t.Errorf("Version = %d, want %d", loaded.Version, CurrentWarchiefConfigVersion)
	}
...
func TestLoadWarchiefConfigNotFound(t *testing.T) {
	t.Parallel()
	_, err := LoadWarchiefConfig("/nonexistent/path.json")
	if err == nil {
		t.Fatal("expected error for nonexistent file")
	}
}
...
func TestAccountsConfigRoundTrip(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "warchief", "accounts.json")
...
func TestMessagingConfigRoundTrip(t *testing.T) {
	...
	original.Lists["oncall"] = []string{"warchief/", "horde/witness"}
	original.Lists["cleanup"] = []string{"horde/witness", "shaman/"}
	original.Queues["work/horde"] = QueueConfig{
		Workers:   []string{"horde/raiders/*"},
		MaxClaims: 5,
	}
	original.Announces["alerts"] = AnnounceConfig{
		Readers:     []string{"@encampment"},
		RetainCount: 100,
	}
	original.NudgeChannels["workers"] = []string{"horde/raiders/*", "horde/clan/*"}
	...
	if q, ok := loaded.Queues["work/horde"]; !ok || q.MaxClaims != 5 {
	...
	if workers, ok := loaded.NudgeChannels["workers"]; !ok || len(workers) != 2 {
		t.Error("workers signal channel not preserved")
	}
	if witnesses, ok := loaded.NudgeChannels["witnesses"]; !ok || len(witnesses) != 1 {
		t.Error("witnesses signal channel not preserved")
	}
}
	...
	tests := []struct {
		...
		},
		{
			...
			config: &MessagingConfig{
				Version: 1,
				Announces: map[string]AnnounceConfig{
					"alerts": {Readers: []string{"@encampment"}, RetainCount: -1},
				},
			},
			wantErr: true,
		},
		{
			name: "valid config with signal channels",
			config: &MessagingConfig{
				Type:    "messaging",
				Version: 1,
				NudgeChannels: map[string][]string{
					"workers": {"horde/raiders/*", "horde/clan/*"},
				},
			},
			wantErr: false,
		},
		{
			name: "signal channel with no recipients",
			config: &MessagingConfig{
				Version: 1,
				NudgeChannels: map[string][]string{
					"empty": {},
...
func TestLoadOrCreateMessagingConfig(t *testing.T) {
	...
	path := filepath.Join(dir, "messaging.json")
	original := NewMessagingConfig()
	original.Lists["test"] = []string{"warchief/"}
	if err := SaveMessagingConfig(path, original); err != nil {
		t.Fatalf("SaveMessagingConfig: %v", err)
	}
...
func TestMessagingConfigPath(t *testing.T) {
	...
	expected := "/home/user/horde/config/messaging.json"
	if path != expected {
...
func TestRuntimeConfigBuildCommandWithPrompt(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			rc:     DefaultRuntimeConfig(),
			prompt: "hd rally",
			want:   `claude --dangerously-skip-permissions "hd rally"`,
		},
		{
...
func TestBuildAgentStartupCommand(t *testing.T) {
	// BuildAgentStartupCommand auto-detects encampment root from cwd when rigPath is empty.
	...
	// Test without warband config (uses defaults)
	// New signature: (role, warband, townRoot, rigPath, prompt)
	cmd := BuildAgentStartupCommand("witness", "horde", "", "", "")
...
func TestBuildRaiderStartupCommand(t *testing.T) {
	t.Parallel()
...
func TestResolveAgentConfigWithOverride(t *testing.T) {
	...
	// Encampment settings: default agent is gemini, plus a custom alias.
	...
	// Warband settings: prefer codex unless overridden.
	...
	t.Run("no override uses warband agent", func(t *testing.T) {
...
func TestBuildRaiderStartupCommandWithAgentOverride(t *testing.T) {
	t.Parallel()
	townRoot := t.TempDir()
	rigPath := filepath.Join(townRoot, "testrig")
	...
	// The warband settings file must exist for resolver calls that load it.
	if err := SaveRigSettings(RigSettingsPath(rigPath), NewRigSettings()); err != nil {
		t.Fatalf("SaveRigSettings: %v", err)
	}

	cmd, err := BuildRaiderStartupCommandWithAgentOverride("testrig", "toast", rigPath, "", "gemini")
	if err != nil {
...
func TestBuildAgentStartupCommandWithAgentOverride(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(townRoot, "warchief", "encampment.json"), []byte("{}"), 0600); err != nil {
		t.Fatalf("WriteFile encampment.json: %v", err)
	}
	...
	t.Run("empty override uses default agent", func(t *testing.T) {
		// New signature: (role, warband, townRoot, rigPath, prompt, agentOverride)
		cmd, err := BuildAgentStartupCommandWithAgentOverride("warchief", "", "", "", "", "")
		if err != nil {
	...
	t.Run("override switches agent", func(t *testing.T) {
		// New signature: (role, warband, townRoot, rigPath, prompt, agentOverride)
		cmd, err := BuildAgentStartupCommandWithAgentOverride("warchief", "", "", "", "", "codex")
		if err != nil {
			t.Fatalf("BuildAgentStartupCommandWithAgentOverride: %v", err)
		}
...
func TestBuildCrewStartupCommandWithAgentOverride(t *testing.T) {
	...
	cmd, err := BuildCrewStartupCommandWithAgentOverride("testrig", "max", rigPath, "hd rally", "gemini")
	if err != nil {
...
func TestBuildStartupCommand_UsesRigAgentWhenRigPathProvided(t *testing.T) {
	...
	if strings.Contains(cmd, "gemini --approval-mode yolo") {
		t.Fatalf("did not expect encampment default agent in command: %q", cmd)
	}
}
...
func TestBuildStartupCommand_UsesRoleAgentsFromTownSettings(t *testing.T) {
	...
	// Configure encampment settings with role_agents
	townSettings := NewTownSettings()
	townSettings.DefaultAgent = "claude"
	townSettings.RoleAgents = map[string]string{
		constants.RoleForge: "gemini",
	...
	// Create empty warband settings (no agent override)
	rigSettings := NewRigSettings()
	if err := SaveRigSettings(RigSettingsPath(rigPath), rigSettings); err != nil {
		t.Fatalf("SaveRigSettings: %v", err)
	}
...
func TestBuildStartupCommand_RigRoleAgentsOverridesTownRoleAgents(t *testing.T) {
	...
	// Encampment settings has witness = gemini
	townSettings := NewTownSettings()
	townSettings.DefaultAgent = "claude"
	townSettings.RoleAgents = map[string]string{
	...
	// Warband settings overrides witness to codex
	rigSettings := NewRigSettings()
	...
	if strings.Contains(cmd, "gemini") {
		t.Fatalf("did not expect encampment role_agents (gemini) in command: %q", cmd)
	}
}
...
func TestBuildAgentStartupCommand_UsesRoleAgents(t *testing.T) {
	...
	townSettings.DefaultAgent = "claude"
	townSettings.RoleAgents = map[string]string{
		constants.RoleForge: "codex",
	}
	if err := SaveTownSettings(TownSettingsPath(townRoot), townSettings); err != nil {
		t.Fatalf("SaveTownSettings: %v", err)
	}

	// Create empty warband settings
	rigSettings := NewRigSettings()
	if err := SaveRigSettings(RigSettingsPath(rigPath), rigSettings); err != nil {
		t.Fatalf("SaveRigSettings: %v", err)
	}
...
func TestResolveRoleAgentConfig_FallsBackOnInvalidAgent(t *testing.T) {
	...
	// Configure encampment settings with an invalid agent for forge
	townSettings := NewTownSettings()
	townSettings.DefaultAgent = "claude"
	townSettings.RoleAgents = map[string]string{
		constants.RoleForge: "nonexistent-agent-xyz", // Invalid agent
	}
	if err := SaveTownSettings(TownSettingsPath(townRoot), townSettings); err != nil {
		t.Fatalf("SaveTownSettings: %v", err)
	}
	...
	rc := ResolveRoleAgentConfig(constants.RoleForge, townRoot, rigPath)
	if rc.Command != "claude" {
...
func TestLoadRuntimeConfigFromSettings(t *testing.T) {
	t.Parallel()
	// Create temp warband with custom runtime config
	dir := t.TempDir()
...
func TestDaemonPatrolConfigRoundTrip(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "warchief", "daemon.json")
	...
	if custom, ok := loaded.Patrols["custom"]; !ok || custom.Agent != "custom-agent" {
		t.Error("custom scout not preserved")
	}
}
...
func TestDaemonPatrolConfigPath(t *testing.T) {
	...
	tests := []struct {
		...
		{"/home/user/gt", "/home/user/horde/warchief/daemon.json"},
		{"/var/lib/horde", "/var/lib/horde/warchief/daemon.json"},
		{"/tmp/test-workspace", "/tmp/test-workspace/warchief/daemon.json"},
		{"~/horde", "~/horde/warchief/daemon.json"},
	}
...
func TestEnsureDaemonPatrolConfig(t *testing.T) {
	...
	t.Run("creates config if missing", func(t *testing.T) {
		dir := t.TempDir()
		if err := os.MkdirAll(filepath.Join(dir, "warchief"), 0755); err != nil {
			t.Fatalf("creating warchief dir: %v", err)
		}
		...
		if len(loaded.Patrols) != 3 {
			t.Errorf("Patrols count = %d, want 3 (shaman, witness, forge)", len(loaded.Patrols))
		}
	})
...
func TestNewDaemonPatrolConfig(t *testing.T) {
	...
	if cfg.Type != "daemon-scout-config" {
		t.Errorf("Type = %q, want 'daemon-scout-config'", cfg.Type)
	}
	...
	for _, name := range []string{"shaman", "witness", "forge"} {
		scout, ok := cfg.Patrols[name]
		if !ok {
			t.Errorf("missing %s scout", name)
			continue
		}
		if !scout.Enabled {
			t.Errorf("%s scout should be enabled by default", name)
		}
		if scout.Agent != name {
			t.Errorf("%s scout Agent = %q, want %q", name, scout.Agent, name)
		}
	}
}
...
func TestSaveTownSettings(t *testing.T) {
	t.Parallel()
	t.Run("saves valid encampment settings", func(t *testing.T) {
		tmpDir := t.TempDir()
		...
		content := string(data)
		if !strings.Contains(content, `"type": "encampment-settings"`) {
...
func TestGetDefaultFormula(t *testing.T) {
	t.Parallel()
	t.Run("returns empty string for nonexistent warband", func(t *testing.T) {
	...
	t.Run("returns default ritual when configured", func(t *testing.T) {
		dir := t.TempDir()
		settings := NewRigSettings()
...
// warband-level agents first, then encampment-level agents, then built-ins.
func TestLookupAgentConfigWithRigSettings(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "warband-custom-agent",
			rigSettings: &RigSettings{
				Agent: "default-warband-agent",
				Agents: map[string]*RuntimeConfig{
					"warband-custom-agent": {
						Command: "custom-warband-cmd",
						Args:    []string{"--warband-flag"},
					},
				},
			},
			townSettings: &TownSettings{
				Agents: map[string]*RuntimeConfig{
					"encampment-custom-agent": {
						Command: "custom-encampment-cmd",
						Args:    []string{"--encampment-flag"},
					},
				},
			},
			expectedCommand: "custom-warband-cmd",
			expectedFrom:    "warband",
		},
		{
			name: "encampment-custom-agent",
			rigSettings: &RigSettings{
				Agents: map[string]*RuntimeConfig{
					"other-warband-agent": {
						Command: "other-warband-cmd",
					},
				},
			},
			townSettings: &TownSettings{
			...
			expectedCommand: "custom-encampment-cmd",
			expectedFrom:    "encampment",
		},
		{
			name:            "unknown-agent",
			rigSettings:     nil,
...
func TestResolveRoleAgentConfig(t *testing.T) {
	...
	// Create encampment settings with role-specific agents
	townSettings := NewTownSettings()
	townSettings.DefaultAgent = "claude"
	townSettings.RoleAgents = map[string]string{
		"warchief":   "claude", // warchief uses default claude
		"witness": "gemini", // witness uses gemini
		"raider": "codex",  // raiders use codex
	}
	townSettings.Agents = map[string]*RuntimeConfig{
		"claude-haiku": {
			Command: "claude",
	...
	// Create warband settings that override some roles
	rigSettings := NewRigSettings()
	rigSettings.Agent = "gemini" // default for this warband
	rigSettings.RoleAgents = map[string]string{
	...
	t.Run("warband RoleAgents overrides encampment RoleAgents", func(t *testing.T) {
		rc := ResolveRoleAgentConfig("witness", townRoot, rigPath)
		// Should get claude-haiku from warband's RoleAgents
		if rc.Command != "claude" {
			t.Errorf("Command = %q, want %q", rc.Command, "claude")
		}
		cmd := rc.BuildCommand()
	...
	t.Run("encampment RoleAgents used when warband has no override", func(t *testing.T) {
		rc := ResolveRoleAgentConfig("raider", townRoot, rigPath)
		// Should get codex from encampment's RoleAgents (warband doesn't override raider)
		if rc.Command != "codex" {
	...
	t.Run("falls back to default agent when role not in RoleAgents", func(t *testing.T) {
		rc := ResolveRoleAgentConfig("clan", townRoot, rigPath)
		// clan is not in any RoleAgents, should use warband's default agent (gemini)
		if rc.Command != "gemini" {
			t.Errorf("Command = %q, want %q", rc.Command, "gemini")
		}
	})

	t.Run("encampment-level role (no rigPath) uses encampment RoleAgents", func(t *testing.T) {
		rc := ResolveRoleAgentConfig("warchief", townRoot, "")
		// warchief is in encampment's RoleAgents
		if rc.Command != "claude" {
			t.Errorf("Command = %q, want %q", rc.Command, "claude")
		}
	})
...
func TestResolveRoleAgentName(t *testing.T) {
	...
	t.Run("warband role-specific agent", func(t *testing.T) {
	...
	t.Run("encampment role-specific agent", func(t *testing.T) {
		name, isRoleSpecific := ResolveRoleAgentName("raider", townRoot, rigPath)
		if name != "codex" {
	...
	t.Run("falls back to warband default agent", func(t *testing.T) {
		name, isRoleSpecific := ResolveRoleAgentName("clan", townRoot, rigPath)
		if name != "amp" {
	...
	t.Run("falls back to encampment default agent when no warband path", func(t *testing.T) {
		name, isRoleSpecific := ResolveRoleAgentName("forge", townRoot, "")
		if name != "claude" {
...
func TestRoleAgentsRoundTrip(t *testing.T) {
	...
	rigSettingsPath := filepath.Join(dir, "warband", "settings", "config.json")

	// Test TownSettings with RoleAgents
	t.Run("encampment settings with role_agents", func(t *testing.T) {
		original := NewTownSettings()
		original.RoleAgents = map[string]string{
			"warchief":   "claude-opus",
			"witness": "claude-haiku",
			"raider": "claude-sonnet",
		}
		...
		if loaded.RoleAgents["warchief"] != "claude-opus" {
			t.Errorf("RoleAgents[warchief] = %q, want %q", loaded.RoleAgents["warchief"], "claude-opus")
		}
		if loaded.RoleAgents["witness"] != "claude-haiku" {
			t.Errorf("RoleAgents[witness] = %q, want %q", loaded.RoleAgents["witness"], "claude-haiku")
		}
		if loaded.RoleAgents["raider"] != "claude-sonnet" {
			t.Errorf("RoleAgents[raider] = %q, want %q", loaded.RoleAgents["raider"], "claude-sonnet")
		}
	})

	// Test RigSettings with RoleAgents
	t.Run("warband settings with role_agents", func(t *testing.T) {
		original := NewRigSettings()
		original.RoleAgents = map[string]string{
			"witness": "gemini",
			"clan":    "codex",
		...
		if loaded.RoleAgents["clan"] != "codex" {
			t.Errorf("RoleAgents[clan] = %q, want %q", loaded.RoleAgents["clan"], "codex")
		}
	})
}
...
func TestEscalationConfigRoundTrip(t *testing.T) {
	...
	original := &EscalationConfig{
		...
		Routes: map[string][]string{
			SeverityLow:      {"bead"},
			SeverityMedium:   {"bead", "drums:warchief"},
			SeverityHigh:     {"bead", "drums:warchief", "email:human"},
			SeverityCritical: {"bead", "drums:warchief", "email:human", "sms:human"},
		},
...
func TestEscalationConfigGetRouteForSeverity(t *testing.T) {
	...
	tests := []struct {
		...
		{SeverityMedium, []string{"bead", "drums:warchief"}},
		{SeverityHigh, []string{"bead", "drums:warchief"}},     // fallback for missing
		{SeverityCritical, []string{"bead", "drums:warchief"}}, // fallback for missing
	}
...
func TestEscalationConfigPath(t *testing.T) {
	...
	expected := "/home/user/horde/settings/escalation.json"
	if path != expected {
...
func TestBuildStartupCommandWithAgentOverride_PriorityOverRoleAgents(t *testing.T) {
	...
	// Configure encampment settings with role_agents: forge = codex
	townSettings := NewTownSettings()
	townSettings.DefaultAgent = "claude"
	townSettings.RoleAgents = map[string]string{
		constants.RoleForge: "codex",
