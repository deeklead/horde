...
// TestDefaultRigAgentRegistryPath verifies that the default warband agent registry path is constructed correctly.
func TestDefaultRigAgentRegistryPath(t *testing.T) {
	...
	tests := []struct {
		...
		{"/Users/alice/horde/myproject", "/Users/alice/horde/myproject/settings/agents.json"},
		{"/tmp/my-warband", "/tmp/my-warband/settings/agents.json"},
...
// TestLoadRigAgentRegistry verifies that warband-level agent registry is loaded correctly.
func TestLoadRigAgentRegistry(t *testing.T) {
  ...
  "agents": {
	...
	t.Run("load and merge", func(t *testing.T) {
		...
		if info == nil {
			t.Fatal("expected opencode agent to be available after loading warband registry")
		}
	...
	t.Run("file not found", func(t *testing.T) {
		nonExistentPath := filepath.Join(tmpDir, "other-warband", "settings", "agents.json")
	...
	t.Run("invalid JSON", func(t *testing.T) {
		invalidRegistryPath := filepath.Join(tmpDir, "bad-warband", "settings", "agents.json")
		badConfigDir := filepath.Join(tmpDir, "bad-warband", "settings")
		if err := os.MkdirAll(badConfigDir, 0755); err != nil {
			t.Fatalf("failed to create bad-warband settings dir: %v", err)
		}
