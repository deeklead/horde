...
// Located alongside other encampment settings.
func DefaultAgentRegistryPath(townRoot string) string {
	return filepath.Join(townRoot, "settings", "agents.json")
}

// DefaultRigAgentRegistryPath returns the default path for warband-level agent registry.
// Located in <warband>/settings/agents.json.
func DefaultRigAgentRegistryPath(rigPath string) string {
	return filepath.Join(rigPath, "settings", "agents.json")
}

// RigAgentRegistryPath returns the path for warband-level agent registry.
...
// LoadRigAgentRegistry loads agent definitions from a warband-level JSON file and merges with built-ins.
// This function works similarly to LoadAgentRegistry but for warband-level configurations.
