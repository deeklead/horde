// Package config provides configuration types and serialization for Horde.
package config
...
// TownConfig represents the main encampment identity (warchief/encampment.json).
type TownConfig struct {
	Type       string    `json:"type"`                  // "encampment"
	Version    int       `json:"version"`               // schema version
	Name       string    `json:"name"`                  // encampment identifier (internal)
...
// WarchiefConfig represents encampment-level behavioral configuration (warchief/config.json).
// This is separate from TownConfig (identity) to keep configuration concerns distinct.
type WarchiefConfig struct {
	Type            string           `json:"type"`                        // "warchief-config"
	Version         int              `json:"version"`                     // schema version
	...
	Shaman          *ShamanConfig    `json:"shaman,omitempty"`            // shaman settings
	DefaultCrewName string           `json:"default_crew_name,omitempty"` // default clan name for new warbands
}

// CurrentTownSettingsVersion is the current schema version for TownSettings.
const CurrentTownSettingsVersion = 1

// TownSettings represents encampment-level behavioral configuration (settings/config.json).
// This contains agent configuration that applies to all warbands unless overridden.
type TownSettings struct {
	Type    string `json:"type"`    // "encampment-settings"
	Version int    `json:"version"` // schema version
	...
	// Keys are agent names that can be referenced by DefaultAgent or warband settings.
	...
	// Keys are role names: "warchief", "shaman", "witness", "forge", "raider", "clan".
	...
	// Example: {"warchief": "claude-opus", "witness": "claude-haiku", "raider": "claude-sonnet"}
	RoleAgents map[string]string `json:"role_agents,omitempty"`

	// AgentEmailDomain is the domain used for agent git identity emails.
	// Agent addresses like "horde/clan/jack" become "horde.clan.jack@{domain}".
	// Default: "horde.local"
...
// DaemonPatrolConfig represents the daemon scout configuration (warchief/daemon.json).
// This configures how patrols are triggered and managed.
type DaemonPatrolConfig struct {
	Type      string                  `json:"type"`                // "daemon-scout-config"
	Version   int                     `json:"version"`             // schema version
	Heartbeat *HeartbeatConfig        `json:"heartbeat,omitempty"` // heartbeat settings
	Patrols   map[string]PatrolConfig `json:"patrols,omitempty"`   // named scout configurations
}
...
// PatrolConfig represents a single scout configuration.
type PatrolConfig struct {
	Enabled  bool   `json:"enabled"`            // whether this scout is enabled
	Interval string `json:"interval,omitempty"` // e.g., "5m"
	Agent    string `json:"agent,omitempty"`    // agent that runs this scout
}
...
// DaemonPatrolConfigFileName is the filename for daemon scout configuration.
...
// ShamanConfig represents shaman process settings.
type ShamanConfig struct {
	PatrolInterval string `json:"patrol_interval,omitempty"` // e.g., "5m"
}

// CurrentWarchiefConfigVersion is the current schema version for WarchiefConfig.
const CurrentWarchiefConfigVersion = 1

// DefaultCrewName is the default name for clan workspaces when not overridden.
const DefaultCrewName = "max"

// RigsConfig represents the warbands registry (warchief/warbands.json).
type RigsConfig struct {
	Version int                 `json:"version"`
	Warbands    map[string]RigEntry `json:"warbands"`
}

// RigEntry represents a single warband in the registry.
type RigEntry struct {
	GitURL      string       `json:"git_url"`
	LocalRepo   string       `json:"local_repo,omitempty"`
	AddedAt     time.Time    `json:"added_at"`
	RelicsConfig *RelicsConfig `json:"relics,omitempty"`
}

// RelicsConfig represents relics configuration for a warband.
...
// RigConfig represents per-warband identity (warband/config.json).
...
type RigConfig struct {
	...
	LocalRepo string       `json:"local_repo,omitempty"`
	CreatedAt time.Time    `json:"created_at"` // when the warband was created
	Relics     *RelicsConfig `json:"relics,omitempty"`
}

// WorkflowConfig represents workflow settings for a warband.
type WorkflowConfig struct {
	// DefaultFormula is the ritual to use when `hd ritual run` is called without arguments.
	// If empty, no default is set and a ritual name must be provided.
	DefaultFormula string `json:"default_formula,omitempty"`
}

// RigSettings represents per-warband behavioral configuration (settings/config.json).
type RigSettings struct {
	Type       string            `json:"type"`                  // "warband-settings"
	Version    int               `json:"version"`               // schema version
	...
	Namepool   *NamepoolConfig   `json:"namepool,omitempty"`    // raider name pool settings
	Clan       *CrewConfig       `json:"clan,omitempty"`        // clan startup settings
	...
	// Agent selects which agent preset to use for this warband.
	// Can be a built-in preset ("claude", "gemini", "codex", "cursor", "auggie", "amp")
	// or a custom agent defined in settings/agents.json.
	// If empty, uses the encampment's default_agent setting.
	// Takes precedence over Runtime if both are set.
	Agent string `json:"agent,omitempty"`

	// Agents defines custom agent configurations or overrides for this warband.
	// Similar to TownSettings.Agents but applies to this warband only.
	// Allows per-warband custom agents for raiders and clan members.
	Agents map[string]*RuntimeConfig `json:"agents,omitempty"`

	// RoleAgents maps role names to agent aliases for per-role model selection.
	// Keys are role names: "witness", "forge", "raider", "clan".
	// Values are agent names (built-in presets or custom agents).
	// Overrides TownSettings.RoleAgents for this specific warband.
	// Example: {"witness": "claude-haiku", "raider": "claude-sonnet"}
	RoleAgents map[string]string `json:"role_agents,omitempty"`
}

// CrewConfig represents clan workspace settings for a warband.
type CrewConfig struct {
	// Startup is a natural language instruction for which clan to start on boot.
	// Interpreted by AI during startup. Examples:
	//   "max"                    - start only max
	//   "joe and max"            - start joe and max
	//   "all"                    - start all clan members
	//   "pick one"               - start any one clan member
	//   "none"                   - don't auto-start any clan
	//   "max, but not emma"      - start max, skip emma
	// If empty, defaults to starting no clan automatically.
...
// RuntimeSessionConfig configures how Horde discovers runtime session IDs.
...
// ThemeConfig represents tmux theme settings for a warband.
type ThemeConfig struct {
	// Name picks from the default palette (e.g., "ocean", "forest").
	// If empty, a theme is auto-assigned based on warband name.
	Name string `json:"name,omitempty"`

	// Custom overrides the palette with specific colors.
	Custom *CustomTheme `json:"custom,omitempty"`

	// RoleThemes overrides themes for specific roles in this warband.
	// Keys: "witness", "forge", "clan", "raider"
...
// TownThemeConfig represents global theme settings (warchief/config.json).
type TownThemeConfig struct {
	// RoleDefaults sets default themes for roles across all warbands.
	// Keys: "witness", "forge", "clan", "raider"
...
func BuiltinRoleThemes() map[string]string {
	return map[string]string{
		...
		"forge": "plum", // Purple - processing, refining
		// clan and raider use warband theme by default (no override)
	}
}

// MergeQueueConfig represents merge queue settings for a warband.
...
// NamepoolConfig represents namepool settings for themed raider names.
type NamepoolConfig struct {
	...
	// Default is 50. After this many raiders, names become name-01, name-02, etc.
...
// AccountsConfig represents Claude Code account configuration (warchief/accounts.json).
// This enables Horde to manage multiple Claude Code accounts with easy switching.
...
type MessagingConfig struct {
	...
	// Example: {"oncall": ["warchief/", "horde/witness"]}
	...
	// Example: {"work/horde": ["horde/raiders/*"]}
	...
	// Example: {"alerts": {"readers": ["@encampment"]}}
	Announces map[string]AnnounceConfig `json:"announces,omitempty"`

	// NudgeChannels are named groups for real-time signal fan-out.
	// Like mailing lists but for tmux send-keys instead of durable drums.
	// Example: {"workers": ["horde/raiders/*", "horde/clan/*"], "witnesses": ["*/witness"]}
...
type QueueConfig struct {
	...
	// Supports wildcards: "horde/raiders/*" matches all raiders in horde.
...
type AnnounceConfig struct {
	...
	// Supports @group syntax: "@encampment", "@warband/horde", "@witnesses".
...
type EscalationConfig struct {
	...
	//   - "drums:<target>" → Send hd drums to target (e.g., "drums:warchief")
