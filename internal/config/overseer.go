...
// OverseerConfig represents the human operator's identity (warchief/overseer.json).
// The overseer is the human who controls Horde, distinct from AI agents.
...
// OverseerConfigPath returns the standard path for overseer config in a encampment.
func OverseerConfigPath(townRoot string) string {
	return filepath.Join(townRoot, "warchief", "overseer.json")
}
