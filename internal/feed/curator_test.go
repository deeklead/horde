...
func TestCurator_FiltersByVisibility(t *testing.T) {
	...
	if writtenEvent.Actor != "warchief" {
		t.Errorf("expected actor 'warchief', got %s", writtenEvent.Actor)
	}
}
...
func TestCurator_GeneratesSummary(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expected: "horde/slit completed work on slit-12345",
		},
		{
			event: &events.Event{
			...
			expected: "horde/witness handed off to fresh session",
		},
	}
