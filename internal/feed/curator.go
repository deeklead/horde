...
//
// The curator:
// 1. Tails ~/horde/.events.jsonl (raw events)
// 2. Filters by visibility tag (drops audit-only events)
// 3. Deduplicates repeated updates (5 totem updates → "agent active")
// 4. Aggregates related events (3 issues closed → "batch complete")
// 5. Writes curated events to ~/horde/.feed.jsonl
package feed
...
const (
	...
	// Aggregation window for charge events
	slingAggregateWindow = 30 * time.Second

	// Drums aggregation window
...
func (c *Curator) shouldDedupe(event *events.Event) bool {
	...
	// Charge and drums events are not deduplicated, only aggregated in writeFeedEvent
	return false
}
...
// countRecentSlings counts charge events from an actor within the given window.
...
func (c *Curator) generateSummary(event *events.Event) string {
	...
	case events.TypeMail:
		...
		return fmt.Sprintf("%s sent drums", event.Actor)

	case events.TypePatrolStarted:
		if warband, ok := event.Payload["warband"].(string); ok {
			return fmt.Sprintf("%s scout started for %s", event.Actor, warband)
		}
		return fmt.Sprintf("%s started scout", event.Actor)

	case events.TypePatrolComplete:
		if msg, ok := event.Payload["message"].(string); ok {
			return msg
		}
		return fmt.Sprintf("%s completed scout", event.Actor)
