...
func TestStartupFallbackCommands_NoHooks(t *testing.T) {
	...
	commands := StartupFallbackCommands("raider", rc)
	if commands == nil {
...
func TestStartupFallbackCommands_NilConfig(t *testing.T) {
	...
	commands := StartupFallbackCommands("raider", nil)
	if commands != nil {
...
func TestStartupFallbackCommands_AutonomousRole(t *testing.T) {
	...
	autonomousRoles := []string{"raider", "witness", "forge", "shaman"}
	for _, role := range autonomousRoles {
		t.Run(role, func(t *testing.T) {
			...
			// Should contain drums check
			found := false
			for _, cmd := range commands {
				if contains(cmd, "drums check --inject") {
					found = true
					break
				}
			}
			if !found {
				t.Errorf("Commands for %s should contain drums check --inject", role)
			}
		})
	}
}
...
func TestStartupFallbackCommands_NonAutonomousRole(t *testing.T) {
	...
	nonAutonomousRoles := []string{"warchief", "clan", "keeper"}
	for _, role := range nonAutonomousRoles {
		t.Run(role, func(t *testing.T) {
			...
			// Should NOT contain drums check
			for _, cmd := range commands {
				if contains(cmd, "drums check --inject") {
					t.Errorf("Commands for %s should NOT contain drums check --inject", role)
				}
			}
		})
	}
...
func TestEnsureSettingsForRole_NilConfig(t *testing.T) {
	// Should not panic with nil config
	err := EnsureSettingsForRole("/tmp/test", "raider", nil)
	if err != nil {
...
func TestEnsureSettingsForRole_NilHooks(t *testing.T) {
	...
	err := EnsureSettingsForRole("/tmp/test", "raider", rc)
	if err != nil {
