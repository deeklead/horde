...
// Common errors
var (
	ErrNotRunning     = errors.New("forge not running")
	ErrAlreadyRunning = errors.New("forge already running")
	ErrNoQueue        = errors.New("no items in queue")
)

// Manager handles forge lifecycle and queue operations.
type Manager struct {
...
// NewManager creates a new forge manager for a warband.
...
// stateFile returns the path to the forge state file.
func (m *Manager) stateFile() string {
	return filepath.Join(m.warband.Path, ".runtime", "forge.json")
}
...
func (m *Manager) loadState() (*Forge, error) {
...
// saveState persists forge state to disk using atomic write.
func (m *Manager) saveState(ref *Forge) error {
...
// The agentOverride parameter allows specifying an agent alias to use instead of the encampment default.
func (m *Manager) Start(foreground bool, agentOverride string) error {
	...
	if foreground {
		...
		agentCfg := config.ResolveRoleAgentConfig(constants.RoleForge, townRoot, m.warband.Path)
	...
	// Background mode: muster a Claude agent in a tmux session
	// The Claude agent handles MR processing using git commands and relics

	// Working directory is the forge worktree (shares .git with warchief/raiders)
	forgeRigDir := filepath.Join(m.warband.Path, "forge", "warband")
	if _, err := os.Stat(forgeRigDir); os.IsNotExist(err) {
		// Fall back to warchief/warband (legacy architecture) - ensures we use project git, not encampment git.
		// Using warband.Path directly would find encampment's .git with warband-named remotes instead of "origin".
		forgeRigDir = filepath.Join(m.warband.Path, "warchief", "warband")
	}

	// Ensure runtime settings exist in forge/ (not forge/warband/) so we don't
	// write into the source repo. Runtime walks up the tree to find settings.
	forgeParentDir := filepath.Join(m.warband.Path, "forge")
	runtimeConfig := config.LoadRuntimeConfig(m.warband.Path)
	if err := runtime.EnsureSettingsForRole(forgeParentDir, "forge", runtimeConfig); err != nil {
	...
	if agentOverride != "" {
		var err error
		command, err = config.BuildAgentStartupCommandWithAgentOverride("forge", m.warband.Name, townRoot, m.warband.Path, "", agentOverride)
		if err != nil {
			return fmt.Errorf("building startup command with agent override: %w", err)
		}
	} else {
		command = config.BuildAgentStartupCommand("forge", m.warband.Name, townRoot, m.warband.Path, "")
	}

	// Create session with command directly to avoid send-keys race condition.
	// See: https://github.com/anthropics/horde/issues/280
	if err := t.NewSessionWithCommand(sessionID, forgeRigDir, command); err != nil {
	...
	_ = t.ConfigureHordeSession(sessionID, theme, m.warband.Name, "forge", "forge")
	...
	_ = runtime.RunStartupFallback(t, sessionID, "forge", runtimeConfig)

	// Inject startup signal for predecessor discovery via /resume
	address := fmt.Sprintf("%s/forge", m.warband.Name)
	...
	_ = t.SignalSession(sessionID, session.PropulsionNudgeForRole("forge", forgeRigDir)) // Non-fatal

	return nil
}

// Stop stops the forge.
...
// Uses relics merge-request issues as the source of truth (not git branches).
func (m *Manager) Queue() ([]QueueItem, error) {
	// Query relics for open merge-request type issues
	// RelicsPath() returns the git-synced relics location
	b := relics.New(m.warband.RelicsPath())
	...
	if err != nil {
		return nil, fmt.Errorf("querying merge queue from relics: %w", err)
	}
...
func (m *Manager) calculateIssueScore(issue *relics.Issue, now time.Time) float64 {
	...
	if fields != nil {
		...
		// Parse raid created at if available
		if fields.RaidCreatedAt != "" {
			if raidTime := parseTime(fields.RaidCreatedAt); !raidTime.IsZero() {
				input.RaidCreatedAt = &raidTime
			}
		}
	}
...
// issueToMR converts a relics issue to a MergeRequest.
func (m *Manager) issueToMR(issue *relics.Issue) *MergeRequest {
	if issue == nil {
		return nil
	}

	// Get configured default branch for this warband
	defaultBranch := m.warband.DefaultBranch()
	...
	// Default target to warband's default branch if not specified
...
// The Forge agent (Claude) handles all merge processing.
// See: ZFC #5 - Move merge/conflict decisions from Go to Forge agent
func (m *Manager) run(_ *Forge) error { // ref unused: deprecated function
	_, _ = fmt.Fprintln(m.output, "")
	_, _ = fmt.Fprintln(m.output, "╔══════════════════════════════════════════════════════════════╗")
	_, _ = fmt.Fprintln(m.output, "║  Foreground mode is deprecated.                              ║")
	_, _ = fmt.Fprintln(m.output, "║                                                              ║")
	_, _ = fmt.Fprintln(m.output, "║  The Forge agent (Claude) handles all merge decisions.   ║")
	_, _ = fmt.Fprintln(m.output, "║  Use 'hd forge start' to run in background mode.         ║")
	_, _ = fmt.Fprintln(m.output, "╚══════════════════════════════════════════════════════════════╝")
	_, _ = fmt.Fprintln(m.output, "")
	return nil
}
...
// ProcessMR is deprecated - the Forge agent now handles all merge processing.
//
// ZFC #5: Move merge/conflict decisions from Go to Forge agent
//
// The agent runs git commands directly and makes decisions based on output:
//   - Agent attempts merge: git checkout -b temp origin/raider/<worker>
//   - Agent detects conflict and decides: retry, notify raider, escalate
...
//
// Deprecated: Use the Forge agent (Claude) for merge processing.
func (m *Manager) ProcessMR(mr *MergeRequest) MergeResult {
	return MergeResult{
		Error: "ProcessMR is deprecated - the Forge agent handles merge processing (ZFC #5)",
	}
}
...
func (m *Manager) completeMR(mr *MergeRequest, closeReason CloseReason, errMsg string) {
	...
	now := time.Now()
	actor := fmt.Sprintf("%s/forge", m.warband.Name)
...
// Deprecated: The Forge agent runs tests directly via shell commands (ZFC #5).
...
func (m *Manager) getMergeConfig() MergeConfig {
	...
	settingsPath := filepath.Join(m.warband.Path, "settings", "config.json")
...
// Deprecated: The Forge agent decides retry strategy (ZFC #5).
...
// notifyWorkerConflict sends a conflict notification to a raider.
func (m *Manager) notifyWorkerConflict(mr *MergeRequest) {
	router := drums.NewRouter(m.workDir)
	msg := &drums.Message{
		From:    fmt.Sprintf("%s/forge", m.warband.Name),
		To:      fmt.Sprintf("%s/%s", m.warband.Name, mr.Worker),
...
Then the Forge will retry the merge.`,
...
// notifyWorkerMerged sends a success notification to a raider.
...
func (m *Manager) FindMR(idOrBranch string) (*MergeRequest, error) {
	...
	for _, item := range queue {
		...
		// Match by branch name (with or without raider/ prefix)
		if item.MR.Branch == idOrBranch {
			return item.MR, nil
		}
		if constants.BranchRaiderPrefix+idOrBranch == item.MR.Branch {
			return item.MR, nil
		}
...
// The processNow parameter is deprecated - the Forge agent handles processing.
// Clearing the error is sufficient; the agent will pick up the MR in its next scout cycle.
func (m *Manager) Retry(id string, processNow bool) error {
	...
	// The Forge agent handles merge processing.
	// It will pick up this MR in its next scout cycle.
	if processNow {
		_, _ = fmt.Fprintln(m.output, "Note: --now is deprecated. The Forge agent will process this MR in its next scout cycle.")
	}
...
// notifyWorkerRejected sends a rejection notification to a raider.
...
// findTownRoot walks up directories to find the encampment root.
func findTownRoot(startPath string) string {
	path := startPath
	for {
		// Check for warchief/ subdirectory (indicates encampment root)
		if _, err := os.Stat(filepath.Join(path, "warchief")); err == nil {
			return path
		}
