...
type MRInfo struct {
	...
	RaidCreatedAt *time.Time // Raid creation time
...
type Engineer struct {
	...
	router  *drums.Router // Drums router for sending protocol messages

	// stopCh is used for graceful shutdown
	stopCh chan struct{}
}

// NewEngineer creates a new Engineer for the given warband.
func NewEngineer(r *warband.Warband) *Engineer {
	cfg := DefaultMergeQueueConfig()
	// Override target branch with warband's configured default branch
	cfg.TargetBranch = r.DefaultBranch()

	// Determine the git working directory for forge operations.
	// Prefer forge/warband worktree, fall back to warchief/warband (legacy architecture).
	// Using warband.Path directly would find encampment's .git with warband-named remotes instead of "origin".
	gitDir := filepath.Join(r.Path, "forge", "warband")
	if _, err := os.Stat(gitDir); os.IsNotExist(err) {
		gitDir = filepath.Join(r.Path, "warchief", "warband")
	}

	return &Engineer{
		warband:     r,
		relics:   relics.New(r.Path),
		git:     git.NewGit(gitDir),
		config:  cfg,
		workDir: gitDir,
		output:  os.Stdout,
		router:  drums.NewRouter(r.Path),
...
// LoadConfig loads merge queue configuration from the warband's config.json.
func (e *Engineer) LoadConfig() error {
	configPath := filepath.Join(e.warband.Path, "config.json")
...
// ProcessMR processes a single merge request from a relics issue.
func (e *Engineer) ProcessMR(ctx context.Context, mr *relics.Issue) ProcessResult {
	// Parse MR fields from description
	mrFields := relics.ParseMRFields(mr)
	if mrFields == nil {
...
func (e *Engineer) doMerge(ctx context.Context, branch, target, sourceIssue string) ProcessResult {
	// Step 1: Verify source branch exists locally (shared .repo.git with raiders)
...
func (e *Engineer) runTests(ctx context.Context) ProcessResult {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		// Note: TestCommand comes from warband's config.json (trusted infrastructure config),
		// not from PR branches. Shell execution is intentional for flexibility (pipes, etc).
		cmd := exec.CommandContext(ctx, "sh", "-c", e.config.TestCommand) //nolint:gosec // G204: TestCommand is from trusted warband config
		cmd.Dir = e.workDir
...
// 5. Log success
func (e *Engineer) handleSuccess(mr *relics.Issue, result ProcessResult) {
	// Parse MR fields from description
	mrFields := relics.ParseMRFields(mr)
	if mrFields == nil {
		mrFields = &relics.MRFields{}
	}

	// 1. Update MR with merge_commit SHA
	mrFields.MergeCommit = result.MergeCommit
	mrFields.CloseReason = "merged"
	newDesc := relics.SetMRFields(mr, mrFields)
	if err := e.relics.Update(mr.ID, relics.UpdateOptions{Description: &newDesc}); err != nil {
		_, _ = fmt.Fprintf(e.output, "[Engineer] Warning: failed to update MR %s with merge commit: %v\n", mr.ID, err)
	}

	// 2. Close MR with reason 'merged'
	if err := e.relics.CloseWithReason("merged", mr.ID); err != nil {
	...
	if mrFields.SourceIssue != "" {
		...
		if err := e.relics.CloseWithReason(closeReason, mrFields.SourceIssue); err != nil {
	...
	if mrFields.AgentBead != "" {
		if err := e.relics.UpdateAgentActiveMR(mrFields.AgentBead, ""); err != nil {
	...
	// Since the self-cleaning model (Jan 10), raiders push to origin before hd done,
...
func (e *Engineer) handleFailure(mr *relics.Issue, result ProcessResult) {
	// Reopen the MR (back to open status for rework)
	open := "open"
	if err := e.relics.Update(mr.ID, relics.UpdateOptions{Status: &open}); err != nil {
...
func (e *Engineer) HandleMRInfoSuccess(mr *MRInfo, result ProcessResult) {
	...
	holder := e.warband.Name + "/forge"
	if err := e.relics.MergeSlotRelease(holder); err != nil {
	...
	if mr.SourceIssue != "" {
		closeReason := fmt.Sprintf("Merged in %s", mr.ID)
		if err := e.relics.CloseWithReason(closeReason, mr.SourceIssue); err != nil {
			_, _ = fmt.Fprintf(e.output, "[Engineer] Warning: failed to close source issue %s: %v\n", mr.SourceIssue, err)
		} else {
			_, _ = fmt.Fprintf(e.output, "[Engineer] Closed source issue: %s\n", mr.SourceIssue)
		}
	...
	if mr.AgentBead != "" {
		if err := e.relics.UpdateAgentActiveMR(mr.AgentBead, ""); err != nil {
			_, _ = fmt.Fprintf(e.output, "[Engineer] Warning: failed to clear agent bead %s active_mr: %v\n", mr.AgentBead, err)
		}
	}
...
func (e *Engineer) HandleMRInfoFailure(mr *MRInfo, result ProcessResult) {
	// Notify Witness of the failure so raider can be alerted
	...
	msg := protocol.NewMergeFailedMessage(e.warband.Name, mr.Worker, mr.Branch, mr.SourceIssue, mr.Target, failureType, result.Error)
	...
	if result.Conflict {
		...
		if err != nil {
			...
			// Block the MR on the conflict resolution task using relics dependency
			// When the task closes, the MR unblocks and re-enters the ready queue
			if err := e.relics.AddDependency(mr.ID, taskID); err != nil {
...
// This task will be picked up by rl ready and can be charged to a fresh raider (spawned on demand).
...
// Before creating a conflict resolution task, we acquire the merge-slot for this warband.
// This serializes conflict resolution - only one raider can resolve conflicts at a time.
...
func (e *Engineer) createConflictResolutionTaskForMR(mr *MRInfo, _ ProcessResult) (string, error) { // result unused but kept for future merge diagnostics
	...
	slotID, err := e.relics.MergeSlotEnsureExists()
	if err != nil {
		...
		holder := e.warband.Name + "/forge"
		status, err := e.relics.MergeSlotAcquire(holder, false)
		if err != nil {
	...
	if mr.SourceIssue != "" {
		if sourceIssue, err := e.relics.Show(mr.SourceIssue); err == nil && sourceIssue != nil {
...
6. Close this task: rl close <this-task-id>

The Forge will automatically retry the merge after you force-push.`,
		mr.Branch,
		mr.ID,
		mr.Branch,
		mr.Target, mainSHA[:8],
	...
	task, err := e.relics.Create(relics.CreateOptions{
		Title:       taskTitle,
		Type:        "task",
		Priority:    boostedPriority,
		Description: description,
		Actor:       e.warband.Name + "/forge",
	})
	if err != nil {
...
func (e *Engineer) IsBeadOpen(beadID string) (bool, error) {
	issue, err := e.relics.Show(beadID)
	if err != nil {
...
// - Not blocked by an open task (handled by rl ready)
// Sorted by priority (highest first).
//
// This queries relics for merge-request wisps.
func (e *Engineer) ListReadyMRs() ([]*MRInfo, error) {
	// Query relics for ready merge-request issues
	issues, err := e.relics.ReadyWithType("merge-request")
	if err != nil {
		return nil, fmt.Errorf("querying relics for merge-requests: %w", err)
	}

	// Convert relics issues to MRInfo
	var mrs []*MRInfo
	for _, issue := range issues {
		...
		// Parse raid created_at if present
		var raidCreatedAt *time.Time
		if fields.RaidCreatedAt != "" {
			if t, err := time.Parse(time.RFC3339, fields.RaidCreatedAt); err == nil {
				raidCreatedAt = &t
			}
		}
...
// This replaces mrqueue.Claim() for relics-based MRs.
// The workerID is typically the forge's identifier (e.g., "horde/forge").
func (e *Engineer) ClaimMR(mrID, workerID string) error {
	return e.relics.Update(mrID, relics.UpdateOptions{
		Assignee: &workerID,
	})
}

// ReleaseMR releases a claimed MR back to the queue by clearing the assignee.
// This replaces mrqueue.Release() for relics-based MRs.
