...
func TestTouchInWorkspace(t *testing.T) {
	...
	// Touch the keepalive
	TouchInWorkspace(tmpDir, "hd status")
	...
	if state.LastCommand != "hd status" {
		t.Errorf("expected last_command 'hd status', got %q", state.LastCommand)
	}
...
func TestStateAge(t *testing.T) {
	...
	// Staleness classification belongs in Shaman totem, not Go code.
}
...
func TestDirectoryCreation(t *testing.T) {
	...
	TouchInWorkspace(workDir, "hd test")
...
func ExampleTouchInWorkspace() {
	...
	// Signal that "hd status" was run
	TouchInWorkspace(workspaceRoot, "hd status")

	// Signal a command with arguments
	TouchInWorkspace(workspaceRoot, "hd charge bd-abc123 ai-platform")
...
func ExampleRead() {
	...
	_ = state.LastCommand // e.g., "hd status"
