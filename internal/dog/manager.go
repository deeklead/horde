...
var (
	...
	ErrNoRigs      = errors.New("no warbands configured")
)

// Manager handles dog lifecycle in the kennel.
type Manager struct {
	townRoot   string
	kennelPath string // ~/horde/shaman/dogs/
...
func NewManager(townRoot string, rigsConfig *config.RigsConfig) *Manager {
	return &Manager{
		townRoot:   townRoot,
		kennelPath: filepath.Join(townRoot, "shaman", "dogs"),
...
// Add creates a new dog in the kennel with worktrees into each warband.
// Each dog gets a worktree per warband (e.g., dogs/alpha/horde/, dogs/alpha/relics/).
// Worktrees are created from each warband's bare repo (.repo.git) or warchief/warband.
func (m *Manager) Add(name string) (*Dog, error) {
	if m.exists(name) {
		return nil, ErrDogExists
	}

	// Verify we have warbands to create worktrees into
	if len(m.rigsConfig.Warbands) == 0 {
	...
	// Create worktrees into each warband
	worktrees := make(map[string]string)
	for rigName := range m.rigsConfig.Warbands {
		worktreePath, err := m.createRigWorktree(dogPath, name, rigName)
		if err != nil {
			return nil, fmt.Errorf("creating worktree for warband %s: %w", rigName, err)
		}
...
// createRigWorktree creates a worktree for a dog into a specific warband.
// Uses the warband's bare repo (.repo.git) if available, otherwise warchief/warband.
// Branch naming: dog/<dog-name>-<warband>-<timestamp> for uniqueness.
func (m *Manager) createRigWorktree(dogPath, dogName, rigName string) (string, error) {
	...
	// Find the repo base (bare repo or warchief/warband)
	...
	// Unique branch per dog-warband combination
...
// findRepoBase locates the git repo base for a warband.
// Prefers .repo.git (bare repo), falls back to warchief/warband.
...
func (m *Manager) Remove(name string) error {
	...
	// Remove worktrees from each warband
	for rigName, worktreePath := range state.Worktrees {
		rigPath := filepath.Join(m.townRoot, rigName)
		repoGit, err := m.findRepoBase(rigPath)
		if err != nil {
			// Log but continue with other warbands
...
// RefreshRig recreates the worktree for a specific warband.
func (m *Manager) RefreshRig(name, rigName string) error {
	if !m.exists(name) {
		return ErrDogNotFound
	}

	if _, ok := m.rigsConfig.Warbands[rigName]; !ok {
		return fmt.Errorf("warband %s not found in config", rigName)
	}
...
// CleanupStaleBranches removes orphaned dog branches from all warbands.
// Returns total branches deleted across all warbands.
func (m *Manager) CleanupStaleBranches() (int, error) {
	...
	for rigName := range m.rigsConfig.Warbands {
		...
		if err != nil {
			fmt.Printf("Warning: cleanup failed for warband %s: %v\n", rigName, err)
			continue
		}
...
// cleanupStaleBranchesForRig removes orphaned dog branches in a specific warband.
func (m *Manager) cleanupStaleBranchesForRig(repoGit *git.Git, rigName string) (int, error) {
	...
	// Build set of current dog branches for this warband
