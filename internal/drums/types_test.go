...
func TestAddressToIdentity(t *testing.T) {
	tests := []struct {
		...
		// Encampment-level agents keep trailing slash
		{"warchief", "warchief/"},
		{"warchief/", "warchief/"},
		{"shaman", "shaman/"},
		{"shaman/", "shaman/"},

		// Warband-level agents: clan/ and raiders/ normalized to canonical form
		{"horde/raiders/Toast", "horde/Toast"},
		{"horde/clan/max", "horde/max"},
		{"horde/Toast", "horde/Toast"},         // Already canonical
		{"horde/max", "horde/max"},             // Already canonical
		{"horde/forge", "horde/forge"},
		{"horde/witness", "horde/witness"},

		// Warband broadcast (trailing slash removed)
		{"horde/", "horde"},
	}
...
func TestIdentityToAddress(t *testing.T) {
	tests := []struct {
		...
		// Warband-level agents: clan/ and raiders/ normalized
		{"horde/raiders/Toast", "horde/Toast"},
		{"horde/clan/max", "horde/max"},
		{"horde/Toast", "horde/Toast"},  // Already canonical
		{"horde/forge", "horde/forge"},
		...
		// Warband name only (no transformation)
		{"horde", "horde"},
	}
...
func TestPriorityToRelics(t *testing.T) {
	tests := []struct {
		priority Priority
		expected int
	}{
	...
	for _, tt := range tests {
		t.Run(string(tt.priority), func(t *testing.T) {
			got := PriorityToRelics(tt.priority)
			if got != tt.expected {
				t.Errorf("PriorityToRelics(%q) = %d, want %d", tt.priority, got, tt.expected)
			}
		})
	}
}
...
func TestNewMessage(t *testing.T) {
	msg := NewMessage("warchief/", "horde/Toast", "Test Subject", "Test Body")

	if msg.From != "warchief/" {
		t.Errorf("From = %q, want 'warchief/'", msg.From)
	}
	if msg.To != "horde/Toast" {
		t.Errorf("To = %q, want 'horde/Toast'", msg.To)
	}
...
func TestNewReplyMessage(t *testing.T) {
	...
	reply := NewReplyMessage("warchief/", "horde/Toast", "Re: Original Subject", "Reply body", original)
	...
	if reply.From != "warchief/" {
		t.Errorf("From = %q, want 'warchief/'", reply.From)
	}
	if reply.To != "horde/Toast" {
		t.Errorf("To = %q, want 'horde/Toast'", reply.To)
	}
...
func TestRelicsMessageToMessage(t *testing.T) {
	now := time.Now()
	bm := RelicsMessage{
		ID:          "hq-test",
		Title:       "Test Subject",
		...
		Status:      "open",
		Assignee:    "horde/Toast",
		Labels:      []string{"from:warchief/", "thread:t-001"},
		CreatedAt:   now,
		Priority:    1,
	}
...
func TestRelicsMessageToMessageWithReplyTo(t *testing.T) {
	bm := RelicsMessage{
		ID:          "hq-reply",
		Title:       "Reply Subject",
		...
		Status:      "open",
		Assignee:    "horde/Toast",
		Labels:      []string{"from:warchief/", "thread:t-002", "reply-to:orig-001", "msg-type:reply"},
		CreatedAt:   time.Now(),
		Priority:    2,
	}
...
func TestRelicsMessageToMessagePriorities(t *testing.T) {
	tests := []struct {
		priority int
		expected Priority
	}{
...
func TestRelicsMessageToMessageTypes(t *testing.T) {
	tests := []struct {
		msgType  string
		expected MessageType
	}{
...
func TestRelicsMessageToMessageEmptyLabels(t *testing.T) {
	bm := RelicsMessage{
		ID:          "hq-empty",
		Title:       "Empty Labels",
...
func TestNewQueueMessage(t *testing.T) {
	msg := NewQueueMessage("warchief/", "work-requests", "New Task", "Please process this")
...
func TestNewChannelMessage(t *testing.T) {
	msg := NewChannelMessage("shaman/", "alerts", "System Alert", "System is healthy")

	if msg.From != "shaman/" {
		t.Errorf("From = %q, want 'shaman/'", msg.From)
	}
...
func TestMessageIsQueueMessage(t *testing.T) {
	directMsg := NewMessage("warchief/", "horde/Toast", "Test", "Body")
	queueMsg := NewQueueMessage("warchief/", "work-requests", "Task", "Body")
	channelMsg := NewChannelMessage("shaman/", "alerts", "Alert", "Body")
...
func TestMessageValidate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			msg:     NewMessage("warchief/", "horde/Toast", "Test", "Body"),
			wantErr: false,
		},
		{
			name:    "valid queue message",
			msg:     NewQueueMessage("warchief/", "work-requests", "Task", "Body"),
			wantErr: false,
		},
		{
			name:    "valid channel message",
			msg:     NewChannelMessage("shaman/", "alerts", "Alert", "Body"),
			wantErr: false,
		},
		{
...
func TestRelicsMessageParseQueueChannelLabels(t *testing.T) {
	...
	if msg.ClaimedBy != "horde/nux" {
		t.Errorf("ClaimedBy = %q, want 'horde/nux'", msg.ClaimedBy)
	}
...
func TestRelicsMessageParseChannelLabel(t *testing.T) {
	bm := RelicsMessage{
		ID:          "hq-channel",
		Title:       "Channel Message",
		Description: "Test channel message",
		Status:      "open",
		Labels:      []string{"from:shaman/", "channel:alerts"},
		Priority:    2,
	}
...
func TestRelicsMessageIsQueueMessage(t *testing.T) {
	queueMsg := RelicsMessage{
		ID:     "hq-queue",
		Labels: []string{"queue:work-requests"},
	}
	directMsg := RelicsMessage{
		ID:       "hq-direct",
		Assignee: "horde/Toast",
	}
	channelMsg := RelicsMessage{
		ID:     "hq-channel",
...
func TestRelicsMessageIsChannelMessage(t *testing.T) {
	queueMsg := RelicsMessage{
		ID:     "hq-queue",
		Labels: []string{"queue:work-requests"},
	}
...
func TestRelicsMessageIsDirectMessage(t *testing.T) {
	queueMsg := RelicsMessage{
		ID:     "hq-queue",
		Labels: []string{"queue:work-requests"},
	}
...
func TestMessageIsClaimed(t *testing.T) {
	unclaimed := NewQueueMessage("warchief/", "work-requests", "Task", "Body")
	if unclaimed.IsClaimed() {
		t.Error("Unclaimed message should not be claimed")
	}

	claimed := NewQueueMessage("warchief/", "work-requests", "Task", "Body")
	claimed.ClaimedBy = "horde/nux"
	now := time.Now()
