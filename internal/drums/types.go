// Package drums provides messaging for agent communication via relics.
package drums
...
const (
	// DeliveryQueue creates the message in the wardrums for periodic checking.
	// This is the default delivery mode. Agent checks with `hd drums check`.
...
// Message represents a drums message between agents.
// This is the GGT-side representation; it gets translated to/from relics messages.
type Message struct {
	// ID is a unique message identifier (relics issue ID like "bd-abc123").
	ID string `json:"id"`

	// From is the sender address (e.g., "horde/Toast" or "warchief/").
	...
	// Read indicates if the message has been read (closed in relics).
	Read bool `json:"read"`
	...
	// Wisp messages auto-cleanup on scout squash.
...
// RelicsMessage represents a message as returned by rl list/show commands.
// Messages are relics issues with type=message and metadata stored in labels.
...
func (bm *RelicsMessage) ParseLabels() {
...
func (bm *RelicsMessage) GetCC() []string {
	return bm.cc
}

// IsCCRecipient checks if the given identity is in the CC list.
func (bm *RelicsMessage) IsCCRecipient(identity string) bool {
...
// ToMessage converts a RelicsMessage to a GGT Message.
func (bm *RelicsMessage) ToMessage() *Message {
	// Parse labels to extract metadata
	bm.ParseLabels()

	// Convert relics priority (0=urgent, 1=high, 2=normal, 3=low) to GGT Priority
...
func (bm *RelicsMessage) GetQueue() string {
	return bm.queue
}

// GetChannel returns the channel name for broadcast messages.
func (bm *RelicsMessage) GetChannel() string {
	return bm.channel
}

// GetClaimedBy returns who claimed the queue message.
func (bm *RelicsMessage) GetClaimedBy() string {
	return bm.claimedBy
}

// GetClaimedAt returns when the queue message was claimed.
func (bm *RelicsMessage) GetClaimedAt() *time.Time {
	return bm.claimedAt
}

// IsQueueMessage returns true if this is a queue-routed message.
func (bm *RelicsMessage) IsQueueMessage() bool {
	bm.ParseLabels()
	return bm.queue != ""
}

// IsChannelMessage returns true if this is a channel broadcast message.
func (bm *RelicsMessage) IsChannelMessage() bool {
	bm.ParseLabels()
	return bm.channel != ""
}

// IsDirectMessage returns true if this is a direct (To-addressed) message.
func (bm *RelicsMessage) IsDirectMessage() bool {
	bm.ParseLabels()
	return bm.queue == "" && bm.channel == "" && bm.Assignee != ""
}

// HasLabel checks if the message has a specific label.
func (bm *RelicsMessage) HasLabel(label string) bool {
...
// PriorityToRelics converts a GGT Priority to relics priority integer.
// Returns: 0=urgent, 1=high, 2=normal, 3=low
func PriorityToRelics(p Priority) int {
	switch p {
	case PriorityUrgent:
		return 0
	case PriorityHigh:
...
// PriorityFromInt converts a relics-style integer priority to a Priority.
...
// addressToIdentity converts a GGT address to a relics identity.
//
...
//   - "shaman/" → "shaman/"
//   - "shaman" → "shaman/"
//   - "horde/raiders/Toast" → "horde/Toast" (normalized)
//   - "horde/clan/max" → "horde/max" (normalized)
//   - "horde/Toast" → "horde/Toast" (already canonical)
//   - "horde/forge" → "horde/forge"
//   - "horde/" → "horde" (warband broadcast)
func addressToIdentity(address string) string {
	...
	// Encampment-level agents: warchief and shaman keep trailing slash
	if address == "warchief" || address == "warchief/" {
		return "warchief/"
	}
	if address == "shaman" || address == "shaman/" {
		return "shaman/"
	}

	// Trim trailing slash for warband-level addresses
	if len(address) > 0 && address[len(address)-1] == '/' {
		address = address[:len(address)-1]
	}

	// Normalize clan/ and raiders/ to canonical form:
	// "warband/clan/name" → "warband/name"
	// "warband/raiders/name" → "warband/name"
	parts := strings.Split(address, "/")
	if len(parts) == 3 && (parts[1] == "clan" || parts[1] == "raiders") {
...
// identityToAddress converts a relics identity back to a GGT address.
//
...
func identityToAddress(identity string) string {
	...
	// Encampment-level agents ensure trailing slash
	if identity == "warchief" || identity == "warchief/" {
		return "warchief/"
	}
	if identity == "shaman" || identity == "shaman/" {
		return "shaman/"
	}
