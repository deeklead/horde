...
// bdError represents an error from running a rl command.
...
// runBdCommand executes a rl command with proper environment setup.
// workDir is the directory to run the command in.
// relicsDir is the RELICS_DIR environment variable value.
...
func runBdCommand(args []string, workDir, relicsDir string, extraEnv ...string) ([]byte, error) {
	cmd := exec.Command("rl", args...) //nolint:gosec // G204: rl is a trusted internal tool
	cmd.Dir = workDir

	env := append(cmd.Environ(), "RELICS_DIR="+relicsDir)
