...
func TestDetectTownRoot(t *testing.T) {
	...
	rigDir := filepath.Join(townRoot, "horde", "raiders", "test")

	// Create warchief/encampment.json marker
	if err := os.MkdirAll(warchiefDir, 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(warchiefDir, "encampment.json"), []byte("{}"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		{
			name:     "from encampment root",
			startDir: townRoot,
			want:     townRoot,
		},
		{
			name:     "from warband subdirectory",
			startDir: rigDir,
			want:     townRoot,
		},
		{
			name:     "from warchief directory",
			startDir: warchiefDir,
			want:     townRoot,
		},
		{
			name:     "from non-encampment directory",
			startDir: tmpDir,
			want:     "", // No encampment.json marker above tmpDir
		},
	}
...
func TestIsSelfMail(t *testing.T) {
	tests := []struct {
		...
		{"warchief", "warchief/", true},
		{"warchief/", "warchief", true},
		{"horde/Toast", "horde/Toast", true},
		{"horde/Toast/", "horde/Toast", true},
		{"warchief/", "shaman/", false},
		{"horde/Toast", "horde/Nux", false},
		{"", "", true},
	}
...
func TestShouldBeWisp(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "RAIDER_STARTED subject",
			msg:  &Message{Subject: "RAIDER_STARTED: Toast"},
			want: true,
		},
		{
			name: "raider_done subject (lowercase)",
			msg:  &Message{Subject: "raider_done: work complete"},
			want: true,
		},
		{
			name: "SIGNAL subject",
			msg:  &Message{Subject: "SIGNAL: check your hook"},
			want: true,
		},
		{
...
func TestResolveRelicsDir(t *testing.T) {
	// With encampment root set
	r := NewRouterWithTownRoot("/work/dir", "/home/user/gt")
	got := r.resolveRelicsDir("horde/Toast")
	want := "/home/user/horde/.relics"
	if got != want {
		t.Errorf("resolveRelicsDir with townRoot = %q, want %q", got, want)
	}

	// Without encampment root (fallback to workDir)
	r2 := &Router{workDir: "/work/dir", townRoot: ""}
	got2 := r2.resolveRelicsDir("warchief/")
	want2 := "/work/dir/.relics"
	if got2 != want2 {
		t.Errorf("resolveRelicsDir without townRoot = %q, want %q", got2, want2)
	}
}

func TestNewRouterWithTownRoot(t *testing.T) {
	r := NewRouterWithTownRoot("/work/warband", "/home/gt")
	if r.workDir != "/work/warband" {
		t.Errorf("workDir = %q, want '/work/warband'", r.workDir)
	}
...
func TestParseListName(t *testing.T) {
	tests := []struct {
		...
		{"list:oncall", "oncall"},
		{"list:cleanup/horde", "cleanup/horde"},
		{"list:", ""},
		{"list:alerts", "alerts"},
	}
...
func TestParseQueueName(t *testing.T) {
	tests := []struct {
		...
		{"queue:work", "work"},
		{"queue:horde/raiders", "horde/raiders"},
		{"queue:", ""},
...
func TestExpandList(t *testing.T) {
  ...
  "lists": {
    ...
    "cleanup/horde": ["horde/witness", "shaman/"]
  }
}`
	...
	tests := []struct {
	...
	}{
		{
			name:     "oncall list",
			listName: "oncall",
			want:     []string{"warchief/", "horde/witness"},
		},
		{
			name:     "cleanup/horde list",
			listName: "cleanup/horde",
			want:     []string{"horde/witness", "shaman/"},
		},
		{
			name:      "unknown list",
...
func TestExpandListNoTownRoot(t *testing.T) {
	...
	if !contains(err.Error(), "no encampment root") {
		t.Errorf("expandList error = %v, want containing 'no encampment root'", err)
	}
}
...
func TestExpandQueue(t *testing.T) {
  ...
  "version": 1,
  "queues": {
    "work/horde": {"workers": ["horde/raiders/*"], "max_claims": 3},
    "priority-high": {"workers": ["warchief/", "horde/witness"]}
  }
}`
	if err := os.WriteFile(filepath.Join(configDir, "messaging.json"), []byte(configContent), 0644); err != nil {
		t.Fatal(err)
	...
	tests := []struct {
		...
		{
			name:        "work/horde queue",
			queueName:   "work/horde",
			wantWorkers: []string{"horde/raiders/*"},
			wantMax:     3,
		},
		{
			name:        "priority-high queue",
			queueName:   "priority-high",
			wantWorkers: []string{"warchief/", "horde/witness"},
...
func TestExpandQueueNoTownRoot(t *testing.T) {
	...
	if !contains(err.Error(), "no encampment root") {
		t.Errorf("expandQueue error = %v, want containing 'no encampment root'", err)
	}
}
...
func TestParseAnnounceName(t *testing.T) {
	tests := []struct {
		...
		{"announce:horde/updates", "horde/updates"},
		{"announce:", ""},
...
func TestParseGroupAddress(t *testing.T) {
	tests := []struct {
		...
		{"@encampment", GroupTypeTown, "", "", false},

		// Role-based patterns (all agents of a role type)
		{"@witnesses", GroupTypeRole, "witness", "", false},
		{"@dogs", GroupTypeRole, "dog", "", false},
		{"@refineries", GroupTypeRole, "forge", "", false},
		{"@shamans", GroupTypeRole, "shaman", "", false},

		// Warband pattern (all agents in a warband)
		{"@warband/horde", GroupTypeRig, "", "horde", false},
		{"@warband/relics", GroupTypeRig, "", "relics", false},

		// Warband+role patterns
		{"@clan/horde", GroupTypeRigRole, "clan", "horde", false},
		{"@raiders/horde", GroupTypeRigRole, "raider", "horde", false},

		// Invalid patterns
		{"warchief/", "", "", "", true},
		{"@invalid", "", "", "", true},
		{"@clan/", "", "", "", true}, // Empty warband
		{"@warband", "", "", "", true},   // Missing warband name
		{"", "", "", "", true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.address, func(t *testing.T) {
			...
			if got.Warband != tt.wantRig {
				t.Errorf("parseGroupAddress(%q).Warband = %q, want %q", tt.address, got.Warband, tt.wantRig)
			}
...
func TestAgentBeadToAddress(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			bead: &agentBead{ID: "bd-horde-witness"},
			want: "",
		},
		{
			name: "empty ID",
...
func TestExpandAnnounce(t *testing.T) {
  ...
  "version": 1,
  "announces": {
    "alerts": {"readers": ["@encampment"], "retain_count": 10},
    "status/horde": {"readers": ["horde/witness", "warchief/"], "retain_count": 5}
  }
}`
	if err := os.WriteFile(filepath.Join(configDir, "messaging.json"), []byte(configContent), 0644); err != nil {
		t.Fatal(err)
	...
	tests := []struct {
	...
	}{
		{
			...
			announceName: "alerts",
			wantReaders:  []string{"@encampment"},
			wantRetain:   10,
		},
		{
			name:         "status/horde announce",
			announceName: "status/horde",
			wantReaders:  []string{"horde/witness", "warchief/"},
			wantRetain:   5,
		},
		{
...
func TestExpandAnnounceNoTownRoot(t *testing.T) {
	...
	if !contains(err.Error(), "no encampment root") {
		t.Errorf("expandAnnounce error = %v, want containing 'no encampment root'", err)
	}
}
