...
func TestNewMailbox(t *testing.T) {
	...
	if !m.legacy {
		t.Error("NewMailbox should create legacy wardrums")
	}
}

func TestNewMailboxRelics(t *testing.T) {
	m := NewMailboxRelics("horde/Toast", "/work/dir")
	if m.identity != "horde/Toast" {
		t.Errorf("identity = %q, want %q", m.identity, "horde/Toast")
	}
	if m.legacy {
		t.Error("NewMailboxRelics should not create legacy wardrums")
	}
}
...
func TestMailboxRelicsAppendError(t *testing.T) {
	m := NewMailboxRelics("horde/Toast", "/work/dir")

	err := m.Append(&Message{})
	if err == nil {
		t.Error("Append on relics wardrums should error")
	}
}
...
func TestMailboxIdentityAndPath(t *testing.T) {
	...
	if legacy.Identity() != "" {
		t.Errorf("Legacy wardrums identity = %q, want empty", legacy.Identity())
	}
	if legacy.Path() != "/tmp/test/inbox.jsonl" {
		t.Errorf("Legacy wardrums path = %q, want /tmp/test/inbox.jsonl", legacy.Path())
	}

	// Relics wardrums
	relics := NewMailboxRelics("horde/Toast", "/work/dir")
	if relics.Identity() != "horde/Toast" {
		t.Errorf("Relics wardrums identity = %q, want horde/Toast", relics.Identity())
	}
	if relics.Path() != "" {
		t.Errorf("Relics wardrums path = %q, want empty", relics.Path())
	}
}
...
func TestMailboxPersistence(t *testing.T) {
	...
	// Create wardrums and add message
	...
	// Create new wardrums pointing to same location
	...
	if len(msgs) != 1 {
		t.Fatalf("Reloaded wardrums has %d messages, want 1", len(msgs))
	}
...
func TestNewMailboxWithRelicsDir(t *testing.T) {
	m := NewMailboxWithRelicsDir("horde/Toast", "/work/dir", "/custom/.relics")
	if m.identity != "horde/Toast" {
		t.Errorf("identity = %q, want 'horde/Toast'", m.identity)
	}
	if m.relicsDir != "/custom/.relics" {
		t.Errorf("relicsDir = %q, want '/custom/.relics'", m.relicsDir)
	}
}
