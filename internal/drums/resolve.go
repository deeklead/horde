// Package drums provides address resolution for relics-native messaging.
// This module implements the resolution order:
// 1. Contains '/' → agent address or pattern
// 2. Starts with '@' → special pattern (@encampment, @clan, @warband/X, @role/X)
...
type Recipient struct {
	Address      string        // The resolved address (e.g., "horde/clan/max")
...
// Resolver handles address resolution for relics-native messaging.
...
func NewResolver(b *relics.Relics, townRoot string) *Resolver {
	return &Resolver{
		relics:    b,
		townRoot: townRoot,
	}
...
// 2. Starts with '@' → special pattern (@encampment, @clan, etc.)
...
func (r *Resolver) Resolve(address string) ([]Recipient, error) {
	...
	if strings.HasPrefix(address, "group:") {
		...
		return r.resolveRelicsGroup(name)
	}
...
// Patterns like "*/witness" or "horde/*" are expanded.
func (r *Resolver) resolvePattern(pattern string) ([]Recipient, error) {
	if r.relics == nil {
		return nil, fmt.Errorf("relics not available for pattern resolution")
	}

	// Get all agent relics
	agents, err := r.relics.ListAgentRelics()
	if err != nil {
...
// These include @encampment, @clan, @warband/X, @role/X, @overseer.
func (r *Resolver) resolveAtPattern(address string) ([]Recipient, error) {
	// First check if this is a relics-native group (if relics available)
	if r.relics != nil {
		groupName := strings.TrimPrefix(address, "@")
		issue, fields, err := r.relics.LookupGroupByName(groupName)
		if err != nil {
			return nil, err
		}
		if issue != nil && fields != nil {
			// Found a relics-native group - expand its members
...
func (r *Resolver) resolveByName(name string) ([]Recipient, error) {
	...
	var groupFields *relics.GroupFields

	// Check for relics-native group
	if r.relics != nil {
		_, fields, err := r.relics.LookupGroupByName(name)
		if err != nil {
			return nil, err
		}
		if fields != nil {
...
// resolveRelicsGroup resolves a relics-native group by name.
func (r *Resolver) resolveRelicsGroup(name string) ([]Recipient, error) {
	if r.relics == nil {
		return nil, fmt.Errorf("relics not available")
	}
...
func (r *Resolver) expandGroupMembers(fields *relics.GroupFields) ([]Recipient, error) {
...
func (r *Resolver) expandGroupMembersWithVisited(fields *relics.GroupFields, visited map[string]bool) ([]Recipient, error) {
	if fields == nil {
		return nil, nil
	}
...
func (r *Resolver) resolveMemberWithVisited(member string, visited map[string]bool) ([]Recipient, error) {
	...
	if r.relics != nil && !strings.Contains(member, "/") && !strings.HasPrefix(member, "@") {
		_, fields, err := r.relics.LookupGroupByName(member)
...
func agentBeadIDToAddress(id string) string {
	...
	switch len(parts) {
	case 1:
		// Encampment-level: gt-warchief → warchief/
		return parts[0] + "/"
	case 2:
		// Warband singleton: gt-horde-witness → horde/witness
		return parts[0] + "/" + parts[1]
	default:
		// Warband named agent: gt-horde-clan-max → horde/clan/max
		if len(parts) >= 3 {
