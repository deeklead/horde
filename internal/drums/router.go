...
// Router handles message delivery via relics.
// It routes messages to the correct relics database based on address:
// - Encampment-level (warchief/, shaman/) -> {townRoot}/.relics
// - Warband-level (warband/raider) -> {townRoot}/{warband}/.relics
type Router struct {
	workDir  string // fallback directory to run rl commands in
	townRoot string // encampment root directory (e.g., ~/horde)
	tmux     *tmux.Tmux
}

// NewRouter creates a new drums router.
// workDir should be a directory containing a .relics database.
// The encampment root is auto-detected from workDir if possible.
func NewRouter(workDir string) *Router {
	// Try to detect encampment root from workDir
...
// NewRouterWithTownRoot creates a router with an explicit encampment root.
...
// isChannelAddress returns true if the address uses channel:name syntax (relics-native channels).
...
func expandFromConfig[T any](r *Router, name string, getter func(*config.MessagingConfig) (T, bool), errType error) (T, error) {
	var zero T
	if r.townRoot == "" {
		return zero, fmt.Errorf("%w: %s (no encampment root)", errType, name)
	}
...
// detectTownRoot finds the encampment root using workspace.Find.
// This ensures consistent detection with the rest of the codebase,
// supporting both primary (warchief/encampment.json) and secondary (warchief/) markers.
...
// resolveRelicsDir returns the correct .relics directory for the given address.
//
// Two-level relics architecture:
// - ALL drums uses encampment relics ({townRoot}/.relics) regardless of address
// - Warband-level relics ({warband}/.relics) are for project issues only, not drums
//
// This ensures messages are visible to all agents in the encampment.
func (r *Router) resolveRelicsDir(_ string) string { // address unused: all drums uses encampment-level relics
	// If no encampment root, fall back to workDir's .relics
	if r.townRoot == "" {
		return filepath.Join(r.workDir, ".relics")
	}

	// All drums uses encampment-level relics
	return filepath.Join(r.townRoot, ".relics")
}

// isTownLevelAddress returns true if the address is for a encampment-level agent or the overseer.
func isTownLevelAddress(address string) bool {
	addr := strings.TrimSuffix(address, "/")
	return addr == "warchief" || addr == "shaman" || addr == "overseer"
}
...
const (
	GroupTypeRig      GroupType = "warband"      // @warband/<rigname> - all agents in a warband
	GroupTypeTown     GroupType = "encampment"     // @encampment - all encampment-level agents
	GroupTypeRole     GroupType = "role"     // @witnesses, @dogs, etc. - all agents of a role
	GroupTypeRigRole  GroupType = "warband-role" // @clan/<rigname>, @raiders/<rigname> - role in a warband
...
type ParsedGroup struct {
	Type      GroupType
	RoleType  string // witness, clan, raider, dog, etc.
	Warband       string // warband name for warband-scoped groups
...
//
// Supported patterns:
//   - @warband/<rigname>: All agents in a warband
//   - @encampment: All encampment-level agents (warchief, shaman)
//   - @witnesses: All witnesses across warbands
//   - @clan/<rigname>: Clan workers in a specific warband
//   - @raiders/<rigname>: Raiders in a specific warband
//   - @dogs: All Shaman dogs
...
func parseGroupAddress(address string) *ParsedGroup {
	...
	case "refineries":
		return &ParsedGroup{Type: GroupTypeRole, RoleType: "forge", Original: address}
	case "shamans":
		return &ParsedGroup{Type: GroupTypeRole, RoleType: "shaman", Original: address}
	}

	// Parse patterns with slashes: @warband/<name>, @clan/<warband>, @raiders/<warband>
	...
	switch prefix {
	case "warband":
		return &ParsedGroup{Type: GroupTypeRig, Warband: qualifier, Original: address}
	case "clan":
		return &ParsedGroup{Type: GroupTypeRigRole, RoleType: "clan", Warband: qualifier, Original: address}
	case "raiders":
		return &ParsedGroup{Type: GroupTypeRigRole, RoleType: "raider", Warband: qualifier, Original: address}
	default:
		return nil // Unknown group type
	}
}

// agentBead represents an agent bead as returned by rl list --type=agent.
...
// agentBeadToAddress converts an agent bead to a drums address.
// Uses the agent bead ID to derive the address:
//   - gt-warchief → warchief/
//   - gt-shaman → shaman/
//   - gt-horde-witness → horde/witness
//   - gt-horde-clan-max → horde/max
//   - gt-horde-raider-Toast → horde/Toast
func agentBeadToAddress(bead *agentBead) string {
	...
	// Strip prefix
	rest := strings.TrimPrefix(id, "hd-")
	parts := strings.Split(rest, "-")

	switch len(parts) {
	case 1:
		// Encampment-level: gt-warchief, gt-shaman
		return parts[0] + "/"
	case 2:
		// Warband singleton: gt-horde-witness
		return parts[0] + "/" + parts[1]
	default:
		// Warband named agent: gt-horde-clan-max, gt-horde-raider-Toast
		// Skip the role part (parts[1]) and use warband/name format
		if len(parts) >= 3 {
			// Rejoin if name has hyphens: gt-horde-raider-my-agent
...
func (r *Router) resolveGroup(group *ParsedGroup) ([]string, error) {
	...
	case GroupTypeRig:
		return r.resolveAgentsByRig(group.Warband)
	case GroupTypeRigRole:
		return r.resolveAgentsByRole(group.RoleType, group.Warband)
	default:
...
func (r *Router) resolveOverseer() ([]string, error) {
	if r.townRoot == "" {
		return nil, errors.New("encampment root not set, cannot resolve @overseer")
	}
...
// resolveTownAgents resolves @encampment to all encampment-level agents (warchief, shaman).
func (r *Router) resolveTownAgents() ([]string, error) {
	// Encampment-level agents have warband=null in their description
	agents, err := r.queryAgents("warband: null")
	if err != nil {
		return nil, err
	}
...
// If warband is non-empty, also filters by warband.
func (r *Router) resolveAgentsByRole(roleType, warband string) ([]string, error) {
	// Build query filter
	...
	for _, agent := range agents {
		...
		if warband != "" {
			// Check if agent's description contains matching warband
			if !strings.Contains(agent.Description, "warband: "+warband) {
				continue
			}
		}
		if addr := agentBeadToAddress(agent); addr != "" {
...
// resolveAgentsByRig resolves @warband/<rigname> to all agents in that warband.
func (r *Router) resolveAgentsByRig(warband string) ([]string, error) {
	// Query for agents with matching warband in description
	query := "warband: " + warband
	agents, err := r.queryAgents(query)
	if err != nil {
		return nil, err
	}
...
// queryAgents queries agent relics using rl list with description filtering.
func (r *Router) queryAgents(descContains string) ([]*agentBead, error) {
	relicsDir := r.resolveRelicsDir("")
	...
	stdout, err := runBdCommand(args, filepath.Dir(relicsDir), relicsDir)
	if err != nil {
...
// - Subject matches lifecycle message patterns (RAIDER_*, SIGNAL, etc.)
...
// Send delivers a message via relics message.
// Routes the message to the correct relics database based on recipient address.
...
func (r *Router) Send(msg *Message) error {
	...
	// Check for relics-native channel address - broadcast with retention
...
func (r *Router) sendToSingle(msg *Message) error {
	// Convert addresses to relics identities
	...
	// Build command: rl create <subject> --type=message --assignee=<recipient> -d <body>
	...
	// Add priority flag
	relicsPriority := PriorityToRelics(msg.Priority)
	args = append(args, "--priority", fmt.Sprintf("%d", relicsPriority))
	...
	relicsDir := r.resolveRelicsDir(msg.To)
	_, err := runBdCommand(args, filepath.Dir(relicsDir), relicsDir)
	if err != nil {
		return fmt.Errorf("sending message: %w", err)
	}

	// Notify recipient if they have an active session (best-effort notification)
	// Skip notification for self-drums (handoffs to future-self don't need present-self notified)
...
// The message is stored in encampment-level relics with queue metadata.
// Workers claim messages using rl update --claimed-by.
func (r *Router) sendToQueue(msg *Message) error {
	...
	// Build command: rl create <subject> --type=message --assignee=queue:<name> -d <body>
	...
	// Queue messages go to encampment-level relics (shared location)
	relicsDir := r.resolveRelicsDir("")
	_, err = runBdCommand(args, filepath.Dir(relicsDir), relicsDir)
	if err != nil {
...
// ONE copy is stored in encampment-level relics with announce_channel metadata.
func (r *Router) sendToAnnounce(msg *Message) error {
	...
	// Build command: rl create <subject> --type=message --assignee=announce:<name> -d <body>
	...
	// Announce messages go to encampment-level relics (shared location)
	relicsDir := r.resolveRelicsDir("")
	_, err = runBdCommand(args, filepath.Dir(relicsDir), relicsDir)
	if err != nil {
...
// sendToChannel delivers a message to a relics-native channel.
...
func (r *Router) sendToChannel(msg *Message) error {
	...
	// Validate channel exists as a relics-native channel
	if r.townRoot == "" {
		return fmt.Errorf("encampment root not set, cannot send to channel: %s", channelName)
	}
	b := relics.New(r.townRoot)
	...
	if fields.Status == relics.ChannelStatusClosed {
	...
	// Build command: rl create <subject> --type=message --assignee=channel:<name> -d <body>
	...
	// Channel messages go to encampment-level relics (shared location)
	relicsDir := r.resolveRelicsDir("")
	_, err = runBdCommand(args, filepath.Dir(relicsDir), relicsDir)
	if err != nil {
...
func (r *Router) pruneAnnounce(announceName string, retainCount int) error {
	...
	// Use rl list with labels filter to find messages with announce:<name> label
	args := []string{"list",
		"--type=message",
	...
	for i := 0; i < toDelete && i < len(messages); i++ {
		...
		_, _ = runBdCommand(deleteArgs, filepath.Dir(relicsDir), relicsDir)
	}
...
// GetMailbox returns a Wardrums for the given address.
// Routes to the correct relics database based on the address.
func (r *Router) GetMailbox(address string) (*Wardrums, error) {
	relicsDir := r.resolveRelicsDir(address)
	workDir := filepath.Dir(relicsDir) // Parent of .relics
	return NewMailboxFromAddress(address, workDir), nil
}

// notifyRecipient sends a notification to a recipient's tmux session.
// Uses SignalSession to add the notification to the agent's conversation history.
// Supports warchief/, warband/raider, and warband/forge addresses.
func (r *Router) notifyRecipient(msg *Message) error {
	...
	notification := fmt.Sprintf("📬 You have new drums from %s. Subject: %s. Run 'hd drums inbox' to read.", msg.From, msg.Subject)
	return r.tmux.SignalSession(sessionID, notification)
}

// addressToSessionID converts a drums address to a tmux session ID.
// Returns empty string if address format is not recognized.
func addressToSessionID(address string) string {
	// Warchief address: "warchief/" or "warchief"
	if strings.HasPrefix(address, "warchief") {
		return session.WarchiefSessionName()
	}

	// Shaman address: "shaman/" or "shaman"
	if strings.HasPrefix(address, "shaman") {
		return session.ShamanSessionName()
	}

	// Warband-based address: "warband/target"
