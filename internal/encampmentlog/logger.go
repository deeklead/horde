// Package encampmentlog provides centralized logging for Horde agent lifecycle events.
...
const (
	...
	// EventCallback indicates a callback was processed during scout.
	EventCallback EventType = "callback"

	// Witness scout events
	EventPatrolStarted  EventType = "patrol_started"
	EventRaiderChecked EventType = "raider_checked"
	EventRaiderNudged  EventType = "raider_nudged"
...
type Event struct {
	...
	Agent     string    `json:"agent"`            // e.g., "horde/clan/max" or "horde/raiders/Toast"
	Context   string    `json:"context,omitempty"` // Additional context (issue ID, error message, etc.)
}

// Logger handles writing events to the encampment log file.
type Logger struct {
	logPath string
	mu      sync.Mutex
}

// logDir returns the directory for encampment logs.
func logDir(townRoot string) string {
	return filepath.Join(townRoot, "logs")
}

// logPath returns the path to the encampment log file.
func logPath(townRoot string) string {
	return filepath.Join(logDir(townRoot), "encampment.log")
}

// NewLogger creates a new Logger for the given encampment root.
...
// LogEvent logs a single event to the encampment log.
...
// Format: 2025-12-26 15:30:45 [muster] horde/clan/max spawned for gt-xyz
func formatLogLine(e Event) string {
	...
	case EventPatrolStarted:
		if e.Context != "" {
			detail = fmt.Sprintf("started scout (%s)", e.Context)
		} else {
			detail = "started scout"
		}
	case EventRaiderChecked:
		if e.Context != "" {
			detail = fmt.Sprintf("checked raider %s", e.Context)
		} else {
			detail = "checked raider"
		}
	case EventRaiderNudged:
		if e.Context != "" {
			detail = fmt.Sprintf("nudged raider (%s)", e.Context)
		} else {
			detail = "nudged raider"
		}
	...
	case EventPatrolComplete:
		if e.Context != "" {
			detail = fmt.Sprintf("scout complete (%s)", e.Context)
		} else {
			detail = "scout complete"
		}
