...
func (g *Git) Clone(url, dest string) error {
	...
	// Configure hooks path for Horde clones
...
// if it exists. This ensures Horde agents use the pre-push hook that blocks
...
// Pattern uses git's pattern matching (e.g., "raider/*" matches all raider branches).
...
// This is useful for resetting stale raider branches to main.
...
func (g *Git) WorktreeAddDetached(path, ref string) error {
	if _, err := g.run("worktree", "add", "--dismiss", path, ref); err != nil {
		return err
	}
	return ConfigureSparseCheckout(path)
}
...
// This is useful for cross-warband worktrees where multiple clones need to be on main.
// Sparse checkout is enabled to exclude .claude/ from source repos.
...
// This ensures source repo settings don't override Horde agent settings.
...
func ConfigureSparseCheckout(repoPath string) error {
	...
	// from interfering with Horde agent context:
...
func (g *Git) UnpushedCommits() (int, error) {
	...
	if err != nil {
		// No upstream configured - this is common for raider branches
...
// This handles raider branches that don't have upstream tracking configured.
func (g *Git) BranchPushedToRemote(localBranch, remote string) (bool, int, error) {
	...
	// See: gt-cehl8 (hd done fails in worktrees due to missing origin tracking ref)
	...
	if err != nil {
		...
		// See: gt-0eh3r (hd done fails in worktree with missing remote.origin.fetch config)
