...
func GetRepoRoot() (string, error) {
	...
	if home != "" {
		...
		for _, candidate := range candidates {
			if isGitRepo(candidate) && hasHordeMarker(candidate) {
				return candidate, nil
			}
		}
	}

	// Check if current directory is in a horde repo
	cmd := exec.Command("git", "rev-parse", "--show-toplevel")
	if output, err := cmd.Output(); err == nil {
		root := strings.TrimSpace(string(output))
		if hasHordeMarker(root) {
			return root, nil
		}
	}

	return "", fmt.Errorf("cannot locate hd source repository")
}
...
// hasHordeMarker checks if a directory looks like the horde repo.
func hasHordeMarker(dir string) bool {
	// Check for cmd/hd directory which is unique to horde
	cmd := exec.Command("test", "-d", dir+"/cmd/hd")
	return cmd.Run() == nil
}
