...
func TestParse_Raid(t *testing.T) {
	data := []byte(`
...
`)
	...
	if f.Name != "test-raid" {
		t.Errorf("Name = %q, want %q", f.Name, "test-raid")
	}
	if f.Type != TypeRaid {
		t.Errorf("Type = %q, want %q", f.Type, TypeRaid)
	}
	if len(f.Legs) != 2 {
...
`)
	...
	_, err := Parse(data)
	if err == nil {
		t.Error("expected error for missing ritual name")
	}
}
...
func TestRaidReadySteps(t *testing.T) {
	data := []byte(`
ritual = "test"
type = "raid"
version = 1
