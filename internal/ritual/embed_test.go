...
// TestGetEmbeddedFormulas verifies embedded rituals can be read and hashed.
func TestGetEmbeddedFormulas(t *testing.T) {
	...
	if len(embedded) == 0 {
		t.Error("should have embedded rituals")
	}

	// Verify at least one known ritual exists
	if _, ok := embedded["totem-shaman-scout.ritual.toml"]; !ok {
		t.Error("should contain totem-shaman-scout.ritual.toml")
	}
...
func TestProvisionFormulas_FreshInstall(t *testing.T) {
	...
	if count == 0 {
		t.Error("should have provisioned at least one ritual")
	}

	// Verify rituals directory was created
	formulasDir := filepath.Join(tmpDir, ".relics", "rituals")
	if _, err := os.Stat(formulasDir); os.IsNotExist(err) {
		t.Error("rituals directory should exist")
	}
	...
	if len(installed.Rituals) != count {
		t.Errorf("installed record has %d entries, want %d", len(installed.Rituals), count)
	}
}
...
func TestProvisionFormulas_SkipsExisting(t *testing.T) {
	...
	// Create rituals directory with a custom ritual
	formulasDir := filepath.Join(tmpDir, ".relics", "rituals")
	if err := os.MkdirAll(formulasDir, 0755); err != nil {
		t.Fatal(err)
	}

	customContent := []byte("# Custom user ritual\nformula = \"totem-shaman-scout\"\n")
	customPath := filepath.Join(formulasDir, "totem-shaman-scout.ritual.toml")
	...
	if string(content) != string(customContent) {
		t.Error("existing ritual should not have been overwritten")
	}
}

// TestCheckFormulaHealth_AllOK tests when all rituals are up to date.
...
// TestCheckFormulaHealth_UserModified tests detection of user-modified rituals.
func TestCheckFormulaHealth_UserModified(t *testing.T) {
	...
	// Verify the specific ritual is marked as modified
	found := false
	for _, f := range report.Rituals {
		if f.Name == "totem-shaman-scout.ritual.toml" {
			if f.Status != "modified" {
				t.Errorf("totem-shaman-scout status = %q, want %q", f.Status, "modified")
			}
			found = true
			break
		}
	...
	if !found {
		t.Error("totem-shaman-scout.ritual.toml not found in report")
	}
}

// TestCheckFormulaHealth_Missing tests detection of deleted rituals.
...
// TestCheckFormulaHealth_Outdated simulates an outdated ritual.
func TestCheckFormulaHealth_Outdated(t *testing.T) {
	...
	// Simulate "old" installed record by changing the installed hash for a ritual
	// This mimics what happens when a new binary has updated ritual content
	formulasDir := filepath.Join(tmpDir, ".relics", "rituals")
	installed, err := loadInstalledRecord(formulasDir)
	if err != nil {
		t.Fatal(err)
	...
	installed.Rituals[targetFormula] = hex.EncodeToString(hash[:])
	...
	if embeddedHash == installed.Rituals[targetFormula] {
		t.Error("embedded hash should differ from installed hash for this test")
	}
}

// TestUpdateFormulas_UpdatesOutdated tests that outdated rituals are updated.
...
// TestUpdateFormulas_SkipsModified tests that user-modified rituals are skipped.
func TestUpdateFormulas_SkipsModified(t *testing.T) {
	...
	// Modify a ritual (user customization)
	formulasDir := filepath.Join(tmpDir, ".relics", "rituals")
	installed, err := loadInstalledRecord(formulasDir)
	if err != nil {
		t.Fatal(err)
	...
	formulaPath := filepath.Join(formulasDir, targetFormula)
	modifiedContent := []byte("# User customized this ritual\nformula = \"custom\"\n")
	if err := os.WriteFile(formulaPath, modifiedContent, 0644); err != nil {
		t.Fatal(err)
	}

	// Run update - should skip the modified ritual
	...
	if string(content) != string(modifiedContent) {
		t.Error("modified ritual should not have been changed")
	}
}

// TestUpdateFormulas_ReinstallsMissing tests that deleted rituals are reinstalled.
func TestUpdateFormulas_ReinstallsMissing(t *testing.T) {
	...
	if _, err := os.Stat(formulaPath); os.IsNotExist(err) {
		t.Error("missing ritual should have been reinstalled")
	}
}

// TestUpdateFormulas_InstallsNew tests that new rituals are installed.
func TestUpdateFormulas_InstallsNew(t *testing.T) {
	...
	emptyInstalled := &InstalledRecord{Rituals: make(map[string]string)}
	if err := saveInstalledRecord(formulasDir, emptyInstalled); err != nil {
		t.Fatal(err)
	}

	// Run update - should install all rituals as "new"
	updated, skipped, reinstalled, err := UpdateFormulas(tmpDir)
	if err != nil {
		t.Fatalf("UpdateFormulas() error: %v", err)
	}

	// All rituals should be installed
	embedded, err := getEmbeddedFormulas()
	if err != nil {
		t.Fatal(err)
	}
...
func TestInstalledRecordPersistence(t *testing.T) {
	...
	if len(installed.Rituals) != count {
		t.Errorf("installed has %d rituals, want %d", len(installed.Rituals), count)
	}
...
// TestCheckFormulaHealth_NewFormula tests detection of new rituals that were never installed.
func TestCheckFormulaHealth_NewFormula(t *testing.T) {
	tmpDir := t.TempDir()

	// Create rituals directory with empty installed record
	formulasDir := filepath.Join(tmpDir, ".relics", "rituals")
	if err := os.MkdirAll(formulasDir, 0755); err != nil {
		t.Fatal(err)
	}

	// Write empty installed record - simulates pre-existing install without this ritual
	emptyInstalled := &InstalledRecord{Rituals: make(map[string]string)}
	if err := saveInstalledRecord(formulasDir, emptyInstalled); err != nil {
		t.Fatal(err)
	}

	// Check health - all embedded rituals should be "new"
	report, err := CheckFormulaHealth(tmpDir)
	if err != nil {
		t.Fatalf("CheckFormulaHealth() error: %v", err)
	}
...
// in .installed.json and don't match embedded (e.g., from older hd version).
func TestCheckFormulaHealth_Untracked(t *testing.T) {
	tmpDir := t.TempDir()

	// Get embedded rituals
	embedded, err := getEmbeddedFormulas()
	if err != nil {
		t.Fatal(err)
	}

	// Create rituals directory without .installed.json
	formulasDir := filepath.Join(tmpDir, ".relics", "rituals")
	if err := os.MkdirAll(formulasDir, 0755); err != nil {
		t.Fatal(err)
	}

	// Write ritual files with different content (simulating older version)
	...
	// Verify all rituals have status "untracked"
	for _, f := range report.Rituals {
		if f.Status != "untracked" {
			t.Errorf("ritual %s status = %q, want %q", f.Name, f.Status, "untracked")
		}
	}
}
...
func TestUpdateFormulas_UpdatesUntracked(t *testing.T) {
	...
	// Run update - should update all untracked rituals
	updated, skipped, reinstalled, err := UpdateFormulas(tmpDir)
	if err != nil {
		t.Fatalf("UpdateFormulas() error: %v", err)
	}
	...
	for name, expectedHash := range embedded {
		if installed.Rituals[name] != expectedHash {
			t.Errorf(".installed.json hash for %s = %q, want %q",
				name, installed.Rituals[name], expectedHash)
		}
	}
...
func TestCheckFormulaHealth_MixedScenarios(t *testing.T) {
	...
	if len(embedded) < 3 {
		t.Skip("need at least 3 rituals for this test")
	}
	...
	// Prepare installed record with only some rituals tracked
	installed := &InstalledRecord{Rituals: make(map[string]string)}
	...
	for name := range embedded {
		switch i {
		case 0:
			// First ritual: write matching content, track it -> should be OK
			okFormula = name
			content, _ := formulasFS.ReadFile("rituals/" + name)
			if err := os.WriteFile(filepath.Join(formulasDir, name), content, 0644); err != nil {
				t.Fatal(err)
			}
			installed.Rituals[name] = computeHash(content)

		case 1:
			// Second ritual: write old content, don't track -> should be untracked
		...
		case 2:
			// Third ritual: write different content, track with original hash -> should be modified
			modifiedFormula = name
			originalContent, _ := formulasFS.ReadFile("rituals/" + name)
			...
			installed.Rituals[name] = originalHash // Track with original hash
		}
		i++
		if i >= 3 {
			break
	...
	// Find status of each test ritual
	...
	if statusMap[okFormula] != "ok" {
		t.Errorf("ritual %s status = %q, want %q", okFormula, statusMap[okFormula], "ok")
	}
	if statusMap[untrackedFormula] != "untracked" {
		t.Errorf("ritual %s status = %q, want %q", untrackedFormula, statusMap[untrackedFormula], "untracked")
	}
	if statusMap[modifiedFormula] != "modified" {
		t.Errorf("ritual %s status = %q, want %q", modifiedFormula, statusMap[modifiedFormula], "modified")
	}
}
