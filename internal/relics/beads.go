...
var (
	ErrNotInstalled = errors.New("bd not installed: run 'pip install relics-cli' or see https://github.com/anthropics/relics")
	ErrNotFound     = errors.New("issue not found")
)

// Issue represents a relics issue.
type Issue struct {
	ID          string   `json:"id"`
	...
	BannerBead   string `json:"banner_bead,omitempty"`   // Current work attached to agent's hook
...
type ListOptions struct {
	...
	Assignee   string // filter by assignee (e.g., "horde/Toast")
...
// SyncStatus represents the sync status of the relics repository.
...
// Relics wraps rl CLI operations for a working directory.
type Relics struct {
	workDir  string
	relicsDir string // Optional RELICS_DIR override for cross-database access
}

// New creates a new Relics wrapper for the given directory.
func New(workDir string) *Relics {
	return &Relics{workDir: workDir}
}

// NewWithRelicsDir creates a Relics wrapper with an explicit RELICS_DIR.
// This is needed when running from a raider worktree but accessing encampment-level relics.
func NewWithRelicsDir(workDir, relicsDir string) *Relics {
	return &Relics{workDir: workDir, relicsDir: relicsDir}
}

// run executes a rl command and returns stdout.
func (b *Relics) run(args ...string) ([]byte, error) {
	...
	cmd := exec.Command("rl", fullArgs...) //nolint:gosec // G204: rl is a trusted internal tool
	cmd.Dir = b.workDir

	// Always explicitly set RELICS_DIR to prevent inherited env vars from
	// causing prefix mismatches. Use explicit relicsDir if set, otherwise
	// resolve from working directory.
	relicsDir := b.relicsDir
	if relicsDir == "" {
		relicsDir = ResolveRelicsDir(b.workDir)
	}
	cmd.Env = append(os.Environ(), "RELICS_DIR="+relicsDir)
	...
	// Handle rl --no-daemon exit code 0 bug: when issue not found,
...
// callers need to run arbitrary rl commands.
func (b *Relics) Run(args ...string) ([]byte, error) {
	return b.run(args...)
}

// wrapError wraps rl errors with context.
...
func (b *Relics) wrapError(err error, stderr string, args []string) error {
	stderr = strings.TrimSpace(stderr)

	// Check for rl not installed
...
func (b *Relics) List(opts ListOptions) ([]*Issue, error) {
...
// The assignee is typically in the format "warband/raiderName" (e.g., "horde/Toast").
func (b *Relics) ListByAssignee(assignee string) ([]*Issue, error) {
...
func (b *Relics) GetAssignedIssue(assignee string) (*Issue, error) {
...
func (b *Relics) Ready() ([]*Issue, error) {
	...
	var issues []*Issue
	if err := json.Unmarshal(out, &issues); err != nil {
		return nil, fmt.Errorf("parsing rl ready output: %w", err)
	}
...
// Uses rl ready --label flag for server-side filtering.
// The issueType is converted to a gt:<type> label (e.g., "totem" -> "gt:totem").
func (b *Relics) ReadyWithType(issueType string) ([]*Issue, error) {
...
func (b *Relics) Show(id string) (*Issue, error) {
...
// ShowMultiple fetches multiple issues by ID in a single rl call.
// Returns a map of ID to Issue. Missing IDs are not included in the map.
func (b *Relics) ShowMultiple(ids []string) (map[string]*Issue, error) {
	if len(ids) == 0 {
		return make(map[string]*Issue), nil
	}

	// rl show supports multiple IDs
	args := append([]string{"show", "--json"}, ids...)
	out, err := b.run(args...)
	if err != nil {
		// If rl fails, return empty map (some IDs might not exist)
		return make(map[string]*Issue), nil
	}
...
func (b *Relics) Blocked() ([]*Issue, error) {
	...
	var issues []*Issue
	if err := json.Unmarshal(out, &issues); err != nil {
		return nil, fmt.Errorf("parsing rl blocked output: %w", err)
	}
...
func (b *Relics) Create(opts CreateOptions) (*Issue, error) {
...
// This is useful for agent relics, role relics, and other relics that need
// deterministic IDs rather than auto-generated ones.
func (b *Relics) CreateWithID(id string, opts CreateOptions) (*Issue, error) {
...
func (b *Relics) Update(id string, opts UpdateOptions) error {
...
// If a runtime session ID is set in the environment, it is passed to rl close
// for work attribution tracking (see decision 009-session-events-architecture.md).
func (b *Relics) Close(ids ...string) error {
	if len(ids) == 0 {
		return nil
	}
...
// If a runtime session ID is set in the environment, it is passed to rl close
// for work attribution tracking (see decision 009-session-events-architecture.md).
func (b *Relics) CloseWithReason(reason string, ids ...string) error {
	if len(ids) == 0 {
		return nil
	}
...
func (b *Relics) Release(id string) error {
...
func (b *Relics) ReleaseWithReason(id, reason string) error {
...
func (b *Relics) AddDependency(issue, dependsOn string) error {
	_, err := b.run("dep", "add", issue, dependsOn)
	return err
}

// RemoveDependency removes a dependency.
func (b *Relics) RemoveDependency(issue, dependsOn string) error {
	_, err := b.run("dep", "remove", issue, dependsOn)
	return err
}

// Sync syncs relics with remote.
func (b *Relics) Sync() error {
	_, err := b.run("sync")
	return err
}

// SyncFromMain syncs relics updates from main branch.
func (b *Relics) SyncFromMain() error {
	_, err := b.run("sync", "--from-main")
	return err
}

// GetSyncStatus returns the sync status without performing a sync.
func (b *Relics) GetSyncStatus() (*SyncStatus, error) {
	...
	if err := json.Unmarshal(out, &status); err != nil {
		return nil, fmt.Errorf("parsing rl sync status output: %w", err)
	}
...
func (b *Relics) Stats() (string, error) {
	out, err := b.run("stats")
	if err != nil {
		return "", err
	}
...
// IsRelicsRepo checks if the working directory is a relics repository.
// ZFC: Check file existence directly instead of parsing rl errors.
func (b *Relics) IsRelicsRepo() bool {
	relicsDir := ResolveRelicsDir(b.workDir)
	info, err := os.Stat(relicsDir)
	return err == nil && info.IsDir()
}

// primeContent is the Horde RALLY.md content that provides essential context
// for clan workers. This is the fallback if the SessionStart hook fails.
const primeContent = `# Horde Worker Context
...
7. ` + "`hd done`" + ` (submit to merge queue and exit)

**Raiders MUST call ` + "`hd done`" + ` - this submits work and exits the session.**
`

// ProvisionPrimeMD writes the Horde RALLY.md file to the specified relics directory.
// This provides essential Horde context (GUPP, startup protocol) as a fallback
// if the SessionStart hook fails. The RALLY.md is read by rl rally.
//
// The relicsDir should be the actual relics directory (after following any redirect).
// Returns nil if RALLY.md already exists (idempotent).
func ProvisionPrimeMD(relicsDir string) error {
	primePath := filepath.Join(relicsDir, "RALLY.md")
	...
	if err := os.WriteFile(primePath, []byte(primeContent), 0644); err != nil {
		return fmt.Errorf("writing RALLY.md: %w", err)
	}

	return nil
}

// ProvisionPrimeMDForWorktree provisions RALLY.md for a worktree by following its redirect.
// This is the main entry point for clan/raider provisioning.
func ProvisionPrimeMDForWorktree(worktreePath string) error {
	// Resolve the relics directory (follows redirect chain)
	relicsDir := ResolveRelicsDir(worktreePath)

	// Provision RALLY.md in the target directory
	return ProvisionPrimeMD(relicsDir)
}
