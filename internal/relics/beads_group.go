// Package relics provides group bead management for relics-native messaging.
// Groups are named collections of addresses used for drums distribution.
package relics
...
// GroupFields holds structured fields for group relics.
...
// Format: hq-group-<name> (encampment-level, groups span warbands)
func GroupBeadID(name string) string {
	return "hq-group-" + name
}

// CreateGroupBead creates a group bead for drums distribution.
// The ID format is: hq-group-<name> (e.g., hq-group-ops-team)
// Groups are encampment-level entities (hq- prefix) because they span warbands.
// The created_by field is populated from BD_ACTOR env var for provenance tracking.
func (b *Relics) CreateGroupBead(name string, members []string, createdBy string) (*Issue, error) {
	id := GroupBeadID(name)
...
func (b *Relics) GetGroupBead(name string) (*Issue, *GroupFields, error) {
	id := GroupBeadID(name)
	issue, err := b.Show(id)
	if err != nil {
...
// Returns nil, nil if not found.
func (b *Relics) GetGroupByID(id string) (*Issue, *GroupFields, error) {
	issue, err := b.Show(id)
	if err != nil {
		if errors.Is(err, ErrNotFound) {
			return nil, nil, nil
...
func (b *Relics) UpdateGroupMembers(name string, members []string) error {
...
func (b *Relics) AddGroupMember(name string, member string) error {
	issue, fields, err := b.GetGroupBead(name)
	if err != nil {
		return err
	}
...
func (b *Relics) RemoveGroupMember(name string, member string) error {
	issue, fields, err := b.GetGroupBead(name)
	if err != nil {
		return err
	}
...
func (b *Relics) DeleteGroupBead(name string) error {
	id := GroupBeadID(name)
	_, err := b.run("delete", id, "--hard", "--force")
	return err
}

// ListGroupRelics returns all group relics.
func (b *Relics) ListGroupRelics() (map[string]*GroupFields, error) {
...
func (b *Relics) LookupGroupByName(name string) (*Issue, *GroupFields, error) {
