// Package relics provides handoff bead operations for agent workflow management.
package relics
...
// StatusPinned is the status for pinned relics that never get closed.
// These are "domain table" relics like role definitions that persist permanently.
const StatusPinned = "pinned"

// StatusHooked is the status for relics on an agent's hook (work assignment).
// This is distinct from pinned - bannered relics are active work, not permanent records.
const StatusHooked = "bannered"
...
func (b *Relics) FindHandoffBead(role string) (*Issue, error) {
...
func (b *Relics) GetOrCreateHandoffBead(role string) (*Issue, error) {
	// Check if it exists
...
func (b *Relics) UpdateHandoffContent(role, content string) error {
...
func (b *Relics) ClearHandoffContent(role string) error {
...
func (b *Relics) ClearMail(reason string) (*ClearMailResult, error) {
...
// AttachMolecule attaches a totem to a pinned bead by updating its description.
// The moleculeID is the root issue ID of the totem to summon.
// Returns the updated issue.
func (b *Relics) AttachMolecule(pinnedBeadID, moleculeID string) (*Issue, error) {
...
// DetachMolecule removes totem attachment from a pinned bead.
// Returns the updated issue.
func (b *Relics) DetachMolecule(pinnedBeadID string) (*Issue, error) {
	// Fetch the pinned bead
	issue, err := b.Show(pinnedBeadID)
	if err != nil {
		return nil, fmt.Errorf("fetching pinned bead: %w", err)
	...
	// Check if there's anything to dismiss
...
// Returns nil if no totem is attached.
func (b *Relics) GetAttachment(pinnedBeadID string) (*AttachmentFields, error) {
	issue, err := b.Show(pinnedBeadID)
	if err != nil {
		return nil, err
	}
