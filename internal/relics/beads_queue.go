// Package relics provides queue bead management.
package relics
...
// QueueFields holds structured fields for queue relics.
// These are stored as "key: value" lines in the description.
type QueueFields struct {
	Name            string // Queue name (human-readable identifier)
	ClaimPattern    string // Pattern for who can claim from queue (e.g., "horde/raiders/*")
...
// Format: hq-q-<name> for encampment-level queues, gt-q-<name> for warband-level queues.
...
func (b *Relics) CreateQueueBead(id, title string, fields *QueueFields) (*Issue, error) {
...
func (b *Relics) GetQueueBead(id string) (*Issue, *QueueFields, error) {
	issue, err := b.Show(id)
	if err != nil {
...
func (b *Relics) UpdateQueueFields(id string, fields *QueueFields) error {
	issue, err := b.Show(id)
	if err != nil {
		return err
	}
...
func (b *Relics) UpdateQueueCounts(id string, available, processing, completed, failed int) error {
...
func (b *Relics) UpdateQueueStatus(id, status string) error {
	// Validate status
...
// ListQueueRelics returns all queue relics.
func (b *Relics) ListQueueRelics() (map[string]*Issue, error) {
...
func (b *Relics) DeleteQueueBead(id string) error {
...
// Patterns support:
//   - "*" matches anyone
//   - "horde/raiders/*" matches any raider in horde warband
//   - "*/witness" matches any witness role across warbands
...
func MatchClaimPattern(pattern, identity string) bool {
	...
	if strings.Contains(pattern, "*") {
		...
		// "horde/raiders/*" should match "horde/raiders/capable"
		// "*/witness" should match "horde/witness"
...
// FindEligibleQueues returns all queue relics that the given identity can claim from.
func (b *Relics) FindEligibleQueues(identity string) ([]*Issue, []*QueueFields, error) {
