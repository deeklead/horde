// Package relics provides channel bead management for relics-native messaging.
...
// ChannelFields holds structured fields for channel relics.
...
// Format: hq-channel-<name> (encampment-level, channels span warbands)
...
// Channels are encampment-level entities (hq- prefix) because they span warbands.
// The created_by field is populated from BD_ACTOR env var for provenance tracking.
func (b *Relics) CreateChannelBead(name string, subscribers []string, createdBy string) (*Issue, error) {
	...
	args := []string{"create", "--json",
		...
		"--labels=gt:channel",
		"--force", // Override prefix check (encampment relics may have mixed prefixes)
	}
...
func (b *Relics) GetChannelBead(name string) (*Issue, *ChannelFields, error) {
	id := ChannelBeadID(name)
	issue, err := b.Show(id)
	if err != nil {
...
// Returns nil, nil if not found.
func (b *Relics) GetChannelByID(id string) (*Issue, *ChannelFields, error) {
	issue, err := b.Show(id)
	if err != nil {
		if errors.Is(err, ErrNotFound) {
			return nil, nil, nil
...
func (b *Relics) UpdateChannelSubscribers(name string, subscribers []string) error {
...
func (b *Relics) SubscribeToChannel(name string, subscriber string) error {
	issue, fields, err := b.GetChannelBead(name)
	if err != nil {
		return err
	}
...
func (b *Relics) UnsubscribeFromChannel(name string, subscriber string) error {
	issue, fields, err := b.GetChannelBead(name)
	if err != nil {
		return err
	}
...
func (b *Relics) UpdateChannelRetention(name string, retentionCount, retentionHours int) error {
	issue, fields, err := b.GetChannelBead(name)
	if err != nil {
		return err
	}
...
func (b *Relics) UpdateChannelStatus(name, status string) error {
	// Validate status
...
func (b *Relics) DeleteChannelBead(name string) error {
	id := ChannelBeadID(name)
	_, err := b.run("delete", id, "--hard", "--force")
	return err
}

// ListChannelRelics returns all channel relics.
func (b *Relics) ListChannelRelics() (map[string]*ChannelFields, error) {
...
func (b *Relics) LookupChannelByName(name string) (*Issue, *ChannelFields, error) {
	...
	channels, err := b.ListChannelRelics()
	if err != nil {
		return nil, nil, err
	}
...
func (b *Relics) EnforceChannelRetention(name string) error {
	// Get channel config
...
// Called by Shaman scout as a backup cleanup mechanism.
// Uses a 10% buffer to avoid thrashing (only prunes if count > retainCount * 1.1).
func (b *Relics) PruneAllChannels() (int, error) {
	channels, err := b.ListChannelRelics()
	if err != nil {
		return 0, err
	}
	...
	for name, fields := range channels {
		...
		toDelete := len(messages) - fields.RetentionCount
		for i := 0; i < toDelete && i < len(messages); i++ {
			if _, err := b.run("close", messages[i].ID, "--reason=scout retention pruning"); err == nil {
				pruned++
			}
		}
	}
