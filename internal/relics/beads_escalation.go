// Package relics provides escalation bead management.
package relics
...
// EscalationFields holds structured fields for escalation relics.
// These are stored as "key: value" lines in the description.
type EscalationFields struct {
	Severity           string // critical, high, medium, low
	Reason             string // Why this was escalated
	Source             string // Source identifier (e.g., plugin:rebuild-gt, scout:shaman)
	EscalatedBy        string // Agent address that escalated (e.g., "horde/Toast")
...
func (b *Relics) CreateEscalationBead(title string, fields *EscalationFields) (*Issue, error) {
...
func (b *Relics) AckEscalation(id, ackedBy string) error {
...
func (b *Relics) CloseEscalation(id, closedBy, reason string) error {
	// First get current issue to preserve other fields
	issue, err := b.Show(id)
	if err != nil {
		return err
...
func (b *Relics) GetEscalationBead(id string) (*Issue, *EscalationFields, error) {
	issue, err := b.Show(id)
	if err != nil {
...
// ListEscalations returns all open escalation relics.
func (b *Relics) ListEscalations() ([]*Issue, error) {
...
// ListEscalationsBySeverity returns open escalation relics filtered by severity.
func (b *Relics) ListEscalationsBySeverity(severity string) ([]*Issue, error) {
	out, err := b.run("list",
		"--label=gt:escalation",
...
func (b *Relics) ListStaleEscalations(threshold time.Duration) ([]*Issue, error) {
...
func (b *Relics) ReescalateEscalation(id, reescalatedBy string, maxReescalations int) (*ReescalationResult, error) {
	// Get the escalation
