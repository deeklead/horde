...
// BdDaemonInfo represents the status of a single rl daemon instance.
...
// BdDaemonHealth represents the overall health of rl daemons.
...
// CheckBdDaemonHealth checks the health of all rl daemons.
// Returns nil if no daemons are running (which is fine, rl will use direct mode).
func CheckBdDaemonHealth() (*BdDaemonHealth, error) {
	cmd := exec.Command("rl", "daemon", "health", "--json")
	...
	err := cmd.Run()
	if err != nil {
		// rl daemon health may fail if rl not installed or other issues
...
// EnsureBdDaemonHealth checks if rl daemons are healthy and attempts to restart if needed.
...
func EnsureBdDaemonHealth(workDir string) string {
	...
	// No daemons running is fine - rl will use direct mode
	if health.Total == 0 {
		return ""
	}
...
// restartBdDaemons restarts all rl daemons.
func restartBdDaemons() error { //nolint:unparam // error return kept for future use
	...
	// The daemon will auto-start when rl commands are run in those directories
	// Just running any rl command will trigger daemon startup if configured
	return nil
}

// StartBdDaemonIfNeeded starts the rl daemon for a specific workspace if not running.
...
func StartBdDaemonIfNeeded(workDir string) error {
	cmd := exec.Command("rl", "daemon", "--start")
	cmd.Dir = workDir
	return cmd.Run()
}

// StopAllBdProcesses stops all rl daemon and activity processes.
...
// CountBdDaemons returns count of running rl daemons.
...
func stopBdDaemons(force bool) (int, int) {
	...
	// daemon auto-start as a side effect of running rl commands.
...
// CountBdActivityProcesses returns count of running `rl activity` processes.
