...
func TestFormatGroupDescription(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			fields: &GroupFields{
				Name:      "ops-team",
				Members:   []string{"horde/clan/max", "horde/witness"},
				CreatedBy: "human",
			...
			want: []string{
				"Group: ops-team",
				"name: ops-team",
				"members: horde/clan/max,horde/witness",
				"created_by: human",
		...
		},
		{
			...
			fields: &GroupFields{
				Name:    "all-witnesses",
				Members: []string{"*/witness", "@clan"},
			},
			want: []string{
				"members: */witness,@clan",
			},
		},
		{
			name:   "nil fields",
...
name: ops-team
members: horde/clan/max,horde/witness,*/forge
created_by: human
created_at: 2024-01-15T10:00:00Z`,
			want: &GroupFields{
				Name:      "ops-team",
				Members:   []string{"horde/clan/max", "horde/witness", "*/forge"},
				CreatedBy: "human",
				CreatedAt: "2024-01-15T10:00:00Z",
			},
		},
...
members: horde/clan/max`,
			want: &GroupFields{
				Name:    "solo",
				Members: []string{"horde/clan/max"},
			},
		},
		{
...
func TestGroupBeadID(t *testing.T) {
	tests := []struct {
		...
		{"all", "hq-group-all"},
		{"clan-leads", "hq-group-clan-leads"},
	}
...
func TestRoundTrip(t *testing.T) {
	...
	original := &GroupFields{
		Name:      "test-group",
		Members:   []string{"horde/clan/max", "*/witness", "@encampment"},
		CreatedBy: "tester",
