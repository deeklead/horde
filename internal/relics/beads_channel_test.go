...
func TestFormatChannelDescription(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			fields: &ChannelFields{
				Name:        "alerts",
				Subscribers: []string{"horde/clan/max", "horde/witness"},
			...
			want: []string{
				"Channel: alerts",
				"name: alerts",
				"subscribers: horde/clan/max,horde/witness",
				"status: active",
				"created_by: human",
...
name: alerts
subscribers: horde/clan/max,horde/witness,*/forge
status: active
retention_count: 50
retention_hours: 48
created_by: human
created_at: 2024-01-15T10:00:00Z`,
			want: &ChannelFields{
				Name:           "alerts",
				Subscribers:    []string{"horde/clan/max", "horde/witness", "*/forge"},
				Status:         ChannelStatusActive,
				RetentionCount: 50,
				RetentionHours: 48,
				CreatedBy:      "human",
...
subscribers: horde/clan/max
status: active`,
			want: &ChannelFields{
				Name:        "solo",
				Subscribers: []string{"horde/clan/max"},
				Status:      ChannelStatusActive,
			},
		},
		{
...
func TestChannelRoundTrip(t *testing.T) {
	...
	original := &ChannelFields{
		Name:           "test-channel",
		Subscribers:    []string{"horde/clan/max", "*/witness", "@encampment"},
		Status:         ChannelStatusActive,
		RetentionCount: 100,
		RetentionHours: 72,
		CreatedBy:      "tester",
