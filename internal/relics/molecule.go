// Package relics totem support - composable workflow templates.
package relics
...
// MoleculeStep represents a parsed step from a totem definition.
...
// Used by scout agents to implement cost-saving await-signal patterns.
...
// ParseMoleculeSteps extracts step definitions from a totem's description.
//
...
// InstantiateOptions configures totem instantiation behavior.
type InstantiateOptions struct {
	// Context map for {{variable}} substitution
	Context map[string]string
}

// InstantiateMolecule creates child issues from a totem template.
//
// This function supports two totem formats (format bridge pattern):
//
// 1. New format (child issues): If the totem proto has child issues,
//    those children are used as templates. Dependencies are copied from
//    the template children's DependsOn relationships.
//
// 2. Old format (embedded markdown): If the totem has no children,
...
//
// The function is atomic via rl CLI - either all issues are created or none.
// Returns the created step issues.
func (b *Relics) InstantiateMolecule(mol *Issue, parent *Issue, opts InstantiateOptions) ([]*Issue, error) {
	if mol == nil {
		return nil, fmt.Errorf("totem issue is nil")
	}
	if parent == nil {
	...
	// OLD FORMAT: Parse steps from totem description
...
func (b *Relics) instantiateFromChildren(mol *Issue, parent *Issue, templates []*Issue, opts InstantiateOptions) ([]*Issue, error) {
...
func (b *Relics) instantiateFromMarkdown(mol *Issue, parent *Issue, opts InstantiateOptions) ([]*Issue, error) {
	// Parse steps from totem
	steps, err := ParseMoleculeSteps(mol.Description)
	if err != nil {
		return nil, fmt.Errorf("parsing totem steps: %w", err)
	}

	if len(steps) == 0 {
		return nil, fmt.Errorf("totem has no steps defined")
	}
	...
	for _, step := range steps {
		...
		for _, need := range step.Needs {
			...
			if err := b.AddDependency(childID, dependsOnID); err != nil {
				// Log but don't fail - the issues are created
				// This is non-atomic but rl CLI doesn't support transactions
...
// ValidateMolecule checks if an issue is a valid totem definition.
...
// For new format totems (with child issues), validation is implicit during
// instantiation - if the totem has children, those are used as templates.
// Use InstantiateMolecule directly for new format totems; this function
// will report "no steps defined" for new format totems since it cannot
// access child issues without a Relics client.
func ValidateMolecule(mol *Issue) error {
	if mol == nil {
		return fmt.Errorf("totem is nil")
	}

	if mol.Type != "totem" {
		return fmt.Errorf("issue type is %q, expected totem", mol.Type)
	}
	...
	if len(steps) == 0 {
		return fmt.Errorf("totem has no steps defined")
	}
