// Package boot manages the Boot watchdog - the daemon's entry point for Shaman triage.
// Boot is a dog that runs fresh on each daemon tick, deciding whether to wake/signal/interrupt
// the Shaman or let it continue. This centralizes the "when to wake" decision in an agent.
package boot
...
type Status struct {
	...
	LastAction  string    `json:"last_action,omitempty"` // start/wake/signal/nothing
	Target      string    `json:"target,omitempty"`      // shaman, witness, etc.
...
type Boot struct {
	townRoot  string
	bootDir   string // ~/horde/shaman/dogs/boot/
	shamanDir string // ~/horde/shaman/
	tmux      *tmux.Tmux
	degraded  bool
}
...
// Muster starts Boot in a fresh tmux session.
// Boot runs the totem-boot-triage totem and exits when done.
...
func (b *Boot) Muster(agentOverride string) error {
	if b.IsRunning() {
...
func (b *Boot) spawnTmux(agentOverride string) error {
	...
	// The "hd boot triage" prompt tells Boot to immediately start triage (GUPP principle)
	var startCmd string
	if agentOverride != "" {
		var err error
		startCmd, err = config.BuildAgentStartupCommandWithAgentOverride("boot", "", b.townRoot, "", "hd boot triage", agentOverride)
		if err != nil {
			return fmt.Errorf("building startup command with agent override: %w", err)
		}
	} else {
		startCmd = config.BuildAgentStartupCommand("boot", "", b.townRoot, "", "hd boot triage")
	}
...
func (b *Boot) spawnDegraded() error {
	// In degraded mode, we run hd boot triage directly
	// This performs the triage logic without a full Claude session
	cmd := exec.Command("hd", "boot", "triage", "--degraded")
	cmd.Dir = b.shamanDir
...
// ShamanDir returns the Shaman's directory.
func (b *Boot) ShamanDir() string {
	return b.shamanDir
}
