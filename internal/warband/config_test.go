...
func TestGetConfig_SystemDefaults(t *testing.T) {
	// Create a temp warband with no wisp or bead config
	tmpDir := t.TempDir()
	...
	result := warband.GetConfigWithSource("status")
	...
	if !warband.GetBoolConfig("auto_restart") {
		t.Error("expected auto_restart to be true by default")
	}

	// Test int default
	maxRaiders := warband.GetIntConfig("max_raiders")
	if maxRaiders != 10 {
		t.Errorf("expected max_raiders=10, got %d", maxRaiders)
	}
}
...
func TestGetConfig_WispBlocked(t *testing.T) {
	...
	result := warband.GetConfigWithSource("auto_restart")
...
func TestGetIntConfig_Stacking(t *testing.T) {
	...
	result := warband.GetIntConfig("priority_adjustment")
	if result != 5 {
...
func TestGetBoolConfig_StringConversion(t *testing.T) {
	...
	if !warband.GetBoolConfig("custom_bool") {
...
func TestGetConfig_UnknownKey(t *testing.T) {
	...
	result := warband.GetConfigWithSource("nonexistent_key")
...
func TestGetStringConfig(t *testing.T) {
	...
	status := warband.GetStringConfig("status")
	if status != "operational" {
		t.Errorf("expected status='operational', got %s", status)
	}

	// Unknown key
	unknown := warband.GetStringConfig("nonexistent")
	if unknown != "" {
...
// TestGetConfig_BeadLabel tests reading config from warband bead labels.
// This requires a more complex setup with a full relics database.
func TestGetConfig_BeadLabel(t *testing.T) {
	...
	// Note: This test demonstrates the structure but rl Show requires
	// a proper relics database. In production, use rl commands or mocks.
	// For now, we test that getBeadLabel returns nil gracefully when
	// relics is not fully set up.
	...
	// Without full relics setup, should fall back to system defaults
	result := warband.GetConfigWithSource("status")
	// Either SourceBead (if relics is set up) or SourceSystem
