...
func TestHealthCheckStateFile(t *testing.T) {
	path := HealthCheckStateFile("/tmp/test-encampment")
	expected := "/tmp/test-encampment/shaman/health-check-state.json"
	if path != expected {
...
func TestSaveAndLoadHealthCheckState(t *testing.T) {
	...
	// Verify loaded data
	agent := loaded.Agents["horde/raiders/max"]
	if agent == nil {
...
func TestSaveHealthCheckState_CreatesDirectory(t *testing.T) {
	tmpDir := t.TempDir()
	nestedDir := filepath.Join(tmpDir, "nonexistent", "shaman")
