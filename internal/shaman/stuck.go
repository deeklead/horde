...
// LoadStuckConfig loads stuck detection config from the Shaman's role bead.
// Returns defaults if no role bead exists or if fields aren't configured.
// Per ZFC: agents control their own thresholds via their role relics.
func LoadStuckConfig(townRoot string) *StuckConfig {
	config := DefaultStuckConfig()

	// Load from hq-shaman-role bead
...
type AgentHealthState struct {
	// AgentID is the identifier (e.g., "horde/raiders/max" or "shaman")
	AgentID string `json:"agent_id"`

	// LastPingTime is when we last sent a HEALTH_CHECK signal
...
func HealthCheckStateFile(townRoot string) string {
	return filepath.Join(townRoot, "shaman", "health-check-state.json")
}
