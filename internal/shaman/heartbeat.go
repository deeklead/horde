...
// The Shaman is a Claude agent that monitors Warchief and Witnesses,
// handles lifecycle requests, and keeps Horde running.
package shaman
...
// Heartbeat represents the Shaman's heartbeat file contents.
// Written by the Shaman on each wake cycle.
...
type Heartbeat struct {
	...
	// LastAction describes what the Shaman did in this cycle.
...
// HeartbeatFile returns the path to the Shaman heartbeat file.
func HeartbeatFile(townRoot string) string {
	return filepath.Join(townRoot, "shaman", "heartbeat.json")
}

// WriteHeartbeat writes a new heartbeat to disk.
// Called by the Shaman at the start of each wake cycle.
...
// ReadHeartbeat reads the Shaman heartbeat from disk.
...
// A fresh heartbeat means the Shaman is actively working or recently finished.
...
// A stale heartbeat may indicate the Shaman is doing a long operation.
...
// A very stale heartbeat means the Shaman should be poked.
func (hb *Heartbeat) IsVeryStale() bool {
	return hb == nil || hb.Age() >= 15*time.Minute
}

// ShouldPoke returns true if the daemon should poke the Shaman.
// The Shaman should be poked if:
// - No heartbeat exists
