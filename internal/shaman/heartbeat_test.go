...
func TestHeartbeatFile(t *testing.T) {
	...
	expected := filepath.Join(townRoot, "shaman", "heartbeat.json")
...
func TestWriteReadHeartbeat(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "shaman-test-*")
	if err != nil {
		t.Fatal(err)
	}
...
func TestWriteHeartbeat_CreatesDirectory(t *testing.T) {
	...
	// Ensure shaman directory doesn't exist
	shamanDir := filepath.Join(tmpDir, "shaman")
	if _, err := os.Stat(shamanDir); !os.IsNotExist(err) {
		t.Fatal("shaman directory should not exist initially")
	}
	...
	if _, err := os.Stat(shamanDir); err != nil {
		t.Errorf("shaman directory should exist: %v", err)
	}
}
