...
func TestRaidTemplate_RendersRaidList(t *testing.T) {
	...
	err = tmpl.ExecuteTemplate(&buf, "raid.html", data)
	if err != nil {
	...
	// Check raid IDs are rendered
	if !strings.Contains(output, "hq-cv-abc") {
		t.Error("Template should contain raid ID hq-cv-abc")
	}
	if !strings.Contains(output, "hq-cv-def") {
		t.Error("Template should contain raid ID hq-cv-def")
	}
...
func TestRaidTemplate_LastActivityColors(t *testing.T) {
	tmpl, err := LoadTemplates()
	if err != nil {
		t.Fatalf("LoadTemplates() error = %v", err)
	}
...
func TestRaidTemplate_HtmxAutoRefresh(t *testing.T) {
	tmpl, err := LoadTemplates()
	if err != nil {
		t.Fatalf("LoadTemplates() error = %v", err)
	}
...
func TestRaidTemplate_ProgressDisplay(t *testing.T) {
	tmpl, err := LoadTemplates()
	if err != nil {
		t.Fatalf("LoadTemplates() error = %v", err)
	}
...
func TestRaidTemplate_StatusIndicators(t *testing.T) {
	tmpl, err := LoadTemplates()
	if err != nil {
		t.Fatalf("LoadTemplates() error = %v", err)
	}
...
func TestRaidTemplate_EmptyState(t *testing.T) {
	tmpl, err := LoadTemplates()
	if err != nil {
		t.Fatalf("LoadTemplates() error = %v", err)
	}
	...
	if !strings.Contains(output, "No raids") {
		t.Error("Template should show empty state message when no raids")
	}
}
