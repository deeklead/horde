// Package web provides HTTP server and templates for the Horde warmap.
package web
...
// RaidData represents data passed to the raid template.
...
// RaiderRow represents a raider worker in the warmap.
...
type MergeQueueRow struct {
	Number     int
	Repo       string // Short repo name (e.g., "roxas", "horde")
	Title      string
	URL        string
...
// RaidRow represents a single raid in the warmap.
type RaidRow struct {
	ID            string
	Title         string
	Status        string // "open" or "closed" (raw relics status)
...
// TrackedIssue represents an issue tracked by a raid.
...
// statusClass returns the CSS class for a raid status.
