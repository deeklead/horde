...
//
// These tests launch a real browser (Chromium) to verify the raid warmap
...
// mockFetcher implements RaidFetcher for testing
type mockFetcher struct {
	raids []RaidRow
}

func (m *mockFetcher) FetchRaids() ([]RaidRow, error) {
	return m.raids, nil
}

// TestBrowser_RaidListLoads tests that the raid list page loads correctly
func TestBrowser_RaidListLoads(t *testing.T) {
	...
	handler, err := NewRaidHandler(fetcher)
	if err != nil {
	...
	if !strings.Contains(title, "Horde") {
		t.Fatalf("Expected title to contain 'Horde', got: %s", title)
	}

	// Verify raid IDs are displayed
	bodyText := page.MustElement("body").MustText()
	if !strings.Contains(bodyText, "hq-cv-abc") {
		t.Error("Expected raid ID hq-cv-abc in page")
	}
	if !strings.Contains(bodyText, "hq-cv-def") {
		t.Error("Expected raid ID hq-cv-def in page")
	}
	...
	t.Log("PASSED: Raid list loads correctly")
}
...
func TestBrowser_LastActivityColors(t *testing.T) {
	// Setup test server with raids at different activity ages
	fetcher := &mockFetcher{
		raids: []RaidRow{
			{
				ID:           "hq-cv-green",
...
// TestBrowser_EmptyState tests the empty state when no raids exist
func TestBrowser_EmptyState(t *testing.T) {
	fetcher := &mockFetcher{
		raids: []RaidRow{}, // Empty raid list
	}
	...
	if !strings.Contains(bodyText, "No raids") {
		t.Errorf("Expected 'No raids' empty state message, got: %s", bodyText[:min(len(bodyText), 500)])
	}

	// Verify help text is shown
	if !strings.Contains(bodyText, "hd raid create") {
		t.Error("Expected help text with 'hd raid create' command")
	}
...
func TestBrowser_StatusIndicators(t *testing.T) {
	...
	if !strings.Contains(html, "status-open") {
		t.Error("Expected status-open class for open raid")
	}
	if !strings.Contains(html, "status-closed") {
		t.Error("Expected status-closed class for closed raid")
	}
