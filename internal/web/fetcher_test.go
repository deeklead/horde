...
func TestGetForgeStatusHint(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := f.getForgeStatusHint(tt.mergeQueueCount)
			if got != tt.want {
				t.Errorf("getForgeStatusHint(%d) = %q, want %q",
...
func TestParseRaiderSessionName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			warband, raider, ok := parseRaiderSessionName(tt.sessionName)
			if ok != tt.wantOk {
				t.Errorf("parseRaiderSessionName(%q) ok = %v, want %v",
					tt.sessionName, ok, tt.wantOk)
			}
			if ok && (warband != tt.wantRig || raider != tt.wantRaider) {
				t.Errorf("parseRaiderSessionName(%q) = (%q, %q), want (%q, %q)",
					tt.sessionName, warband, raider, tt.wantRig, tt.wantRaider)
			}
		})
	}
}
...
func TestIsWorkerSession(t *testing.T) {
	tests := []struct {
		...
		{"raider dag is worker", "dag", true},
		{"raider nux is worker", "nux", true},
		{"forge is worker", "forge", true},
		{"witness is not worker", "witness", false},
		{"warchief is not worker", "warchief", false},
		{"shaman is not worker", "shaman", false},
		{"boot is not worker", "boot", false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := isWorkerSession(tt.raider)
			if got != tt.wantWork {
				t.Errorf("isWorkerSession(%q) = %v, want %v",
					tt.raider, got, tt.wantWork)
			}
		})
	}
}
