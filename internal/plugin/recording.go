...
func (r *Recorder) RecordRun(record PluginRunRecord) (string, error) {
	...
	if record.RigName != "" {
		labels = append(labels, fmt.Sprintf("warband:%s", record.RigName))
	}

	// Build rl create command
	args := []string{
		"create",
		"--ephemeral",
		"--json",
	...
	cmd := exec.Command("rl", args...) //nolint:gosec // G204: rl is a trusted internal tool
	cmd.Dir = r.townRoot
	// Set RELICS_DIR explicitly to prevent inherited env vars from causing
	// prefix mismatches when redirects are in play.
	cmd.Env = append(os.Environ(), "RELICS_DIR="+relics.ResolveRelicsDir(r.townRoot))
	...
	if err := json.Unmarshal(stdout.Bytes(), &result); err != nil {
		return "", fmt.Errorf("parsing rl create output: %w", err)
	}
...
func (r *Recorder) queryRuns(pluginName string, limit int, since string) ([]*PluginRunBead, error) {
	...
	if since != "" {
		...
		// rl supports relative dates with - prefix (e.g., -1h, -24h)
		sinceArg := since
	...
	if err := json.Unmarshal(stdout.Bytes(), &relics); err != nil {
	...
	runs := make([]*PluginRunBead, 0, len(relics))
