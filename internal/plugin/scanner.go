...
// Scanner discovers plugins in encampment and warband directories.
type Scanner struct {
	townRoot string
	rigNames []string
}
...
// Encampment-level plugins are scanned first, then warband-level plugins.
// Plugins are deduplicated by name (warband-level overrides encampment-level).
func (s *Scanner) DiscoverAll() ([]*Plugin, error) {
	pluginMap := make(map[string]*Plugin)

	// Scan encampment-level plugins first
	townPlugins, err := s.scanTownPlugins()
	if err != nil {
		return nil, fmt.Errorf("scanning encampment plugins: %w", err)
	}
	for _, p := range townPlugins {
		pluginMap[p.Name] = p
	}

	// Scan warband-level plugins (override encampment-level by name)
	for _, rigName := range s.rigNames {
		rigPlugins, err := s.scanRigPlugins(rigName)
		if err != nil {
			// Log warning but continue with other warbands
			fmt.Fprintf(os.Stderr, "Warning: scanning plugins for warband %q: %v\n", rigName, err)
			continue
		}
...
// scanTownPlugins scans the encampment-level plugins directory.
...
// scanRigPlugins scans a warband's plugins directory.
...
// Searches warband-level plugins first (more specific), then encampment-level.
func (s *Scanner) GetPlugin(name string) (*Plugin, error) {
	// Search warband-level plugins first
	for _, rigName := range s.rigNames {
	...
	// Search encampment-level plugins
