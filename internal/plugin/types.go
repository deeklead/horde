// Package plugin provides plugin discovery and management for Horde.
//
// Plugins are periodic automation tasks that run during Shaman scout cycles.
// Each plugin is defined by a plugin.md file with TOML frontmatter.
//
// Plugin locations:
//   - Encampment-level: ~/horde/plugins/ (universal, apply everywhere)
//   - Warband-level: <warband>/plugins/ (project-specific)
package plugin
...
type Plugin struct {
	...
	// RigName is set for warband-level plugins (empty for encampment-level).
...
const (
	// LocationTown indicates a encampment-level plugin (~/horde/plugins/).
	LocationTown Location = "encampment"

	// LocationRig indicates a warband-level plugin (<warband>/plugins/).
	LocationRig Location = "warband"
)
