...
func TestPluginRunRecord(t *testing.T) {
	...
	if record.RigName != "horde" {
		t.Errorf("expected warband name 'horde', got %q", record.RigName)
	}
...
func TestNewRecorder(t *testing.T) {
	recorder := NewRecorder("/tmp/test-encampment")
	if recorder == nil {
		t.Fatal("NewRecorder returned nil")
	}
	if recorder.townRoot != "/tmp/test-encampment" {
		t.Errorf("expected townRoot '/tmp/test-encampment', got %q", recorder.townRoot)
	}
}

// Integration tests for RecordRun, GetLastRun, GetRunsSince require
// a working relics installation and are skipped in unit tests.
