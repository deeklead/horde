// ABOUTME: Shell integration installation and removal for Horde.
...
const (
	markerStart = "# --- Horde Integration (managed by gt) ---"
	markerEnd   = "# --- End Horde ---"
)
...
func updateRCFile(path, content string) error {
	...
	endIdx := strings.Index(content[startIdx:], markerEnd)
	if endIdx == -1 {
		return fmt.Errorf("malformed Horde block in %s", path)
	}
	endIdx += startIdx + len(markerEnd)
...
# Horde Shell Integration
# Installed by: hd install --shell
# Location: ~/.config/horde/shell-hook.sh
...
_horde_ignored() {
    local dir="$PWD"
    while [[ "$dir" != "/" ]]; do
        [[ -f "$dir/.horde-ignore" ]] && return 0
        dir="$(dirname "$dir")"
    done
    return 1
}

_horde_already_asked() {
    local repo_root="$1"
    local asked_file="$HOME/.cache/horde/asked-repos"
...
_horde_offer_add() {
    ...
    _horde_already_asked "$repo_root" && return 0
    ...
    echo ""
    echo -n "Add '$repo_name' to Horde? [y/N/never] "
    read -r response </dev/tty
    
    _horde_mark_asked "$repo_root"
    
    case "$response" in
        y|Y|yes)
            echo "Adding to Horde..."
            local output
            output=$(hd warband quick-add "$repo_root" --yes 2>&1)
            local exit_code=$?
            echo "$output"
            ...
            ;;
        never)
            touch "$repo_root/.horde-ignore"
            echo "Created .horde-ignore - won't ask again for this repo."
            ;;
        *)
            echo "Skipped. Run 'hd warband quick-add' later to add manually."
            ;;
    esac
}
...
_horde_hook() {
    ...
    local cache_file="$HOME/.cache/horde/warbands.cache"
...
case "${SHELL##*/}" in
    zsh)
        ...
        add-zsh-hook chpwd _horde_chpwd_hook
        add-zsh-hook precmd _horde_hook
        ;;
    bash)
        if [[ ";${PROMPT_COMMAND[*]:-};" != *";_horde_hook;"* ]]; then
            PROMPT_COMMAND="_horde_chpwd_hook${PROMPT_COMMAND:+;$PROMPT_COMMAND}"
        fi
        ;;
esac
