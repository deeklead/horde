...
func TestTownSessions(t *testing.T) {
	...
	// Verify order is correct (Warchief, Boot, Shaman)
	expectedOrder := []string{"Warchief", "Boot", "Shaman"}
...
func TestTownSession_ShutdownOrder(t *testing.T) {
	// Verify that shutdown order is Warchief -> Boot -> Shaman
	// This is critical because Boot monitors Shaman
	sessions := TownSessions()

	if sessions[0].Name != "Warchief" {
		t.Errorf("First session should be Warchief, got %q", sessions[0].Name)
	}
	if sessions[1].Name != "Boot" {
		t.Errorf("Second session should be Boot, got %q", sessions[1].Name)
	}
	if sessions[2].Name != "Shaman" {
		t.Errorf("Third session should be Shaman, got %q", sessions[2].Name)
	}
}
