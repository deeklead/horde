...
type TownSession struct {
	Name      string // Display name (e.g., "Warchief")
	SessionID string // Tmux session ID (e.g., "hq-warchief")
}

// TownSessions returns the list of encampment-level sessions in shutdown order.
// Order matters: Boot (Shaman's watchdog) must be stopped before Shaman,
// otherwise Boot will try to restart Shaman.
func TownSessions() []TownSession {
	return []TownSession{
		{"Warchief", WarchiefSessionName()},
		{"Boot", boot.SessionName},
		{"Shaman", ShamanSessionName()},
	}
}

// StopTownSession stops a single encampment-level tmux session.
...
func stopTownSessionInternal(t *tmux.Tmux, ts TownSession, force bool) (bool, error) {
	...
	_ = events.LogFeed(events.TypeSessionDeath, ts.SessionID,
		events.SessionDeathPayload(ts.SessionID, ts.Name, reason, "hd down"))
