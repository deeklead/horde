...
> Horde's architecture explained through enterprise AI challenges
...
Horde treats work as structured data. Every action is recorded. Every agent
...
**The solution:** Every Horde agent has a distinct identity. Every action is
attributed:

```
Git commits:    horde/raiders/toast <owner@example.com>
Relics records:  created_by: horde/clan/joe
Event logs:     actor: horde/raiders/nux
```
...
bd audit --actor=horde/raiders/toast

# Success rate on Go projects
bd stats --actor=horde/raiders/toast --tag=go
```
...
```
depends_on:
  relics://github/acme/backend/be-456  # Backend API
  relics://github/acme/shared/sh-789   # Shared types
```
...
hd remote add partner hop://partner.com/their-project
...
[14:32:08] + scout-x7k.arm-ace bonded (5 steps)
[14:32:09] → scout-x7k.arm-ace.capture in_progress
[14:32:10] ✓ scout-x7k.arm-ace.capture completed
[14:32:14] ✓ scout-x7k.arm-ace.decide completed
[14:32:17] ✓ scout-x7k.arm-ace COMPLETE
```
...
Horde is a developer tool - like an IDE, but for AI orchestration. However,
...
Horde is built to answer the questions enterprises will ask as AI agents
