...
A durability proof demonstrating Horde's ability to execute arbitrarily long
sequential workflows with crash recovery and session cycling.

## What This Proves

1. **Large Totem Creation**: Creating 1000+ issues in a single workflow
...
| Disks | Moves   | Ritual Size | Est. Runtime |
|-------|---------|--------------|--------------|
| 7     | 127     | ~19 KB       | ~14 sec      |
| 9     | 511     | ~74 KB       | ~1 min       |
...
## Pre-Generated Rituals

Located in `.relics/rituals/`:

- `towers-of-hanoi-7.ritual.toml` - 127 moves (quick test)
- `towers-of-hanoi-9.ritual.toml` - 511 moves (medium test)
- `towers-of-hanoi-10.ritual.toml` - 1023 moves (standard demo)
...
while read id; do rl --no-daemon close "$id" >/dev/null; done < /tmp/ids.txt
...
# Time the execution
START=$(date +%s)
while read id; do rl --no-daemon close "$id" >/dev/null 2>&1; done < /tmp/ids.txt
END=$(date +%s)
...
The demo uses wisps (ephemeral totems) because:
...
### `rl ready` Excludes Wisps

By design, `rl ready` filters out ephemeral issues:
```go
...
## Generating Larger Rituals

Use the generator script:

```bash
# Generate 15-disk ritual (32K moves)
python3 scripts/gen_hanoi.py 15 > .relics/rituals/towers-of-hanoi-15.ritual.toml
```

**Warning**: 20-disk ritual is ~163MB and creates 1M+ issues. Only for
...
hd handoff -s "Hanoi demo" -m "Wisp: $WISP, progress: 400/1025"
```

```bash
# Session 2: Resume where you left off
# (Read handoff drums for wisp ID)
bd list --parent=$WISP --status=open --limit=2000 --json | jq -r '.[].id' > /tmp/ids.txt
# ... continue closing ...
```

The totem IS the state. No memory of previous session needed.
