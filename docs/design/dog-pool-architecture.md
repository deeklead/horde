...
Boot needs to run multiple shutdown-dance totems concurrently when multiple death
warrants are issued. The current hook design only allows one totem per agent.

Example scenario:
- Warrant 1: Kill stuck raider Toast (60s into interrogation)
- Warrant 2: Kill stuck raider Shadow (just started)
...
type Dog struct {
    ...
    StartedAt time.Time
    StateFile string            // Persistent state: ~/horde/shaman/dogs/active/<id>.json
}
...
type Warrant struct {
    ...
    Target    string    // Session to interrogate (e.g., "gt-horde-Toast")
...
type DogPool struct {
    ...
    stateDir string           // ~/horde/shaman/dogs/active/
}
...
Each active dog writes state to `~/horde/shaman/dogs/active/<id>.json`:
...
1. **Polling**: `hd dog status --active` every tick
...
### Not Reusing Raider Infrastructure

**Decision**: Dogs do NOT reuse raider infrastructure.

Rationale:
- Raiders are Claude sessions with totems, banners, sandboxes
- Dogs are simple state machine executors
- Raiders have 3-layer lifecycle (session/sandbox/slot)
...
1. State files persist in `~/horde/shaman/dogs/active/`
...
# Output:
# Pending Warrants: 2
#   1. gt-abc: witness-horde (stuck_no_progress)
#   2. gt-def: raider-Copper (crash_loop)
```

## Integration with Existing Dogs

The existing `dog` package (`internal/dog/`) manages Shaman's multi-warband helper dogs.
Those are different from shutdown-dance dogs:

| Aspect          | Helper Dogs (existing)      | Dance Dogs (new)           |
|-----------------|-----------------------------|-----------------------------|
| Purpose         | Cross-warband infrastructure    | Shutdown dance execution    |
| Sessions        | Claude sessions             | Goroutines (no Claude)      |
| Worktrees       | One per warband                 | None                        |
...
| Reuse raider infrastructure? | No - too heavyweight, different model |
