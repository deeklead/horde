...
> Multi-workspace coordination for Horde and Relics

## Overview

Federation enables multiple Horde instances to reference each other's work,
...
Level 2: Chain     - Workspace/encampment per entity
Level 3: Work Unit - Issues, tasks, totems on chains
```
...
```
hop://entity/chain/warband/issue-id
hop://steve@example.com/main-encampment/greenplace/gp-xyz
```

Cross-repo reference (same platform):

```
relics://platform/org/repo/issue-id
relics://github/acme/backend/ac-123
```
...
greenplace/gp-xyz  # Different warband, same chain
./gp-xyz           # Explicit current-warband ref
```

See `~/horde/docs/hop/GRAPH-ARCHITECTURE.md` for full URI specification.
...
```json
{
  "references": [
    {
      "type": "depends_on",
      "target": "hop://other-entity/chain/warband/issue-id"
    }
  ]
}
```
...
```json
{
  ...
  "child": "hop://alice@example.com/encampment/greenplace/gp-xyz",
...
GIT_AUTHOR_NAME="greenplace/clan/joe"
GIT_AUTHOR_EMAIL="steve@example.com"  # Workspace owner
```

Result: `abc123 Fix bug (greenplace/clan/joe <steve@example.com>)`

### Relics Operations

```bash
BD_ACTOR="greenplace/clan/joe"  # Set in agent environment
...
```json
{
  ...
  "type": "charge",
  "actor": "greenplace/clan/joe",
  "payload": { "bead": "gp-xyz", "target": "greenplace/raiders/Toast" }
}
```
...
```json
// ~/horde/.encampment.json
{
  "owner": "steve@example.com",
  "name": "main-encampment",
...
bd audit --actor=greenplace/clan/joe
```

## Implementation Status

- [x] Agent identity in git commits
- [x] BD_ACTOR default in relics create
- [x] Workspace metadata file (.encampment.json)
- [x] Cross-workspace URI scheme (hop://, relics://, local forms)
- [ ] Remote registration
- [ ] Cross-workspace queries
...
```
Alice's Encampment → works on → Project X
Bob's Encampment   → works on → Project X
```
...
Rally contractor delegates to subcontractors:
